...
> **CORRECTION 2026-09-19 (session 7, after the exact-Opus slice read F-1 REQUEST_CHANGES):** every RED/GREEN arm below ran on the shell's `python` = **Python 3.14.2**, NOT on the pinned lane interpreter (Python 3.12.12 at `C:/tmp/P020_IMPL_20260912/.../.venv`). On 3.14 `json.loads` accepts 3000 levels and the walk raises; on the pinned 3.12 `json.loads` raises FIRST at 3000, so on the pinned interpreter the two N4 walk-guard arms ("walk guard removed -> 1 error") do NOT reproduce (both checkers stay green with any walk guard deleted) and the "parser-clause mutant SURVIVED - superseded" rows are inverted (the parser clause is the only layer the depth-3000 arm reached there). The N1/N3/N3b/F8 arms are unaffected by the interpreter. Repaired in round 2 (`b4df1413`): depths derived per layer at test time, RED on BOTH interpreters, third adapter parse site guarded, receipt-side race arm - see `REPAIR_R2_20260919/LEAD_VERIFICATION_P030_NIT_R2.md`. Lesson: memory `red-evidence-on-the-pinned-interpreter`.
