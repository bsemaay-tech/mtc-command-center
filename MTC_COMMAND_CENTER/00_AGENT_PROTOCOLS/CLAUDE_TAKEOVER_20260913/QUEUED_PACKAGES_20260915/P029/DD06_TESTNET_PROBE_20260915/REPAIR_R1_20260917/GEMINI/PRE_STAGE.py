"""Stages the Gemini packet DD06_R1_20260917: DETECTION/delta review of the DD-06 probe T0 repair round 1
(1af85067 -> a46b2a9d: second execution gate, offline fixture suite, refusal-class narrowing, whose-funds-moved
attribution, abort propagation). Idempotent; refuses while agy runs."""
import pathlib, shutil, hashlib, subprocess, sys, re
DST = pathlib.Path("C:/LAB/Tradingview_LAB_CLEAN/_gemini_packets_20260913/DD06_R1_20260917")
WT = "C:/tmp/P029_DD06_20260915"
RUN = pathlib.Path("C:/tmp/CLAUDE_P0_RUN_20260913/DD06_TESTNET_PROBE_20260915/REPAIR_R1_20260917")
...
(DST / "subject/DIFF_1af85067_a46b2a9d.patch").write_bytes(git("diff", PRE, HEAD))
(DST / "subject/DIFF_STAT_1af85067_a46b2a9d.txt").write_bytes(git("diff", "--stat", PRE, HEAD))
(DST / "subject/COMMIT_a46b2a9d.txt").write_bytes(git("log", "-1", "--format=%H%n%an <%ae>%n%ci%n%n%B", HEAD))
...
shutil.copy2(LANE / "ATTEMPT1_REQUEST_CHANGES_1af85067" / "OPUS_T0_REPORT.md", DST / "sources/OPUS_T0_REPORT_attempt1_1af85067.md")
