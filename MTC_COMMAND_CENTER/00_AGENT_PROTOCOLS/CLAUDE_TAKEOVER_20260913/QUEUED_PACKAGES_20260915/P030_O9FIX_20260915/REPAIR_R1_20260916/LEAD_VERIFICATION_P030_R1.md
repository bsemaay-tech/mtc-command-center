...
## Correction 2026-09-17 (second exact-Opus read, N5)
The "Sibling checkers … `check_p030_closed_partition_backup_adapter.py` OK" row above carries an unstated precondition: it was run from a shell whose `TEMP` is the 8.3 short form `C:\Users\BARSEM~1\AppData\Local\Temp` (ASCII). Under the owner's long-form `TEMP` (`C:\Users\Barış…`, non-ASCII) that checker FAILS on `test_isolated_config_redirect_after_check_never_reaches_restore` (`AssertionError: 0 != 1`, exit 1) — the F8 defect already logged as out of scope (`ensure_ascii=False` in the adapter's config write vs the test's default `json.dumps` anchor). Reproduced by the Lead on 2026-09-17: ASCII `TEMP` → `Ran 44 tests … OK`, exit 0; long-form `TEMP` → `FAILED (failures=1)`, exit 1. The row is therefore true only with an ASCII `TEMP`; F8 stays a carried engineering follow-up (fix the test anchor or the adapter's `ensure_ascii`), outside the `45a7f50e` candidate (both files byte-identical to the base).
