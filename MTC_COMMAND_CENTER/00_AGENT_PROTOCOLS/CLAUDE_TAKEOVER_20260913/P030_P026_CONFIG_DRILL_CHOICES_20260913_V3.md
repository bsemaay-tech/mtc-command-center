...
> **P030/P026 config and drill choices packet V3** — READY FOR OWNER DECISIONS. Source `C:/tmp/CLAUDE_P0_RUN_20260913/laneFX30C_fix3/P030_P026_CONFIG_DRILL_CHOICES_V3.md` sha256 `fc8e49ed93ef30a094db9a6a488f7fca8b85d266e1c931efc42e61594a8ad226`. Audit chain: V1 draft (Claude Max lane) -> Grok audit BLOCKING -> V2 (Claude) -> Grok re-audit CORRECTIONS_NEEDED -> V3 (Claude) -> Grok delta audit CLEAN (laneGK30C). Lead mechanical sweep: 348/348 citations resolve. Owner items O-1..O-10 unchanged in numbering; the ratified daily backup cadence is now honoured in C-5/O-2. This supersedes the earlier DRAFT recorded in this folder. Nothing here is a decision, acceptance or authorization; every item is a question with a recommended answer.

# P030 / P026 — configuration and drill choices (V3, corrected), with verification of Luna's decision packet

**Lane:** P30-LUNA (drafting) · FX30 (V2 corrections) · FX30C (V3 corrections) · **Role:** independent drafting analyst (`claude-opus-5`, xhigh), corrected by two separate corrections-editor lanes (`claude-opus-5`, xhigh). None of the three is a reviewer of record, an implementer, or the Lead. **Nothing here decides anything.**
...
This file is V3 of `C:/tmp/CLAUDE_P0_RUN_20260913/laneP30D_luna/P030_P026_CONFIG_DRILL_CHOICES_DRAFT.md` (the V1 draft; also published with a Lead audit note as `C:/CT13/MTC_COMMAND_CENTER/00_AGENT_PROTOCOLS/CLAUDE_TAKEOVER_20260913/P030_P026_CONFIG_DRILL_CHOICES_20260913.md`, whose line numbers run seven ahead of the draft's). Two corrections passes stand behind it, each against an independent detection audit of the version before it:

- **V2** — `C:/tmp/CLAUDE_P0_RUN_20260913/laneFX30_fix/P030_P026_CONFIG_DRILL_CHOICES_V2.md`, corrected against the detection audit `C:/tmp/CLAUDE_P0_RUN_20260913/laneGK30_grok_audit/GROK_AUDIT_REPORT.md` (grok-4.6, findings 1-13: one BLOCKING, eight CORRECTION, four NIT; report verdict `GROK_AUDIT_VERDICT: BLOCKING`).
- **V3** — this file, corrected against the re-audit `C:/tmp/CLAUDE_P0_RUN_20260913/laneGK30B_grok_reaudit/GROK_REAUDIT_REPORT.md` (grok-4.6; report verdict `GROK_REAUDIT_VERDICT: CORRECTIONS_NEEDED`). Its section (a) re-opened all thirteen prior findings against the V2 bytes and records every one **RESOLVED**, with none REGRESSED; its section (b) raises **three new findings** — two CORRECTION and one NIT — and all three are FIXED here.

**How V2 was produced.** Every finding's cited source was re-opened in that pass and re-checked line by line against the file as it exists on 2026-09-13 — no pin was accepted from memory or from the audit's summary. Twelve findings were confirmed and corrected; one (Grok 11) is rebutted with the source line quoted. Per-finding dispositions are in `C:/tmp/CLAUDE_P0_RUN_20260913/laneFX30_fix/DISPOSITION.md`. That pass was read-only outside its two output files: **no Git command of any kind (not even `git status`), no code execution, no test run, no network, no host, no credential, no account action, no drill.** Corrections were made by editing text and re-reading files only.

**How V3 was produced.** The same method. Every new finding's cited source was re-opened in this pass and re-checked line by line against the file as it exists on 2026-09-13 — [PLAN]:570-649 (the whole WP-P0-26 and WP-P0-30 blocks), [WT] `p030_closed_partition_backup_adapter.py`:218-297 and 365-407, [WT] `market_data_collector.py`:262-277, [PKT]:28-33, and the V1 draft's C-4, C-5, O-1, O-2, O-3, O-6, O-7 and O-8 text — and the V2 lines each finding names were re-read in the V2 file before anything was changed. No pin was accepted from the re-audit's summary. **All three new findings are FIXED; none is rebutted.** One re-audit observation that is *not* one of its findings was re-checked and **not** changed: its section (c) sample 25 calls C-1's `[PLAN]:633` retention pin an off-by-one, but in the file as it exists now :633 **is** the WP-P0-30 **Objective** line and does carry "the venue serves only a rolling retention window, empirically ≈ 5000 candles per interval — 15m ≈ 52 days"; :632 is the map-#67 amendment and :634 is Inputs. The V2 pin is right and stands. Per-finding dispositions are in `C:/tmp/CLAUDE_P0_RUN_20260913/laneFX30C_fix3/DISPOSITION_V3.md`. This pass was read-only outside its two output files: **no Git command of any kind (not even `git status`), no execution of any inspected package, no test run, no network, no host, no credential, no account action, no drill.** Sources were read and nothing else; the two output files were written with ordinary file-copy and text-splice commands (`cp`, `cat`, `sed -n`) applied only to this lane's own output and scratch files — this file was seeded as a byte copy of the V2 file and edited from there — and no file in the repository or in any inspected tree was modified.

**What neither pass did.** No owner decision item was added, removed or renumbered: O-1..O-10 keep their numbers in both passes. No new drill, no new number and no new scope was introduced in either pass. The NOT VERIFIED section only grew — V2 added NV-14..NV-16 and corrected NV-8 and NV-12; V3 extended NV-14 and NV-15 — and nothing was removed from it in either pass. No Lead-verified finding was softened.

**What did change, item by item *(re-audit (b) 3 — NIT, fixed here)*.** V2's own version of the paragraph above went further and claimed that the owner items keep "their meanings" and that "no new recommendation" was introduced. Both are false as written. This is exactly what changed:

| What changed | V1 draft | after V2 | why |
|---|---|---|---|
| **C-5's recommendation** | "**Recommendation: A** as the ratified default; B only after it has its own drill (§3, D-3b) and once the daily cadence in [PLAN]:581(b) is switched on" (V1 draft:169) | **C** — high-water captures at the plan's already-ratified daily cadence plus a month-end capture | Grok 1 (BLOCKING, Lead-verified): the daily cadence is ratified and preserved unchanged, not a switch to be thrown later |
| **O-2's decision** | `<BACKUP_ROOT>` "plus confirmation of the store shape" (V1 draft:521) | the same **plus the capture cadence that shape will run at**, with monthly-only put to the owner as an explicit departure | the same finding: the cadence question had to reach the owner |
| **O-6's answer** | "Revisit immediately after O-5/O-8 rather than after full P026 acceptance" (V1 draft:525) | a revisit decides **only when to look again**; any actual lift still sits behind [PLAN]:638 and [HO]:40 | Grok 5: the V1 wording read as moving an edge it cannot move |
| **O-7's answer** | "**Ratify the implemented semantics** … No new code, no new numbers" (V1 draft:526) | the same answer, with the persisted-WS_LIVE-gap residual now stated in the owner's own cell | Grok 6 |
| **O-8's approval** | "**Approve**, with the explicit fence: scratch fixtures only …" (V1 draft:527) | the same fence **narrowed by one carve-out** — D-13's GREEN half is outside O-8 | Grok 9 |

No other owner item's decision, recommended answer or meaning changed in V2, and **V3 changes none of them**: V3's three fixes touch C-5's justification, C-4's draft store path, and this section. The only owner-facing text V3 edits is wording inside O-2's cell — the cadence comparison and the per-capture path — and that changes neither the decision asked there nor the recommended answer.

**V2 pass — the thirteen GK30 findings.** The line numbers in the last column address the **V2 file** (`laneFX30_fix/P030_P026_CONFIG_DRILL_CHOICES_V2.md`), which is unchanged and still exists; this file's own line numbers differ.

| Grok # | severity | what changed in V2 | V2-file lines |
|---|---|---|---|
| 1 | BLOCKING | C-5 no longer presents the plan's daily cadence as a future switch; its recommendation now conforms to the ratified daily duty, with the plan text quoted. The §2 summary row for C-5 and D-3b's role line follow it. O-2 now states the cadence, quotes [PLAN]:580 and :581(b), and puts a monthly-only shape to the owner as an explicit departure decision. | 182, 191, 195, 197, 199, 297, 345, 551, 586 |
| 2 | CORRECTION | O-1's "why an agent cannot decide it" no longer points at `p030_opsa_backup_config.json`:3 (`backup_root`, which is O-2's second location); it points at the collector's `MonthlyArchive(root)` constructor argument, which has no config field at all. | 550, 141 |
| 3 | CORRECTION | C-4 constraint 1 corrected: three of the four values are `null`; `class` already ships as `"protected"`. The "cannot load" conclusion is unchanged and is re-derived from the three nulls. | 155, 93, 178, 296 |
| 4 | CORRECTION | O-3 now carries, in the owner's own row, that `300` s and the five-minute cadence have no repository basis (only the 60 s emission interval is a shipped default), and that ratifying a bound or a cadence authorizes no schedule installation. | 552, 230, 300, 561 |
| 5 | CORRECTION | O-6's revisit clause now states P030's declared dependency on WP-P0-26 ([PLAN]:638) and [HO]:40's "P026 monitoring/delivery acceptance"; the revisit is decision timing only and lifts nothing. C-1 carries the same pointer. | 555, 125, 293 |
| 6 | CORRECTION | O-7 now carries the residual that C-12 already recorded: option A does not close the persisted WS_LIVE-gap interaction, D-11 must run first, and [HO]:40's restart item is only partly closed. | 556, 281, 304 |
| 7 | CORRECTION | D-15's evidence pin moved from the unread `[DRILL]:20-170` to `[LANE]:95-100`, which this lane and the audit both read; NV-12 extended. | 427, 582 |
| 8 | CORRECTION | The "What was done here" sentence now matches §7: read-only `git rev-parse`/`git log`, `find`/`grep` and SHA-256 hashing were used. The true negatives (no mutation, no `status`/`diff`, no network, no host, no credential, no execution of the inspected packages) are kept and stated exactly. | 30, 630 |
| 9 | CORRECTION | O-8's fence now excludes D-13's GREEN half, whose transform is the O-9 implementation; under O-8 alone D-13 runs its RED half only. | 557, 410, 416, 451, 521 |
| 10 | NIT | §1 carries a recomputed verdict tally (the wrong tally is in the Lead audit note attached to the CT13 copy, which is outside this file and unchanged by it). | 68 |
| 11 | NIT | **Rebutted** — no text change beyond recording the re-check. `[GH]:10` does carry the PASS counts; the quote is in `DISPOSITION.md`. | 79 |
| 12 | NIT | C-7's `interval ≤ bound / 3` is now labelled as this document's reasoning with no repository basis, beside the coded rule it is not. | 211, 299 |
| 13 | NIT | NV-8's suite sizes corrected to the counts read in this pass: 1792 and 2028 lines. | 578, 625-628 |

**V3 pass — the three GK30B re-audit findings.** These line numbers address **this file**.

| re-audit (b) # | severity | what changed in V3 | V3 lines |
|---|---|---|---|
| 1 | CORRECTION | C-5's option C no longer claims to be the only option that can meet the ratified daily duty. The row now states which options do meet it — **B run daily, and C** — which one does not — **A alone**, which leaves up to a month with no second-location copy — and what C actually adds over B: the month-end capture, whose closure is self-evident from the filename rather than resting on the caller's byte arithmetic. The recommendation is unchanged (**C**); its justification, the §2 summary row and O-2's cell are corrected to match. | 226, 228, 326, 580 |
| 2 | CORRECTION | C-4's draft store `path` is no longer a fixed monthly directory: the example carries a per-capture element, and option B's "cadence-neutral" claim is restated as what it is — the per-capture *mechanism* is cadence-neutral, while one reused directory is refused at any cadence and unavoidably so under daily captures ([WT] `p030_closed_partition_backup_adapter.py`:256-257, 393-407). C-4's recommendation wording, C-13's config-location rule, the §2 summary row and O-2's cell follow. | 192, 195, 203, 209, 316, 325, 580 |
| 3 | NIT | The corrections section is now a **V2/V3 corrections** section that states what each pass changed, including the one recommendation that changed (C-5, A → C) and the four owner items whose meaning V2 changed (O-2, O-6, O-7, O-8). The "keep their meanings / no new recommendation" sentence is gone. | 6, 8-11, 15, 17, 19-29, 31, 49-57, 575 |

**Re-checked and not changed *(re-audit section (c), sample 25 — not one of its findings)*.** C-1's `[PLAN]:633` retention pin was flagged there as an off-by-one; it is correct as the V2 pass wrote it and stands unchanged. The quoted source line is in `DISPOSITION_V3.md`.

**What was done here (V1 lane, corrected in V2).** Read-only inspection of the packet, the two handoffs, the merged P030 worktree, the P026 tooling and the plan/decision indexes. The read-only tooling used is exactly what §7 records: `git rev-parse` and `git log` (**no `git status`, no `git diff`, no mutation of any kind**), `find` and `grep -rln` over the inspected trees, SHA-256 hashing of the ten files in the table below, and file reads. **Nothing else was executed**: no test run, no invocation of the collector, adapters, watchdog, heartbeat, backup or restore tools, no runtime start, no network, no host, no credential, no account action. Every drill in section 3 is a **paper plan**; none was started. *(Corrected in V2 — the V1 sentence read "no tool invocation", which its own §7, its verification-basis table and §4.1 contradict; Grok 8.)*
...
**V2/V3 note on this table.** None of these identities was re-established in the V2 pass or in the V3 pass: no Git command and no hashing was run in either (NV-14). They stand exactly as the V1 lane recorded them.
...
**Tally of the 35 rows below, recounted in V2 (Grok 10):** **23** rows VERIFIED outright (2, 4-7, 9-11, 13-17, 19, 20, 22, 24, 27-30, 32, 33); **2** rows VERIFIED in part with the remainder UNVERIFIABLE-HERE (1, 8); **6** rows PARTLY VERIFIED (3, 12, 21, 23, 25, 34); **2** rows CONTRADICTED in part (18, 26); **2** rows CONTRADICTED (31, 35). 23 + 2 + 6 + 2 + 2 = 35. *(The Lead audit note prepended to the CT13 copy reads "21 VERIFIED, 5 PARTLY VERIFIED"; that note is not part of this file and cannot be edited from here — see `DISPOSITION.md` row 10. Its CONTRADICTED counts are right.)*
...
| 8 | The handoff records collector/contracts PASS, backup `44/44`, heartbeat `10/10`, unchanged P026 `33/33` ([PKT]:9) | [HO]:32; [GH]:10; test-method counts in [WT] `check_p030_closed_partition_backup_adapter.py`, `check_p030_opsa_heartbeat_adapter.py`, `MTC_COMMAND_CENTER/tools/opsa/test_opsa.py` | VERIFIED (record + counts) / UNVERIFIABLE-HERE (results) | The handoff says exactly that. The three suites contain exactly **44**, **10** and **33** `def test_…` methods, so the denominators are real, not rounded. The PASS results themselves cannot be re-established here — no execution is authorized in this lane. **V2: the `[GH]:10` pin was re-read and is correct** — line 10 is the PR #187 record and carries "collector PASS, contracts PASS, backup `44/44`, heartbeat `10/10`, unchanged P026 `33/33`" (Grok 11 rebutted). |
| 9 | The handoff "explicitly says this is an implementation milestone rather than package acceptance" ([PKT]:9) | [HO]:37 | VERIFIED | "Completed merges are scoped implementation milestones, NOT P021/P030 package acceptance or readiness." Same substance at [GH]:16. |
...
**Recommendation: B for this lane; put A to the owner as a separate scope request (§5, O-6).** Lifting a coded refusal is an implementation act with its own review; it must not ride along on a configuration draft. The retention cost of waiting is real and belongs in the owner's decision, not in an agent's discretion. **Sequencing note (V2, Grok 5):** option A is additionally behind P030's own declared dependency — "**Depends on:** WP-P0-26 (watchdog + backup for the archive), **WP-P0-21**" ([PLAN]:638) — and behind [HO]:40, which lists "P026 monitoring/delivery acceptance" among what P030 still needs. Raising A as a scope request does not move that edge.
...
The archive root is the `MonthlyArchive(root)` argument ([WT] `market_data_collector.py`:264-266); the layout under it is fixed: `bars/<venue>/<symbol>/<interval>/<YYYY-MM>.jsonl`, every segment passed through `_SAFE_PART = ^[A-Za-z0-9._-]+$` ([WT] `market_data_collector.py`:268-276, 54, 158-161). Files are opened `O_APPEND|O_CREAT|O_WRONLY` mode `0o600` and fsynced per record (294-306).
...
1. `backup_root` and every store's `id`, `path`, `class` must be **non-empty strings**. **Three of those four are `null` in the shipped file** — `backup_root` (:3), store `id` (:6) and store `path` (:7); **`class` already ships as `"protected"` (:8)**, which is exactly the value the adapter requires ([WT] `p030_closed_partition_backup_adapter.py`:402-403), so there is no class choice left to make. The three nulls are enough on their own: the file cannot load ([WT] `p030_opsa_backup_config.json`:3, 6, 7, 8; `opsa_common.py`:227-245, and `require_non_empty_string_field` at 232, 238-239). *(Corrected in V2 — V1 said `null` "for all four", contradicting its own table row 22; Grok 3.)*
...
| B. One config **generated per capture**, pointing at that capture's stable-prefix directory, all sharing one `backup_root` (hence one append-only `manifest.jsonl` and distinct timestamped `run_id`s, `opsa_common.py`:80-87) | Works with the code as shipped. Restores stay addressable by `run_id`, and `restore_verified_prefix` re-validates only class/uniqueness in the restore direction ([WT] `p030_closed_partition_backup_adapter.py`:685 with 393-407), so an old per-capture config stays usable later. **The mechanism is cadence-neutral; a fixed directory is not.** One generated config per capture works the same whether captures run daily or monthly (C-5) **only because each capture points at its own stable-prefix directory**: constraint 4 forbids reusing one directory at any cadence, and daily captures of the same monthly partition make a per-capture path element unavoidable. *(Restated in V3 — V2 called this option "cadence-neutral" without that condition while its own draft path below was a fixed monthly directory; re-audit (b) 2.)* |
...
**Recommendation: B, with a fixed store `id` and a per-capture stable-prefix directory.** Draft shape for owner review:
...
```
{
  ...
  "stores": [
    { "id": "p030_closed_partition",
      "path": "<STABLE_PREFIX_ROOT>/HYPERLIQUID/BTC/1h/<CAPTURE_LABEL>",
      "class": "protected" }
  ]
}
```

`backup_root` — OWNER, the second location, not the archive volume. `id` — LEAD DEFAULT, one segment, stable across captures. `path` — **one directory per capture**: it must not exist before its own capture (constraint 4) and must resolve equal to the stable prefix that capture is given ([WT] `p030_closed_partition_backup_adapter.py`:256-257, 393-407). `<CAPTURE_LABEL>` is that per-capture element, and **this document does not fix its format** — any label is legal to the code provided no two captures resolve to the same directory. *(Corrected in V3 — V1 and V2 wrote the partition name `2026-09` in this slot, which is a fixed directory reused across captures; the adapter refuses the second capture into it, and at the daily cadence C-5 recommends the second capture is the second day. Re-audit (b) 2.)* `class` — `protected` is required by the adapter and is already the shipped value. `<BACKUP_ROOT>` must sit on a **different device or host** from `<ARCHIVE_ROOT>`; that is the point of [PLAN]:580 ("automated second-location copy for every evidence store (cross-copy VPS ↔ owner PC)"). **Owner input needed:** both roots (§5, O-1, O-2).

### C-5 · Stable-prefix capture parameters — LEAD DEFAULT **inside a cadence the plan has already ratified**
...
**The capture cadence is not this document's to choose, and it is not a future switch (V2, Grok 1 — BLOCKING, Lead-verified).** The plan's Outputs for WP-P0-26 state the duty in force: "automated second-location copy for **every evidence store** (cross-copy VPS ↔ owner PC; text-class also to GitHub), **daily backups** with hourly ledger/admission sync during active windows" ([PLAN]:580). The safety-operations amendment confirms it is already ratified and unchanged: "the **already-ratified daily all-store** and hourly active-window critical-ledger cadence is **preserved unchanged**" ([PLAN]:581(b)). WP-P0-30 places this archive under that policy: "the archive on disk under **WP-P0-26's second-location backup policy**" ([PLAN]:635). The V1 draft's phrase "once the daily cadence in [PLAN]:581(b) is switched on" read a preserved ratified cadence as something to enable later; it is neither switchable by a Lead default nor pending.

| Option for "which byte count is closed" | Consequence |
|---|---|
| A. Month boundary — capture the whole `<YYYY-MM>.jsonl` after the month rolls over | Matches "closed monthly partitions" ([PKT]:30) and the collector's layout. One capture per interval per month; the "closed" claim is self-evident from the filename. Cost: up to a month of collector output with **no second-location copy** — which is a **departure from the ratified daily all-store cadence** ([PLAN]:580, 581(b)), not a Lead default. |
...
| C. Both — periodic high-water captures plus a month-end capture | Best recovery-point objective, most receipts to manage. **Which options meet the ratified daily duty:** [PLAN]:580 requires "automated second-location copy for **every evidence store** … **daily backups**", so **B run daily meets it, and so does C**; **A alone does not**, because it leaves up to a month of collector output with no second-location copy. What C adds over B is therefore not plan-conformance but the month-end capture, whose "closed" claim is self-evident from the filename ([PKT]:30's "closed monthly partitions") where a high-water capture rests on the caller's byte arithmetic — the adapter refuses to infer closure (229). *(Corrected in V3 — V2 said C was "the only option in this table that can meet the ratified daily duty"; [PLAN]:580 requires daily backups, not a month-end closed-file capture, and the row above defines B as backing up sooner. Re-audit (b) 1.)* |

**Recommendation: C, at the cadence the plan already ratifies — high-water captures on the ratified daily all-store cadence ([PLAN]:580, 581(b)) plus a month-end capture at the month boundary.** **Why C rather than B alone, stated honestly (V3):** both meet the ratified daily duty once the high-water captures run daily, so this is not a conformance argument; what C adds is the month-end capture, whose closure is self-evident from the filename rather than resting on the caller's byte arithmetic ([PKT]:30; the adapter refuses to infer closure, 229). **B run daily is not a departure from the plan — only A alone is.** C's precondition is D-3b (§3), which is the drill that proves the boundary-aligned and mid-record offsets behave as the code says; until D-3b has run, no high-water capture should be taken against real data. **No new number is introduced here: "daily" is the plan's ratified cadence, quoted above, not a Lead choice.** If the owner prefers the monthly-only shape (option A), that is an **explicit departure from an already-ratified cadence** and belongs to the owner, with the plan text in front of them — it is put in exactly those terms in §5, O-2. This document does not take that departure and does not recommend it.
...
**Recommendation: adopt 60 s.** One atomic write per minute (`heartbeat.py`:46-53), and it makes the silence bound in C-8 expressible as a whole number of missed beats. **On choosing an interval for a different bound:** the only coded rule is the alert test itself — the watchdog alerts when `age > silence_seconds` ([WT] `watchdog.py`:115-118) — and `60.0` is a CLI default, not a contract. The `interval ≤ bound / 3` guidance the V1 draft offered here is **this document's own reasoning with no repository basis** (it keeps a single missed beat from alerting); it is not a rule any source states, and an owner picking another bound is not bound by it. *(Labelled in V2 — Grok 12.)*
...
**Recommendation: put option A's concrete candidate — `--silence-seconds 300` with a 60 s emission interval and a 5-minute check cadence — to the owner for ratification (§5, O-3), and run option C until that ratification exists.** Rationale, offered as reasoning and not as a repository fact: five consecutive missed beats is the smallest count a single write stall cannot produce, and 300 s leaves headroom under any plausible N14 once delivery latency is measured. **Neither `300` nor the five-minute check cadence has any repository basis; both are proposals, the only sourced number in this entry is the 60 s emission default (`heartbeat.py`:76), and the code will keep refusing to assume a bound.**
...
**Recommendation: A** (§5, O-7). It ratifies behaviour that already exists and has tests, keeps every refusal fail-closed, and converts "restart-first-message closure" from an open engineering question into a recorded owner policy without a line of new code. What A does **not** close: the persisted-history gap check (374-413) refuses a restart whenever the archive holds any WS_LIVE gap — including one created by a legitimate outage later backfilled by `CANDLE_SNAPSHOT`, because the check filters to `source_producer == "WS_LIVE"` before looking for gaps (378). That interaction needs a fixture drill before any real restart (§3, D-11). **This residual is carried in the owner's own row at O-7 (V2, Grok 6), so that ratifying A is not read as closing [HO]:40's "general restart-first-message closure beyond identical replay".**
...
**Recommendation:** keep generated configs beside the backup root under `configs/<run label>.json`, never inside **a capture's own stable-prefix directory** (which must not exist before that capture — C-4 constraint 4; writing a config there would create the directory and the capture would then refuse), and have the freeze step assert that no value resolves outside `<BACKUP_ROOT>` or `<STABLE_PREFIX_ROOT>`. This is the packet's step 2 ([PKT]:48) made concrete. *(Wording corrected in V3 — V2 said "never inside `<STABLE_PREFIX_ROOT>`"; what must not pre-exist is the per-capture directory, not the root above it, which `stable_prefix.mkdir(parents=True)` creates or finds ([WT] `p030_closed_partition_backup_adapter.py`:256-257, 267). Re-audit (b) 2.)*

### Summary of section 2

| Ref | Configuration | Decider | Recommended choice |
|---|---|---|---|
| C-1 | Collector backend + permission protocol | OWNER | Keep the coded refusal; raise backend/permission as a separate scope (O-6), which does not move P030's declared dependency on P026 ([PLAN]:638) |
...
| C-4 | Backup root / store id / path / class | OWNER (values), LEAD (shape) | One config **generated per capture**; fixed store id `p030_closed_partition`; **store `path` a fresh directory per capture, never a reused one**; class `protected` (already the shipped value); shared `backup_root` (O-2) |
| C-5 | Stable-prefix capture parameters | LEAD **inside the plan's ratified cadence** | Daily high-water captures plus a month-end capture — a shape that conforms to the already-ratified daily all-store cadence ([PLAN]:580, 581(b)), as daily high-water captures alone also would; the month-end capture is what option C adds. D-3b is its precondition; monthly-only is an owner departure decision (O-2) |
| C-6 | Heartbeat id + state dir | LEAD | `p030_market_data_collector` in a dedicated `hb` directory |
| C-7 | Heartbeat emission interval | LEAD | 60 s (shipped default); the `bound / 3` guidance is this document's reasoning, not a sourced rule |
| C-8 | Watchdog silence bound | **OWNER** | Propose `300` s for ratification (no repository basis); `local_log`-only until ratified (O-3) |
...
| C-12 | Restart-first-message policy | **OWNER** | Ratify the implemented refuse-and-require-CORRECTION semantics (O-7), with the persisted-gap residual stated in O-7 |
...
**Does NOT prove.** That any particular production offset is the right one — that arithmetic belongs to the caller, which is why C-5 pairs the high-water captures with a self-evident month-end capture.
**Authorization.** T-A. **This drill is the precondition for C-5's recommended cadence-conforming capture** (it is the high-water half of option C), so it is on the critical path rather than an optional variant. *(Role restated in V2 — Grok 1.)*
...
### D-13 · Producer-to-contract identity bridge drill — T-A (RED half) · **prerequisite for D-3…D-7 on real data**
...
**Authorization.** T-A for the **RED half only**. The GREEN half cannot run under an execution scope alone: **the transform itself is new code and needs the O-9 implementation scope**, so an offline-execution approval (O-8) authorizes the RED half and stops there. *(Split stated in V2 — Grok 9.)*
...
**Steps.** The [PLAN]:585 shape: show an un-backed-up loss unrecoverable, then restore the backed-up copy byte-identically. The 2026-08-25 lane already reported this **on fixtures** ([LANE]:95-100: "dry-run-writes-nothing PASS; backup+readback PASS; RED unrecoverable-without-backup PASS (0/3 baseline hashes remain in damaged live tree); GREEN restore PASS (3/3 byte-identical, empty dir preserved); tampered-backup refused PASS (rc 1)"). *(Re-pinned in V2 — the V1 draft cited `[DRILL]:20-170`, which its own NV-12 records as unread; the command transcripts do live in [DRILL], but neither the V1 lane nor this one re-read them, and the summary above is from [LANE], which both read. Grok 7.)*
...
D-1 config validation
  └─ D-2 dry-run
       └─ D-13 identity bridge  ── RED half under O-8; GREEN half needs O-9
            └─ D-3 capture ─ D-3b high-water variant  ← C-5's cadence precondition
...
now ─┬─ C-1…C-13 configuration choices (this document)
     │
     ├─ owner decisions O-1…O-10 (§5)
     │
     ├─ [T-A execution scope] D-1 config validation → D-2 dry-run
     │        → D-13 identity bridge RED half  ← §4.1 prerequisite; its GREEN half needs O-9
...
Only decisions that genuinely cannot be made by an agent are listed. Each carries a recommended answer, so the owner can reply with a short line per item. **Nothing here has been acted on.** Item numbers are unchanged from the V1 draft and no item was added in either pass; V2 corrected the text of O-1, O-2, O-3, O-6, O-7 and O-8, and in doing so changed what O-2 asks (the cadence), what O-6's revisit means and what O-8's approval covers — the V2/V3 corrections section lists those changes. V3 corrected wording inside O-2's cell only (the cadence comparison and the per-capture store path) and changed no item's decision or recommended answer.

| Ref | Decision needed | Why an agent cannot decide it | Recommended answer |
|---|---|---|---|
| **O-1** | `<ARCHIVE_ROOT>` — the volume and path for the market-data archive, plus a free-space budget | It is a host/infrastructure fact, and **no repository value exists to inherit**: the archive root is a plain constructor argument, `MonthlyArchive(root)` ([WT] `market_data_collector.py`:264-266), with **no field in any config file**. It is **not** `p030_opsa_backup_config.json`'s `backup_root` (:3) — that is O-2's second location, and filling it would collapse the two-volume design this table keeps separate. *(Pin corrected in V2 — Grok 2.)* | Name the target volume now as a placeholder and bind the concrete path at the same moment the `G9` host step is authorized (C-3). Layout is fixed: `<ARCHIVE_ROOT>/bars/HYPERLIQUID/BTC/<interval>/<YYYY-MM>.jsonl` |
| **O-2** | `<BACKUP_ROOT>` — the second location, on a **different device or host** from O-1; confirmation of the store shape; **and the capture cadence that shape will run at** | Same: a host fact. The "second location" duty is the plan's, the value is the owner's ([PLAN]:580). **The cadence is also the plan's and is already ratified, not open:** [PLAN]:580 requires "automated second-location copy for **every evidence store** … **daily backups**", [PLAN]:581(b) records that "the **already-ratified daily all-store** … cadence is **preserved unchanged**", and [PLAN]:635 puts this archive "under WP-P0-26's second-location backup policy". An agent may neither relax that cadence nor treat it as a future switch — so if a slower cadence is wanted, only the owner can take that departure, knowingly. *(Cadence conflict stated in V2 — Grok 1, BLOCKING, Lead-verified.)* | Approve the C-4 shape: one config **generated per capture**, fixed store id `p030_closed_partition`, a store **`path` that is a fresh directory per capture** (`<CAPTURE_LABEL>`; a reused monthly directory refuses on the second capture — C-4 constraint 4), class `protected` (already the shipped value), shared `backup_root`, configs under `<BACKUP_ROOT>/configs/`. On that condition the shape runs at any cadence. **Approve C-5's cadence-conforming capture** — high-water captures at the ratified daily cadence plus a month-end capture, with D-3b run first. **So that the alternatives are stated accurately (corrected in V3):** daily high-water captures *without* the month-end capture (option B run daily) also meet the ratified daily duty; the month-end capture is what option C adds, and what it adds is a capture whose closure is self-evident from the filename. **Monthly-only (option A) is the one shape that departs from the ratified cadence. If you want monthly-only, answer "monthly-only" explicitly**: that leaves up to a month of collector output with **no second-location copy**, which departs from the ratified daily all-store cadence quoted in the column to the left. It is your decision to take, not a configuration default |
| **O-3** | The watchdog silence bound `--silence-seconds`, and the check cadence | The code refuses to ship a default because no such value is ratified ([WT] `watchdog.py`:241-246); the plan withdrew the old `~15 minutes` figure ([PLAN]:580) | **Ratify `300` seconds** with a 60 s heartbeat interval and a 5-minute check cadence (C-7, C-8). **Neither `300` nor the 5-minute cadence has any repository basis** — they are this document's proposals; the only sourced number here is the 60 s emission default (`heartbeat.py`:76), and "five missed beats" is reasoning, not a rule any source states. **Ratifying a bound or a cadence authorizes no schedule installation and no host contact** — that stays behind `G9` ([PLAN]:584) and is explicitly not requested below. If you prefer to wait for measured delivery latency, answer "defer" and the checker runs `local_log`-only *(labelling corrected in V2 — Grok 4)* |
...
| **O-6** | Collector backend + permission protocol: lift the coded runtime refusal, or keep waiting? | Lifting it needs a ratified permission protocol and a backend; the code refuses unconditionally today ([WT] `market_data_collector.py`:534-538). It is also **not this document's edge to move**: P030 "**Depends on:** WP-P0-26 (watchdog + backup for the archive), **WP-P0-21**" ([PLAN]:638), and [HO]:40 lists "P026 monitoring/delivery acceptance" among what P030 still needs | **Keep the refusal for now**, and note the cost explicitly: every day of delay permanently loses venue candles against the retention window ([PLAN]:632-633). **Revisit the decision timing immediately after O-5/O-8** — but a revisit decides only when to look again: **any actual lift still sits behind P030's declared dependency on WP-P0-26 ([PLAN]:638; [HO]:40) and behind `G9` for anything on the host**, and this answer grants none of that *(dependency stated in V2 — Grok 5)* |
...
**Not requested now, and deliberately so:** `G9` host contact, KVM2 installation, any schedule installation, any credential, any real send, any venue contact, any deployment, any acceptance decision. Those follow O-5/O-8/O-9 in the §4.4 order and each needs its own owner authorization at the time it happens ([PLAN]:584, 640). This applies to O-3 as well: ratifying a bound or a check cadence is a value decision, not authorization to install a schedule.

---

## 6. NOT VERIFIED

Everything in this list is something a next agent must **not** treat as established by this document. V2 extended this section (NV-8 and NV-12 corrected; NV-14, NV-15 and NV-16 added) and V3 extended NV-14 and NV-15; nothing was removed from it in either pass.
...
| NV-8 | That the §4.1 conclusion holds against every fixture in the check suites | §4.1 is derived from the **production** modules (`market_data_collector.py`, `p030_closed_partition_backup_adapter.py`, `p030_market_data_contracts.py`) and the import graph. The two suites behind it — `check_p030_closed_partition_backup_adapter.py` (**1792** lines) and `check_market_data_collector.py` (**2028** lines) — were inspected structurally (class/test counts, entry points), not read line by line. D-13 is what would settle it empirically. *(Sizes corrected in V2 by re-reading both files; V1 said "1791-line and 1600-line", and no inspected suite is ~1600 lines — `check_p030_market_data_contracts.py` is 1248 and `check_p030_opsa_heartbeat_adapter.py` is 249. Grok 13.)* |
...
| NV-12 | The detailed command outputs in [DRILL] | Read as an outline (section headings plus the first 30 lines) by the V1 lane, and re-opened only over the same first 30 lines in the V2 pass. The fixture-drill *claims* summarized in table row 12 and in D-15 come from [LANE], not from re-checking [DRILL]'s transcripts. *(V2: D-15's `[DRILL]:20-170` pin was replaced by `[LANE]:95-100` for exactly this reason — Grok 7.)* |
...
| NV-14 | Everything in the "Verification basis" table and the SHA-256 table, as of the V2 and V3 passes | The V2 corrections pass ran **no Git command and computed no hash** — it was forbidden both. Those identities stand exactly as the V1 lane recorded them and were not re-established here; the byte-identity of the `C:/CT13/` and `C:/tmp/P030_INTEGRATION_20260913/` copies is therefore unre-checked. V2's own re-reads used the `[WT]` worktree copies for code and the `C:/CT13/` copies for [PLAN], [GH], [LANE] and [DRILL]. **The V3 pass ran none of them either** — no Git command, no hash, nothing from the inspected trees executed — and its re-reads used the `[WT]` copies for code, the `C:/CT13/` copy for [PLAN], and the `C:/tmp/` originals for [PKT] and the V1 draft |
| NV-15 | The Grok audit report and the GK30B re-audit report, as wholes | The audit's section (c) marks its own coverage: no Git command, no hash, [DRILL] beyond line 30 unread, no execution. V2's dispositions rest on that lane's own re-reads of the named sources, not on the audit's sampling; one audit finding (11) is rebutted on the bytes. The re-audit's section (d) records the same limits for itself. **V3's dispositions likewise rest on this lane's own re-reads**, and one re-audit observation outside its findings (its section (c) sample 25, the `[PLAN]:633` retention pin) was re-checked and found correct as V2 wrote it. Any part of this packet that neither audit sampled is no more verified than before |
| NV-16 | That C-5's cadence-conforming capture is *operationally* achievable at the ratified daily cadence | The duty is a documentary reading of [PLAN]:580, :581(b) and :635 and is quoted, not inferred. Whether a daily capture-and-copy actually completes within a day against the real archive and the real second location is a host fact (NV-3), unmeasured here, and nothing has been scheduled or executed |

---

## 7. Citation index

Source keys used throughout. All paths are absolute; line numbers refer to these exact files as read on 2026-09-13. Every line number touched by a V2 correction was re-opened and re-checked in the V2 pass, and every line number touched by a V3 correction was re-opened and re-checked in the V3 pass.
...
Files read inside [WT] (each byte-identical to its `C:/CT13/` copy — SHA-256 table at the top of this document; not re-established in the V2 pass, NV-14):
...
| `check_p030_closed_partition_backup_adapter.py` | structure only: single `unittest` class, 44 `def test_…` methods (1792 lines) |
| `check_p030_opsa_heartbeat_adapter.py` | structure only: single `unittest` class, 10 `def test_…` methods (249 lines) |
| `check_p030_market_data_contracts.py` | structure only: entry points and `EVENT_FAMILIES` assertion at 922 (1248 lines) |
| `check_market_data_collector.py` (read in `C:/CT13/`) | 69-71, 87-92, 921-928 by targeted search (2028 lines) |

Read-only Git commands used **by the V1 lane** (no `status`, no `diff`, no mutation): `git rev-parse HEAD`, `git rev-parse --abbrev-ref HEAD`, `git log --oneline -1 <sha>`, `git log --oneline <a>..<b>`, `git log --oneline --ancestry-path <a>..<b>`, `git log --oneline -8 --name-only <sha>`. Other read-only tooling used by the V1 lane: `find` over `C:/CT13` (table row 3), `grep -rln` over `*.py` in the merged worktree (§4.1), and SHA-256 hashing of the ten files in the table above. **The V2 corrections pass used none of these** — it ran no Git command of any kind, computed no hash and executed nothing; it read files and edited its own two output files. **The V3 corrections pass used none of them either** — no Git command, no hash, no test run, nothing from the inspected trees executed. It read the sources, and it wrote its own two output files with ordinary file-copy and text-splice commands (`cp`, `cat`, `sed -n`) over its own output and scratch files; this file was seeded as a byte copy of the V2 file and edited from there.

---

**End of V3.** V1 prepared by lane P30-LUNA as an independent drafting analyst; corrected by lane FX30 as a corrections editor against the GK30 Grok detection audit, and by lane FX30C as a corrections editor against the GK30B Grok re-audit. It contains no acceptance decision, no readiness claim, no host or account action, and no executed drill. Every owner item remains open and is the owner's to answer.
