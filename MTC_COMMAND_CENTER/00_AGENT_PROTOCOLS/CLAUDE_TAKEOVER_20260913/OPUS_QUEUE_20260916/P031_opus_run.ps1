# Exact claude-opus-5 xhigh T0 review of WP-P0-31 candidate 48bd70de on the Claude PRO profile (~/.claude), NOT the Max desktop session.
...
if ($head -ne '48bd70dec54aba83dd4270fa0bef97b542163bda') { "HEAD $head != 48bd70de: refuse" | Out-File "$lane\launch.exit" -Encoding ascii; exit 3 }
$start = Get-Date
& claude --print "Read and execute exactly $lane\BRIEF.md. Start by reading that file." --model claude-opus-5 --effort xhigh --permission-mode dontAsk --allowedTools 'Bash' 'Edit' 'Read' 'Write' 'Glob' 'Grep' --disallowedTools 'Agent' 'WebFetch' 'WebSearch' 'NotebookEdit' --add-dir 'C:\tmp\OPUS_QUEUE_20260916\P031' 'C:\tmp\OPUS_P031_SCRATCH' 'C:\tmp\P031_M1_20260913' 'C:\CT13' --strict-mcp-config --max-turns 100 --output-format stream-json --verbose 2>&1 | Out-File -FilePath "$lane\stream.jsonl" -Encoding utf8
