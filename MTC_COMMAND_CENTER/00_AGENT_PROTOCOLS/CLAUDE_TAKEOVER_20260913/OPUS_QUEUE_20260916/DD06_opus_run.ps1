# Exact claude-opus-5 xhigh T0 review of WP-P0-29 DD-06 testnet falsification probe (script + fixture tests; ran once on testnet) candidate 1af85067 on the Claude PRO profile (~/.claude), NOT the Max desktop session. — RE-PINNED 2026-09-17 21:5x to a46b2a9d (T0 repair round 1: second execution gate, offline fixture suite, whose-funds-moved attribution; after the first read returned REQUEST_CHANGES on R-1..R-4 and disclosed the unintended testnet run)
...
if ($head -ne 'a46b2a9db861433e1128dba21fc18362867b5d7f') { "HEAD $head != a46b2a9d: refuse" | Out-File "$lane\launch.exit" -Encoding ascii; exit 3 }
$start = Get-Date
