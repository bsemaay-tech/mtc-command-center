...
- 12:0x UTC+3 owner chat exact words: `DD-06 amend` + `we can do it in mainnet with real money` → amendment applied (`OD-20260916-P029-DD06-AMEND-1`, CT13 `f6579df1`, docs `30e24760`); mainnet proposal answered with the concrete meaning (throwaway agent, ~12 USDC, worst case = own money between own addresses + fees, packet + T0 roster or waiver). 12:2x UTC+3 owner: `leave it for now` → parked, nothing queued.
