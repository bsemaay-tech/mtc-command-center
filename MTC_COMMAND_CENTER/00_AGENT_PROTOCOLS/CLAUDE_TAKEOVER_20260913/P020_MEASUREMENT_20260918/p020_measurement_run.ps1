...
# FIXED 2026-09-18 08:2x UTC+3 (Lead): the roster-gate content variables shadowed the path variables (PowerShell names are case-insensitive: $sol == $SOL), so Sha $SOL hashed the report TEXT and threw before any swap/run; renamed to $solText/$gemText/$leadText/$grokText. Procedure unchanged.
...
$solText = Get-Content $SOL -Raw -Encoding utf8
if ($solText -notmatch 'VERDICT: PASS(-WITH-NITS)?\s*$') { throw 'Sol derived-plan verdict is not PASS/PASS-WITH-NITS: refuse' }
$gemText = Get-Content $GEM -Raw -Encoding utf8
if ($gemText -notmatch 'slot SATISFIED for the derived plan') { throw 'Gemini derived-plan corroboration not SATISFIED: refuse' }
$leadText = Get-Content $LEAD -Raw -Encoding utf8
if ($leadText -notmatch 'RESULT: ALL EQUAL') { throw 'Lead trial-by-trial check not ALL EQUAL: refuse' }
...
$grokText = Get-Content $GROK -Raw -Encoding utf8
if ($grokText -notmatch 'Grok pre-screen slot SATISFIED for the derived plan') { throw 'Grok pre-screen not SATISFIED: refuse' }
