...
| DD-06 | Withdrawal-disabled requirement primary-source mapped and permission-proven, or gate formally amended by owner authority | AMENDED 2026-09-16 by owner authority — chat word `DD-06 amend` (`OD-20260916-P029-DD06-AMEND-1`, 12:0x UTC+3) on the testnet falsification probes r1-r3 (`OD-20260915-P029-DD06-PROBE-STEPS-APPROVED-1`, `OD-20260916-P029-DD06-R2-GO-1`, `OD-20260916-P029-DD06-R3-GO-1`): **restriction VERIFIED on testnet with direct evidence** — a fund-moving action signed with the Bridge's agent key acts on the agent wallet's OWN account (r3, 2026-09-16 09:00Z: agent-signed spotSend moved the agent's own 1 USDC to the master, agent 14.0→13.0 / master +1.0, venue ledger `user = agent`; withdraw3/usdSend refused on the signer's empty balance; the master's funds were never reachable in r1-r3). Records: `QUEUED_PACKAGES_20260915/P029/DD06_TESTNET_PROBE_20260915/LEAD_READING_r2_20260916.md`, `LEAD_READING_r3_20260916.md`. Limits stated: no primary-source capability matrix exists (still NOT DOCUMENTED); mainnet not probed; the same venue rule is exercised on both networks but no real-money measurement was made. The overall disposition HOLD FOR FIRST MAINNET DEPOSIT is unchanged by this amendment. |
