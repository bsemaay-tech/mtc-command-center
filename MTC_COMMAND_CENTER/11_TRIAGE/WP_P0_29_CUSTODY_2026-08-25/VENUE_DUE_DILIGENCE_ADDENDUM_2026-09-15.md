...
- **DD-07 stays BLOCK**; **DD-06 was AMENDED by owner authority on 2026-09-16** (`OD-20260916-P029-DD06-AMEND-1`, word `DD-06 amend`) on the testnet probe evidence below — the primary capability matrix is still not found; the amendment rests on measured venue behaviour, not on documentation.
...
- **Testnet probe r2 (2026-09-16 05:12Z, `OD-20260916-P029-DD06-R2-GO-1`, slice 3 `1af85067`):** control order accepted and cancelled (the tick-size defect fixed); withdraw3 / usdSend / spotSend / approveAgent all REFUSED with `Must deposit before performing actions. User: <address>` — script verdict `DD06_INCONCLUSIVE` by its rule (UNCLASSIFIED text); Lead reading: user-signed actions are attributed to the SIGNER's own (empty) agent account (`LEAD_READING_r2_20260916.md`).
- **Testnet probe r3 (2026-09-16 09:00Z, `OD-20260916-P029-DD06-R3-GO-1`, same script):** after the owner funded the `kvm2-bridge` agent wallet himself (14.0 test-USDC spot, 08:57Z), withdraw3 REFUSED (`Error withdrawing from bridge`), usdSend REFUSED (`Insufficient balance for withdrawal.` — the signer's empty perp), **spotSend 1 USDC NOT refused** (script label `DD06_FINDING_NOT_REFUSED`, stop rule); public Info reads: agent spot 14.0 → 13.0, master spot 983.987457 → 984.987457, venue ledger `spotTransfer` `user = the agent`, `destination = the master`; no order rests; agents unchanged; Bridge DISARMED, NRestarts 0. **Reading: an agent-key-signed fund action acts on the agent wallet's own account; the master's funds were never reachable through the agent key in r1-r3 — the DD-06 restriction is verified on testnet with direct evidence** (`LEAD_READING_r3_20260916.md`). Mainnet not probed; the same venue rule is what the probe exercised. The agent wallet keeps 13 test-USDC.
