...
## 2026-09-15 night — Claude Opus 5 Lead, session 5 of the P0 takeover (`tradingview-lab-clean-55`, 13:48Z → ~05:30Z 09-16) — route assessments (appended 20:2xZ; the morning close-out may add a line)

- **Gemini 3.8 flash-high (paid CLI, read-only route, `Invoke-GeminiProReadOnly.ps1` `eff6a727`):** two regimes in one night. 15:05Z-17:25Z: **8 of 8 full calls ended `UNAVAILABLE (code 503)` AFTER a complete report** (P027 x3, DD06 x2, O9FIX, intake x2; 649-979 s each; probes also failed intermittently) — every report was recovered from `%TEMP%\gemini_wrapper_failures\`, native-read-audited (0 reads outside any packet) and used as SUPPLEMENTAL; findings were real and applied (DD06 slice 2/3, intake slices 2/3). 19:45Z onward: **6 of 6 clean** (P021 S1 297 s, DD06 214 s, O9FIX 212 s, intake 139-221 s, P027 165 s) — all counted, all PASS-WITH-NITS, native-read audits 0 outside, and the reports were concordant with the voided ones. Grade for the night: **content A, transport C** (the voided reports cost ~2 h of recovery routine but no wrong finding). Two packet-craft facts: the reviewer's native `view_file` refuses text files with non-ASCII bytes (a `…` in a lane log made one required file unread → sanitise `LEAD_*.txt` to ASCII at staging); a lane `SHA256SUMS.txt` must hash blob bytes, not the CRLF checkout (Gemini flagged the divergence, direction reversed).
- **Claude Opus 5 as builder (owner "all" + evening asks):** OPS-A CI PR #194, O9FIX `153edee9`, intake adapter `65c4bc40`/`4c802e9b`, DD-06 probe `1af85067`, P0-21 S1 catalogue slice `701c5ddd`/`7fecf204`, README `e114ed31` — each with RED arms, Ruff, guard, disclosed authorship and a Gemini detection read; none accepted by the Lead. One real defect escaped the fixtures and both Gemini reads: the probe's control price (`round(mid*0.9, 1)` → six significant figures) was rejected by the venue on the first testnet call — fixtures never carried a real-shaped BTC mid. Lesson filed; the fix took 10 minutes, the lesson is permanent.
- **Hyperliquid testnet as an evidence route (KVM2 P4-03, SSH `baris`, `sudo -n`, root-sourced env, `runuser -u mtc-bridge`):** works exactly as the 09-14 smoke; one run under the owner's word cost 6 s of venue time and returned a precise venue rejection text; public Info API reads (`openOrders`, `extraAgents`, `spotClearinghouseState`) confirm state without a key. Unified accounts report perp `accountValue` 0.0 with the USDC in spot — read both.
- **Codex (all homes) / SuperGrok:** untouched tonight (weekly caps: Codex to Sep 19, Grok to Sep 18). Friday Sol lanes are GENERATED from the Wednesday briefs (`SOL_QUEUE_20260919/make_sol_lane.ps1`) so Wednesday's verdicts re-pin them.
- **Claude exact Opus (Pro):** not used; Wednesday queue (lanes 1-5) prepared and self-tested (launcher refuses before the 20:00Z reset).
