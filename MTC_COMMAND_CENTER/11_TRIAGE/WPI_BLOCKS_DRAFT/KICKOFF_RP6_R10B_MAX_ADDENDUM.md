# ADDENDUM — RP6 round 10b, Claude Max dispatch 2026-08-11 (supersedes the GLM addendum;
# GLM's 5h window proved closed until 18:50)

Read and execute `KICKOFF_RP6_REPAIR_R10.md` (same directory) with these binding additions
from the Lead. You are `claude-opus-5` xhigh via the Max account, IMPLEMENTER. Codex
remains auditor of record.
...
6. **Scope fence:** touch ONLY `RP6-P0.sh`, `SELF_QA_RP6.md`, the new report file, and (if
   a finding requires) the preregistration draft with every edit listed. A parallel Max
   session owns `pathscope_prover.py` in `WPI_PREREG_DRAFT_ROUND1/` — do not touch that
   file or directory except the prereg draft edits just described. Files in this directory
   contain uncommitted work from prior agents; do NOT run git checkout, git reset, or git
   stash on any tracked file.
