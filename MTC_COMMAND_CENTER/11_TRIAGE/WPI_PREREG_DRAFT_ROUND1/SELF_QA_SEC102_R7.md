# SELF-QA — SEC102 composite path proof, round 7

Implementer: `claude-opus-5` xhigh (Max). Input commit `90868b86`, whose Codex T1 audit
(`SEC102_CODEX_T1_AUDIT_R6_2026-08-12.md`) returned **REQUEST_CHANGES** on one finding and
explicitly ACCEPTED the interpreter-vocabulary residual and the conservative false stops as
scoped limits. Round 7 repairs that one finding. **No audit or acceptance is claimed here.**
...
## 1. The finding, the repair, and where the evidence lives
...
| What round 7 does NOT close | 11 |
...
# 6. No carried tracked artifact was edited by this round.
carried = ["pathscope_prover.py", "sec102_r1_fixtures", "sec102_r2_fixtures",
           "sec102_r3_fixtures", "sec102_r4_fixtures", "sec102_r5_fixtures",
           "sec102_r6_fixtures"]
for name in carried:
...
Command rc: `0`. Round 7 ran under **Python 3.14.2**; nothing here establishes behaviour under
any other interpreter version (`STATUS_SEC102.md` item 15).
...
The three evidence documents (`SELF_QA_SEC102_R7.md`, `STATUS_SEC102.md` and
`SEC102_R7_REPORT_2026-08-11.md`) are re-derived by appending them to the `NAMES` list in the
...
## 11. Honest residual scope — what round 7 does NOT close

The full carried list is `STATUS_SEC102.md` items 1-31, updated for round 7. The round-7-specific
statements:
...
| Pattern | Round-7 self-assessment |
|---|---|
...
| 4 — External interpreter/environment boundary | Unchanged disclosed production blocker (residual 1). |
| 5 — Grammar incompleteness | **The round-6 finding.** Addressed by inverting the test so incompleteness produces a STOP rather than an admission; swept over every printable character in section 7. The residual is now the safe set's *justification*, not its *coverage*. |
| 6 — Probe status before adjudication | Unchanged. |
| 7 — Incomplete-reader path | Unchanged. |
| 8 — Deployed identity | Unchanged lexical scope. |
| 9 — Claim wording vs predicate | The closure claim is narrowed to exactly the whitelist property in `STATUS_SEC102.md`; the `?(` fixture is declared a carried control rather than counted as a new RED; the kickoff's illustrative safe set is measured, not assumed. |
| 10 — Declared vs executed evidence | Every block here was executed and its real transcript published; section 13 re-runs all of them from outside the repository. |
| 11 — Instrument defects | The sweep asserts that the safe set it measures against equals the one the module implements. |
| 12 — Unmodeled behavior disappearing | The classifier's default is refusal; `SWEEP_LEAK_ADMITTED=0` and `SILENT_LEAK=0` are the measurements. |
| 13 — Terminal-disposition conservation | Unchanged; every RED reaches a terminal STOP with a reason token, verified in sections 3-5. |
...
repository, executed from a working directory outside the repository, and its output compared
with the ```` ```text ```` block published under it. The comparison is a subset check, so a block
whose published transcript is a declared excerpt (section 6) passes on the lines it does publish.

The harness is written outside the repository and is reproduced here in full so the Lead can
re-run it without trusting this document. The fence marker is built with `chr(96)` so this source
contains no literal triple backtick and is safe to publish inside the document it parses.

```python
#!/usr/bin/env python3
"""Extract every fenced powershell block from SELF_QA_SEC102_R7.md, run it from a working
...
for index, (command, published) in enumerate(blocks, 1):
    ...
    print(f"BLOCK={index:02d} PUBLISHED_LINES={len(want)} MISSING_FROM_REAL_OUTPUT={len(missing)} "
          f"{'OK' if not missing else 'MISMATCH'} {label}")
    for line in missing[:5]:
        print(f"    MISSING {line}")
print(f"BLOCKS={len(blocks)} COMPARED={compared} MISMATCHED={mismatched} CWD={OUTSIDE}")
sys.exit(0 if mismatched == 0 else 1)
```

```text
BLOCK=01 PUBLISHED_LINES=51 MISSING_FROM_REAL_OUTPUT=0 OK $tool=(Resolve-Path 'MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROU
BLOCK=02 PUBLISHED_LINES=59 MISSING_FROM_REAL_OUTPUT=0 OK $tool='MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROUND1\composite_
BLOCK=03 PUBLISHED_LINES=26 MISSING_FROM_REAL_OUTPUT=0 OK $tool=(Resolve-Path 'MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROU
BLOCK=04 PUBLISHED_LINES=42 MISSING_FROM_REAL_OUTPUT=0 OK $tool=(Resolve-Path 'MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROU
BLOCK=05 PUBLISHED_LINES=9 MISSING_FROM_REAL_OUTPUT=0 OK $tool=(Resolve-Path 'MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROU
BLOCK=06 PUBLISHED_LINES=10 MISSING_FROM_REAL_OUTPUT=0 OK $tool=(Resolve-Path 'MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROU
BLOCK=07 PUBLISHED_LINES=30 MISSING_FROM_REAL_OUTPUT=0 OK $tool=(Resolve-Path 'MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROU
BLOCK=08 PUBLISHED_LINES=7 MISSING_FROM_REAL_OUTPUT=0 OK $tool=(Resolve-Path 'MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROU
BLOCK=09 PUBLISHED_LINES=16 MISSING_FROM_REAL_OUTPUT=0 OK $base='MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROUND1'
BLOCK=10 PUBLISHED_LINES=21 MISSING_FROM_REAL_OUTPUT=0 OK $base='MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROUND1'
BLOCKS=10 COMPARED=10 MISMATCHED=0 CWD=C:\Users\BARSEM~1\AppData\Local\Temp\claude\C--LAB-Tradingview-LAB-CLEAN\5f80961c-a2cc-40a8-9e11-02d3d7613305\scratchpad\outside
```
