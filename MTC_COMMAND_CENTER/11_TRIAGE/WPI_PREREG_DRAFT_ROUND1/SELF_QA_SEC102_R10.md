# SELF-QA — SEC102 composite path proof, round 10

Implementer: `claude-opus-5` xhigh (Max). Input commit `ba929abc`, whose Codex T1 audit
(`SEC102_CODEX_T1_AUDIT_R9_2026-08-12.md`) returned **REQUEST_CHANGES** on one finding, **CLOSED**
the round-8 newline finding after independently reproducing every number in it, and judged the
round-9 disclosed residual honest. **The round-10 finding is in the EVIDENCE HARNESS, not in the
module** — for the third round running. Round 10 repairs that one finding and changes no code.
...
about to hand the interpreter so that no other process can modify, replace, rename or delete it,
proves byte identity **through that pin**, and only then runs the block from a working directory
OUTSIDE the repository — so the document cannot quietly drift from the tool, the tool cannot
quietly drift from the document, and neither can drift in the interval between the two.

**Sections 1-12 are the round-7/8/9 record, carried forward.** Every one of their `powershell`
blocks is byte-identical to `SELF_QA_SEC102_R9.md` — including round-8 and round-9 wording inside
the section-9 comments, because a re-typed carried block is not a carried block, and because not
re-typing an instrument is the discipline round 8 introduced and rounds 9 and 10 extend. Every
carried block was re-executed for round 10 by section 13, and its real byte identity, process
status and stderr are published there.

## 0. What round 10 changes

### The round-9 finding — the check was bound to a NAME, and the interpreter resolved that name again

`SEC102_CODEX_T1_AUDIT_R9_2026-08-12.md`, MEDIUM, Design Defect Pattern 11 with a Pattern 9
overlay. The round-9 wrapper created a named temporary file, wrote the fence bytes, **closed every
handle**, re-opened the *name* to read the bytes back, compared them with the fence, and then
launched `powershell.exe -File <pathname>`. PowerShell resolves that pathname a **second** time,
after the equality decision has already been made and published. Between the two resolutions the
object is unowned: a concurrent process running as the same principal can modify it in place or
replace the directory entry entirely, and the child then executes bytes the parent never compared
— while `SCRIPT_BYTES_IDENTICAL=1`, the published SHA-256 and the published `LF`/`CRLF` counts all
describe the *earlier* bytes. If that child exits 0 with empty stderr and prints the published
lines, round 9 reports the block reproduced.

The finding is not the newline translation of round 8 and not the status ordering of round 7. It
is **temporal rebinding**: a proof about a *name* presented as a proof about an *object*. Round 9's
own sentences — "there is no path through the wrapper on which unproven bytes are executed", and
"the instrument this document declares is the instrument the interpreter is handed" — are exactly
the sentences a name-bound check cannot support.

This is the third consecutive round in which the finding is in the harness rather than in
`composite_pathproof.py`, and the three are the same shape rotated: round 7's harness never
established that the child *completed*; round 8's never established that the child was handed
*this document's bytes*; round 9's never established that the bytes it proved were *still there
when the interpreter opened them*. So round 10 closes the **class** — every window between the
verification and the interpreter's consumption — rather than the instance.

### The repair — pin the object first, verify second, and never let go until the child is dead

Round 10 takes route (A) of the kickoff, object pinning. The interval is not shortened; it is
removed, and then its removal is measured:

1. **The object is pinned BEFORE it is verified.** `CreateFileW` opens the temporary script with
   share mode `FILE_SHARE_READ` **only** — read sharing granted so the interpreter can open it,
   **write and delete sharing withheld**. While that handle is held, no process running as any
   principal can modify, replace, rename or delete the object. The order matters and is the whole
   argument: anything that happened to the file *before* the pin is caught by the verification
   that happens *after* it, and nothing can happen *after* the pin at all.
2. **The bytes are read back THROUGH the held handle**, never by re-opening the name. Re-opening
   the name to read it back is precisely the round-9 defect.
3. **The exclusion is measured, not asserted.** The wrapper asks Windows for the write handle and
   for the delete handle that the pin is supposed to forbid, and requires
   `ERROR_SHARING_VIOLATION` (32) on both. "I passed the right share mode" is the same class of
   claim as round 8's "I called the API that does not translate", and it is refused for the same
   reason.
4. **The name is bound to the object, not trusted.** The pathname the child will be given is
   resolved once more and its volume serial number and NTFS file index must equal the pinned
   object's. A name that has been re-pointed at a different object cannot reproduce that triple.
5. **Every component of the name is pinned too.** A pinned file is not enough on its own, because
   a name is resolved component by component: the directory holding the script, and every ancestor
   up to the volume root, is opened and held with the same share mode, so none of them can be
   renamed or deleted either. If any component cannot be pinned, **no child is launched at all**.
6. **The handle is held across the child's entire lifetime**, and after the child exits the
   name-to-object binding and the pinned object's bytes are **both re-measured**. What the lock
   prevents is therefore also detected; what a lock cannot reach is detected too.

Round 9's three gates — process status, adjudicated stderr, published-subset comparison — are
conserved **verbatim** behind the new binding, and section 13b measures that conservation with six
cases rather than asserting it.

### It is measured, not asserted — and in both directions

Section 13b writes synthetic documents outside the repository and runs the **published round-9
wrapper** and the **published round-10 wrapper** over them, each extracted byte-for-byte from the
document that publishes it. The window is made **deterministic** rather than raced: one textual
substitution — the same one, applied to both published wrappers — inserts a genuinely separate
same-principal process at the exact launch boundary, after the equality decision and before the
interpreter opens the name. That process is a harmless **rewriter**: it tries to put a different
benign marker line into the temporary script, first by writing in place and then by replacing the
directory entry, and it prints what Windows told it.

* `rebind_certified` — the document publishes fence bytes that print one marker and a transcript
  containing the **other** marker, the one only the rewrite can produce. **Round 9 ACCEPTS it**,
  with `SCRIPT_BYTES_IDENTICAL=1` on the same page, having certified a transcript its compared
  bytes cannot produce. Round 10 REJECTS it. *This is Codex's finding, at rc level.*
* `rebind_honest` — the same fence with the transcript its published bytes really produce.
  **Round 9 REJECTS it**: once the window is occupied it can no longer reproduce an honest
  document. Round 10 ACCEPTS it.
* Under round 10 the rewriter reports `INPLACE_WRITE=DENIED WINERROR=32` and
  `ENTRY_REPLACE=DENIED`: **the real denial from the operating system**, on both vectors, not an
  argument that a denial would occur.
* Six carried round-9 children conserve every round-9 gate: the well-behaved child is accepted by
  both wrappers, and the nonzero-exit, stderr-writing, missing-line and line-ending-sensitive
  children are adjudicated identically by both.

Three mutants keep the new gates from being branches nothing ever takes. `M2` opens the one
constant that denies writers and is caught **before the child exists**, because the exclusion is
measured rather than trusted. `M3` opens that constant *and* drops the pre-launch requirement —
the only way to reach the post-run re-measurement — and the rewrite then succeeds, the child
really does execute bytes the parent never compared, and gate 4 refuses the block with its stdout
unread. `M1` is round 9's mutant carried: restore the round-8 translating write path and the
pre-launch byte gate still fires. A fourth measurement takes the pin's precondition directly on
the primitive: while another process holds a writable handle on the name, **the pin cannot be
taken at all** (`WINERROR=32`), and a block whose object cannot be pinned is refused.

### What round 10 does NOT change

`composite_pathproof.py` is **untouched** — same bytes, same SHA-256 as rounds 7, 8 and 9
...
limitation stays disclosed exactly as the owner ratified it, and section 11 states in one place
...
| Claim | Section |
|---|---|
| **Round 10: the object it runs is pinned before it is verified and held until the child is dead, so the bytes it proved are the bytes that ran** | 0, 13a |
| **Round 10: the rebinding at rc level in both directions — 2 rebinding children x 2 published wrappers under a deterministic hold, 6 carried children, 3 mutants, the pin precondition** | 13b |
| **Round 10: real byte identity, name-to-object binding before and after, process status and stderr for all eleven blocks, plus the outer wrapper** | 13c, 13d, 13e |
...
| What rounds 7, 8, 9 and 10 do NOT close | 11 |
| Thirteen-pattern self-adjudication | 12 |
| Paste-and-run verification of this document | 13 |

### What changed in the code in round 7 — and nothing in round 8, round 9 or round 10
...
of rounds 8, 9 and 10, measured:** the module this document adjudicates has no worktree modification, so every
classification transcript above is the round-7 code re-executed rather than a new build.
`.gitattributes` is clean for the same reason — round 10 adds no fixture, so there is nothing to
pin. Round 10 ran under **Python 3.14.2**; nothing here establishes behaviour under any other
...
The three evidence documents (`SELF_QA_SEC102_R10.md`, `STATUS_SEC102.md` and
`SEC102_R10_REPORT_2026-08-12.md`) are re-derived by appending them to the `NAMES` list in the
...
rounds 8, 9 and 10 changed no code and added no fixture. Section 9 measures the same fact from the
...
## 11. Honest residual scope — what rounds 7, 8, 9 and 10 do NOT close

The full carried list is `STATUS_SEC102.md` items 1-49, of which 45-49 are the round-10 additions
and 41-44 are round 9's, all carried. Item 43 now names the class round 10 closed — temporal
rebinding — which is the naming Codex r9 found missing.

### Round-10 statements — the executed object
...
1. **The pin binds every FILE and DIRECTORY on the path; it does not bind the VOLUME.** Windows
   ...
   level below that — a per-session DOS-device or mount redefinition that makes the drive letter
   itself point somewhere else — is *not* prevented by any share mode. It is **detected**: after
   the child exits, the name is resolved again and its volume serial number and file index must
   still equal the pinned object's, and a block that fails this is terminal with its stdout never
   interpreted (`SCRIPT_REBOUND_UNDER_PIN`). Prevention where the operating system gives a lock,
   detection where it does not, and a terminal reject either way — but the two are not the same
   strength and are not presented as if they were.
2. **The pinned object is the SCRIPT. The interpreter binary is not pinned.**
   `powershell.exe` is resolved by name from `PATH` by `subprocess.run`, and round 10 makes no
   claim whatsoever about the identity of the executable that opens the script. Every round of
   this document has been about the *instrument this document publishes*; the toolchain that runs
   it is outside that boundary and is named here rather than left to be discovered.
3. **A concurrent writer cannot get past the pin, but it can stop the run.** If another process
   already holds a writable handle on the temporary name, the pin cannot be taken at all — the
   `CreateFileW` returns `ERROR_SHARING_VIOLATION` and the block is refused before the child
   exists (`PIN_HELD=0`, `SCRIPT_NOT_PINNED`). Section 13b measures exactly this on the primitive.
   The failure direction is closed: such a run refuses loudly instead of executing unproved bytes.
   It is still a denial-of-availability surface for a hostile local principal, and that is a
   different property from the integrity one this round closes.
4. **The binding is a Windows property.** `FILE_SHARE_READ` semantics are enforced by the Windows
   I/O manager and the filesystem. Nothing here establishes equivalent binding on another
   operating system, and nothing here establishes it for a temporary directory that is not a local
   NTFS volume. This document's evidence is what this host did, on this volume.
5. **The exclusion is measured for the two access classes that can rewrite the object** — a
   write-open and a delete-open — not for every access mask Windows defines. Those two are the
   ones that carry modify, truncate, rename and delete; the measurement is a demonstration that
   the lock is real, not an enumeration of the access-mask space.
6. **`HOLD`, `M1`, `M2` and `M3` in section 13b are mutations of the published harness, not
   independent implementations.** They show that the window existed, that closing it is what
   removes the false acceptance, that the pin is load-bearing, and that the post-run gate fires —
   but a mutation cannot show that no other window exists anywhere in the wrapper. (These are
   *harness* mutants; the module mutants `M1`-`M7` of section 5 are a separate, unchanged set.)
7. **The rewriter is synthetic and harmless.** It writes one benign marker line and prints what
   Windows told it. It is a test of the harness, not an attack fixture, and no sensitive body was
   authored for it.
8. **Round 10 measures no new property of `composite_pathproof.py`.** Every classification claim in
   this document is the round-7 claim re-executed. Round 10 guarantees only that the block being
   re-executed is the block on this page, that it is still that block when the interpreter opens
   it, and that a block which fails cannot be reported as reproduced.
9. **Round 9's disclosed residual 1 is carried unchanged** (`STATUS_SEC102.md` item 41): byte
   identity is asserted against this document as it exists on disk, not against a pinned checkout.
   The pin binds the *temporary script* to the *fence bytes on this disk*; it says nothing about
   whether a fresh clone would materialise those same fence bytes. That failure remains loud — a
   CRLF-materialised checkout changes the published counts and digests — and it remains the Lead's
...
| Pattern | Round-10 self-assessment |
|---|---|
...
| 3 — Host-object, symlink, mount identity | Unchanged explicit non-claim **in the module**. In the harness, round 10 now makes a bounded object-identity claim about one file: the temporary script is pinned and its volume serial number plus file index are compared with the object the name resolves to. That is a claim about the harness's own temporary file, not about any analysed path, and the mount-level residual is stated as round-10 residual 1. |
| 4 — External interpreter/environment boundary | Unchanged disclosed production blocker (residual 1 of the round-7 list). Round 10 proves that the interpreter is handed this page's bytes and that they are still this page's bytes while it runs; it claims nothing about how the interpreter decodes them, and nothing about the identity of the interpreter binary (round-10 residual 2). |
| 5 — Grammar incompleteness | The round-6 finding, closed in round 7 by inverting the test so incompleteness produces a STOP rather than an admission; swept over every printable character in section 7. Codex r7 judged the whitelist a fixpoint for the class. Unchanged by rounds 8, 9 and 10. |
| 6 — Probe status before adjudication | The round-7 finding, closed in round 8 and confirmed closed by Codex r8. Rounds 9 and 10 conserve that ordering verbatim; round 10 **measures the conservation** with six carried cases and adds one more gate that runs before stdout is read — the post-run re-measurement of the executed object. |
| 7 — Incomplete-reader path | Unchanged in the module. In the harness there are now three ways a run is refused before its output is believed: it never started (bytes, pin or binding), it did not complete (status, stderr), or the object it ran changed underneath it (post-run re-measurement). |
| 8 — Deployed identity | Unchanged lexical scope. |
| 9 — Claim wording vs predicate | **THE ROUND-9 FINDING.** Round 9's "no path on which unproven bytes are executed" and "the instrument the interpreter is handed" were name-bound checks written as object-bound claims. Section 13a now states the predicate the mechanism actually supports: pinned before verification, verified through the pin, name proved to reach the pinned object, re-proved after the child exits — with the volume-level residual named in the same breath rather than after an audit. |
| 10 — Declared vs executed evidence | The round-8 finding, closed in round 9 and confirmed closed by Codex r9 after independent reproduction. Round 10 conserves it: extraction and writing are still byte-mode, section 13b still reproduces the round-8 write path on this document's own first fence, and `M1` still fires. |
| 11 — Instrument defects | **THE ROUND-9 FINDING, and the round-10 repair.** The verified named file was not atomically the instrument the production caller later opened. Round 10 removes the interval instead of narrowing it: the object is pinned before it is verified, read back through the pin, proved to be what the name reaches, and held until the child is dead. The D026 block still extracts every instrument **as bytes** and pins the digest of each mutant it runs. |
| 12 — Unmodeled behavior disappearing | The classifier's default is refusal; `SWEEP_LEAK_ADMITTED=0` and `SILENT_LEAK=0` are the measurements. The harness's default is refusal too. None of the new gates is an unexercised branch: `M1` fires the byte gate, `M2` fires the exclusion measurement before the child exists, and `M3` — two declared substitutions, because that is what it takes to get there — fires the post-run gate with a real rewrite behind it. |
| 13 — Terminal-disposition conservation | Unchanged; every RED reaches a terminal STOP with a reason token, verified in sections 3-5. The harness's own rejections are terminal too — `PATH_PIN_INCOMPLETE`, `SCRIPT_NOT_PINNED`, `SCRIPT_BYTES_MISMATCH`, `SCRIPT_NOT_BOUND_TO_PINNED_OBJECT`, `SCRIPT_REBOUND_UNDER_PIN`, `SCRIPT_BYTES_CHANGED_UNDER_PIN`, `STATUS_REJECT_NONZERO_EXIT`, `STATUS_REJECT_UNADJUDICATED_STDERR`, `NO_PUBLISHED_TRANSCRIPT` and `MISMATCH` each carry a named reason token and force a non-zero wrapper exit. |
...
repository, **pinned so that no other process can touch it**, and executed from a working
directory outside the repository. **Round 10 makes the middle clause of that sentence true.** The
round-9 wrapper wrote the temporary script, closed it, re-opened the *name* to read it back,
compared, and then handed the same *name* to `powershell.exe` — which resolved it again. Round 10
opens the object once, verifies through that handle, proves the name reaches that object, and
holds the handle until the child is dead.

### 13a. What the round-10 wrapper adjudicates, in order

| # | Test | If it fails |
|---|---|---|
| 0a | Every component of the script's path — the directory holding it and every ancestor up to the volume root — is open and held with write and delete sharing denied. | `PATH_PIN_INCOMPLETE` — **no child is launched at all**, for any block. |
...
| 4 | After the child exits — with the pin still held — the name still resolves to the pinned object, and the pinned object's bytes are still the fence's bytes. | `SCRIPT_REBOUND_UNDER_PIN` / `SCRIPT_BYTES_CHANGED_UNDER_PIN` — **stdout is never read.** |
| 3 | Every published line appears in the real stdout (subset check). | `MISMATCH`, with the first five missing lines printed. |

Tests 0a-0c and 4 are round 10's repair; tests 1-3 are round 8's and round 9's, conserved
verbatim, and section 13b measures that conservation rather than asserting it. The numbering is
kept from round 9 so the carried gates keep their names; the table is in execution order.

**What that order supports, stated as the predicate the mechanism actually proves.** Every block
whose output this wrapper believes was executed from an object that was pinned before it was
verified, proved byte-identical to the fence *through the pin*, proved to be the object its
pathname reaches, and re-proved on both counts after the child exited — with the pin held
continuously across the child's whole lifetime, and with the operating system's refusal to grant a
write handle or a delete handle *measured* rather than assumed. Under Windows sharing semantics no
process, at any privilege, can modify, replace, rename or delete that object or any directory on
its path while those handles are held, so on this platform there is **no path through the wrapper
on which unproven bytes are executed**. What that argument does *not* cover is a rebinding one
level below the filesystem — a redefinition of the volume or drive letter itself, which no share
mode can prevent. That case is not prevented; it is **detected** by the post-run re-resolution,
and it makes the block terminal with its stdout never interpreted. Round-10 residual 1 says so in
the same words.

A block failing 0a, 0b or 0c reaches `continue` before `subprocess.run` exists. A block failing 1,
2 or 4 reaches `continue` before the comparison exists. The wrapper exits `0` only if no block was
rejected for any of these reasons, and it prints each block's byte count, `LF`/`CRLF`/non-ASCII
composition, SHA-256, pin state, name-binding state, `RC=` and `STDERR_BYTES=` for **every** block
whether or not that block passed, so the identity, the binding and the real status of all eleven
children are on the page.
...
### 13b. D026 — the rebinding at rc level, both directions, plus every new gate firing

Eight synthetic documents are written outside the repository, each publishing one harmless child
block and the transcript that child is supposed to produce. **No wrapper is re-typed:** each is
extracted from the `python` fence of the document that publishes it — round 8 and round 9 from
their frozen files, round 10 from this one — and extracted **as bytes**, because reading the
instrument through translating I/O would be the round-8 defect one level up. The SHA-256 of the
exact bytes executed is printed for every instrument and every mutant, so what ran is what is on
the page (Pattern 11).

**The window is made deterministic, not raced.** `HOLD` is one textual substitution, the *same*
one applied to both published wrappers, which inserts a genuinely separate same-principal process
at the exact launch boundary: after the parent has decided the bytes are identical, before the
interpreter opens the name. It changes no gate, no comparison and no output; it only occupies the
window. If the window does not exist, the inserted process can achieve nothing — which is exactly
what round 10 has to show.

That process is the **rewriter**: it opens the temporary script by name and tries to put a
different harmless marker line there, first by writing in place, then by replacing the directory
entry. It prints what Windows told it and always exits 0, so the wrapper under test is never
disturbed by the rewriter's own status.
...
`rebind_certified` accepted under round 9 *is* Codex's finding at rc level: the wrapper reports
`SCRIPT_BYTES_IDENTICAL=1`, publishes a SHA-256 of bytes it compared, and then certifies a
transcript those bytes cannot produce — because it did not run them. `rebind_honest` is the same
defect from the other side: once the window is occupied, round 9 can no longer reproduce a
document that is honest about its own published bytes. Under round 10 the rewriter is refused on
both vectors and reports the refusal in the operating system's own numbers.

Six carried round-9 children conserve every round-9 gate under round 10 — the well-behaved child,
the nonzero-exit child, the stderr-writing child, the missing-line child, and the two line-ending
sentinels that decided round 9. Three harness mutants keep the new gates honest: `M2` opens the
one constant that denies writers and is caught before the child exists, because round 10 measures
the exclusion instead of trusting its own share mode; `M3` opens that constant *and* drops the
pre-launch requirement — two substitutions, both declared and counted, because that is the only
way to reach the post-run gate — and the rewrite then succeeds, the child really does execute
bytes the parent never compared, and the block is refused with its stdout unread; `M1` is round
9's mutant carried, restoring the round-8 translating write path so the pre-launch byte gate is
still shown firing. `D026_PIN_PRECONDITION` measures the pin's precondition directly on the
primitive: while another process holds a writable handle on the name, the pin cannot be taken at
all.
...
$base='MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROUND1'
$d026=@'
import ctypes, hashlib, os, pathlib, re, subprocess, sys, tempfile
...
if R9_WRAPPER is None or R10_WRAPPER is None or R8_WRAPPER is None:
    print("D026_ABORT=instrument_not_uniquely_published")
    sys.exit(1)

# HOLD is the round-10 finding made deterministic.  ONE substitution, applied to the PUBLISHED
# bytes of BOTH wrappers, inserts a genuinely separate same-principal process at the exact launch
# boundary - after the parent has decided the bytes are identical, before the interpreter opens
# the name.  It changes no gate and no comparison; it only occupies the window.  If the window
# does not exist the rewriter cannot do anything, which is precisely what round 10 must show.
...
R9_HOLD = R9_WRAPPER.replace(ANCHOR, INJECT)
R10_HOLD = R10_WRAPPER.replace(ANCHOR, INJECT)
for tag, source, mutant in (("R9_HOLD", R9_WRAPPER, R9_HOLD), ("R10_HOLD", R10_WRAPPER, R10_HOLD)):
    print(f"D026_MUTANT={tag:18s} SUBSTITUTIONS={source.count(ANCHOR)} "
          f"SHA256={hashlib.sha256(mutant).hexdigest()}")
    off += source.count(ANCHOR) != 1

# M2 flips the ONE constant that denies writers, on top of the same hold.  It exists so the pin
# is shown to be load-bearing rather than decorative.  Round 10 does not TRUST the share mode, it
# MEASURES it, so M2 is caught before the child exists: the write-open Windows was supposed to
# refuse succeeds, and the block is refused instead.
PIN = b"PIN_SHARE = FILE_SHARE_READ"
UNPINNED = b"PIN_SHARE = FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE"
M2 = R10_HOLD.replace(PIN, UNPINNED)
print(f"D026_MUTANT={'M2_pin_share_open':18s} SUBSTITUTIONS={R10_HOLD.count(PIN)} "
      f"SHA256={hashlib.sha256(M2).hexdigest()}")
off += R10_HOLD.count(PIN) != 1

# M3 is the only mutant here that needs TWO substitutions, and both are declared and counted: it
# opens the share mode AND drops the pre-launch exclusion requirement, which is the only way to
# reach the POST-RUN re-measurement.  Without it, gate 4 would be a branch nothing ever takes -
# exactly the Pattern-12 defect this project keeps refusing to publish.
GATE = b"    if not (excluded and name_bound):"
GATE_OFF = b"    if not (name_bound):"
M3 = M2.replace(GATE, GATE_OFF)
print(f"D026_MUTANT={'M3_post_run_reach':18s} SUBSTITUTIONS={R10_HOLD.count(PIN)}+"
      f"{M2.count(GATE)} SHA256={hashlib.sha256(M3).hexdigest()}")
off += M2.count(GATE) != 1

# M1 is round 9's mutant, carried: it restores the round-8 translating write path inside the
# round-10 instrument by one textual substitution, so the pre-launch byte gate is still shown
# firing rather than published as a branch nothing ever takes.
REPAIR = b"    path.write_bytes(command)"
ROUND8 = b'    path.write_text(command.decode("utf-8"), encoding="utf-8")'
M1 = R10_WRAPPER.replace(REPAIR, ROUND8)
print(f"D026_MUTANT={'M1_round8_write':18s} SUBSTITUTIONS={R10_WRAPPER.count(REPAIR)} "
      f"SHA256={hashlib.sha256(M1).hexdigest()}")
off += R10_WRAPPER.count(REPAIR) != 1

# The rewriter.  A separate process, same principal, no privilege, no attack fixture: it opens
# the temporary script by name and tries to put different HARMLESS bytes there - first in place,
# then by replacing the directory entry.  It prints what Windows told it and always exits 0, so
# the wrapper under test is never disturbed by the rewriter's own status.
...
# Round 9's six children, carried verbatim so every round-9 gate is re-measured under round 10.
...
# (case, child body, published transcript, want r9 verdict, want r10 verdict, want r10 reason)
CARRIED = [
...
# The two round-10 children.  ONE body, published byte-for-byte in both documents; only the
# published TRANSCRIPT differs.  `rebind_certified` publishes the line only the REWRITE can
# produce, so a wrapper that accepts it has certified bytes it never compared.  `rebind_honest`
# publishes the line the COMPARED bytes really produce, so a wrapper that rejects it cannot
# reproduce an honest document once the window is occupied.
...
REBIND = [
    ("rebind_certified", COMPARED_BODY, REBOUND_LINE,  "ACCEPTED", "REJECTED", "MISMATCH"),
    ("rebind_honest",    COMPARED_BODY, COMPARED_LINE, "REJECTED", "ACCEPTED", "OK"),
]
REASONS = ("SCRIPT_BYTES_MISMATCH", "SCRIPT_NOT_BOUND_TO_PINNED_OBJECT",
           "SCRIPT_REBOUND_UNDER_PIN", "SCRIPT_BYTES_CHANGED_UNDER_PIN",
...
conserved = false_accept = false_reject = denied = 0
with tempfile.TemporaryDirectory() as name:
    tmp = pathlib.Path(name)
    rebinder = tmp / "d026_rebinder.py"
    rebinder.write_bytes(REBINDER)
    env = dict(os.environ, D026_REBINDER=str(rebinder))
    tools = {}
    for tag, source in (("r9", R9_WRAPPER), ("r10", R10_WRAPPER), ("r9_hold", R9_HOLD),
                        ("r10_hold", R10_HOLD), ("m1", M1), ("m2", M2), ("m3", M3)):
    ...
    for case, body, want_line, want9, want10, want_reason in CARRIED:
        ...
        reason = reason_of(out10)
        unread = int("STDOUT_NOT_INTERPRETED" in out10)
        asserted = int("SCRIPT_BYTES_IDENTICAL=1" in out10)
        bound = int("NAME_BOUND_TO_PINNED_OBJECT=1" in out10)
        want_unread = int(want_reason.startswith("STATUS_REJECT"))
        ok = (v9, v10, reason, unread, asserted, bound) == (want9, want10, want_reason,
                                                            want_unread, 1, 1)
        off += not ok
        conserved += (want9 == want10) and ok
        print(f"D026 CARRIED={case:25s} R9={v9}/rc{rc9} R10={v10}/rc{rc10} R10_REASON={reason} "
              f"UNREAD_STDOUT={unread} BYTES_ASSERTED={asserted} NAME_BOUND={bound} "
              f"WANT={want9}/{want10}/{want_reason}/{want_unread} "
              f"{'OK' if ok else 'OFF_EXPECTATION'}")

    for case, body, want_line, want9, want10, want_reason in REBIND:
        doc = document(tmp / (case + ".md"), body, want_line)
        rc9, out9 = run("r9_hold", doc)
        rc10, out10 = run("r10_hold", doc)
        v9 = "ACCEPTED" if rc9 == 0 else "REJECTED"
        v10 = "ACCEPTED" if rc10 == 0 else "REJECTED"
        reason = reason_of(out10)
        # What the rewriter achieved against each wrapper, in the rewriter's own words.
        r9_effected = int("REBIND_EFFECTED=1" in out9)
        r10_effected = int("REBIND_EFFECTED=1" in out10)
        r10_sharing = int("INPLACE_WRITE=DENIED WINERROR=32" in out10
                          and "ENTRY_REPLACE=DENIED" in out10)
        # Round 9 asserted byte identity for bytes it did not execute: both claims in one run.
        r9_asserted = int("SCRIPT_BYTES_IDENTICAL=1" in out9)
        ok = ((v9, v10, reason) == (want9, want10, want_reason)
              and r9_effected == 1 and r10_effected == 0 and r10_sharing == 1
              and r9_asserted == 1)
        off += not ok
        false_accept += want9 == "ACCEPTED" and want10 == "REJECTED" and ok
        false_reject += want9 == "REJECTED" and want10 == "ACCEPTED" and ok
        denied += r10_effected == 0 and r10_sharing == 1
        print(f"D026 REBIND={case:25s} R9_HOLD={v9}/rc{rc9} R10_HOLD={v10}/rc{rc10} "
              f"R10_REASON={reason} REBOUND_UNDER_R9={r9_effected} "
              f"REBOUND_UNDER_R10={r10_effected} R10_DENIAL=SHARING_VIOLATION_32/{r10_sharing} "
              f"R9_ASSERTED_IDENTITY={r9_asserted} WANT={want9}/{want10}/{want_reason} "
              f"{'OK' if ok else 'OFF_EXPECTATION'}")

    # M2: the same hold, the same rewriter, ONE constant changed - write and delete sharing are
    # granted again.  Round 10 does not believe its own share mode: it asks Windows for the write
    # handle the pin is supposed to forbid, gets it, and refuses the block BEFORE the child
    # exists.  The rewriter is never even reached.
    ...
    # M3: share mode open AND the pre-launch exclusion requirement dropped - the only path that
    # reaches the post-run re-measurement.  The rewriter succeeds, the child really does execute
    # bytes the parent never compared, and gate 4 refuses the block with its stdout unread.  Gate
    # 4 is therefore exercised, not published as a branch nothing ever takes.
    ...
    # M1, carried from round 9: restore the round-8 write path and the pre-launch byte gate must
    # refuse BEFORE the child exists.
    ...
    # The pin's PRECONDITION, measured on the primitive itself.  A same-principal process that
    # already holds a writable handle on the temporary name is not a way past round 10: it makes
    # the pin IMPOSSIBLE TO TAKE, and a block whose object cannot be pinned is refused before the
    # child exists (PIN_HELD=0, SCRIPT_NOT_PINNED).  The failure direction is closed.
    ...
    # Round 9's write-path measurement on the real artifact, carried: this document's own first
    # powershell fence through both write paths.
    first = [m.group(2) for m in FENCE.finditer((BASE / "SELF_QA_SEC102_R10.md").read_bytes())
             if m.group(1) == b"powershell"][0]
    for tag in ("R8_TEXTMODE", "R10_BYTEMODE"):
        path = tmp / (tag + ".ps1")
        if tag == "R8_TEXTMODE":
...
print(f"D026_CARRIED={len(CARRIED)} CONSERVED_R9_VERDICTS={conserved} "
      f"D026_REBIND={len(REBIND)} FALSE_ACCEPT_UNDER_R9={false_accept} "
      f"FALSE_REJECT_UNDER_R9={false_reject} REBIND_DENIED_UNDER_R10={denied} "
      f"M1_GATE_FIRED={int(m1_ok)} M2_EXCLUSION_MEASURED={int(m2_ok)} "
      f"M3_POST_RUN_GATE_FIRED={int(m3_ok)} D026_OFF_EXPECTATION={off}")
sys.exit(0 if off == 0 else 1)
'@
$d026 | python -B - $base
...
D026_MUTANT=R9_HOLD            SUBSTITUTIONS=1 SHA256=c25f4f61d498ef97cb703862de4a55207ce295aafc187e0f9c2d98aa93f8e566
D026_MUTANT=R10_HOLD           SUBSTITUTIONS=1 SHA256=ab1e25db446c926c77a18ce31b087eafa020ab33d0edab32694481b9e029a592
D026_MUTANT=M2_pin_share_open  SUBSTITUTIONS=1 SHA256=8fa90ba532dc3ac7e6f4e115ff3d09d68d29e3ba1142f21dc8f701c137f531f8
D026_MUTANT=M3_post_run_reach  SUBSTITUTIONS=1+1 SHA256=97d82205f8296c1795bf6ba9206c1626d865d3be90e7fe97684cf60e35f0d770
D026_MUTANT=M1_round8_write    SUBSTITUTIONS=1 SHA256=4a4d3a7b6eaf8a5fe7a131f554933a20c1b751ff524bcdc6979939799f5d8062
D026 CARRIED=well_behaved_child        R9=ACCEPTED/rc0 R10=ACCEPTED/rc0 R10_REASON=OK UNREAD_STDOUT=0 BYTES_ASSERTED=1 NAME_BOUND=1 WANT=ACCEPTED/ACCEPTED/OK/0 OK
D026 CARRIED=fails_after_summary       R9=REJECTED/rc1 R10=REJECTED/rc1 R10_REASON=STATUS_REJECT_NONZERO_EXIT UNREAD_STDOUT=1 BYTES_ASSERTED=1 NAME_BOUND=1 WANT=REJECTED/REJECTED/STATUS_REJECT_NONZERO_EXIT/1 OK
D026 CARRIED=stderr_after_summary      R9=REJECTED/rc1 R10=REJECTED/rc1 R10_REASON=STATUS_REJECT_UNADJUDICATED_STDERR UNREAD_STDOUT=1 BYTES_ASSERTED=1 NAME_BOUND=1 WANT=REJECTED/REJECTED/STATUS_REJECT_UNADJUDICATED_STDERR/1 OK
D026 CARRIED=published_line_absent     R9=REJECTED/rc1 R10=REJECTED/rc1 R10_REASON=MISMATCH UNREAD_STDOUT=0 BYTES_ASSERTED=1 NAME_BOUND=1 WANT=REJECTED/REJECTED/MISMATCH/0 OK
D026 CARRIED=crlf_transcript_certified R9=REJECTED/rc1 R10=REJECTED/rc1 R10_REASON=MISMATCH UNREAD_STDOUT=0 BYTES_ASSERTED=1 NAME_BOUND=1 WANT=REJECTED/REJECTED/MISMATCH/0 OK
D026 CARRIED=lf_exact_bytes            R9=ACCEPTED/rc0 R10=ACCEPTED/rc0 R10_REASON=OK UNREAD_STDOUT=0 BYTES_ASSERTED=1 NAME_BOUND=1 WANT=ACCEPTED/ACCEPTED/OK/0 OK
...
Command rc: `0`. `FALSE_ACCEPT_UNDER_R9=1` is Codex's finding at rc level, and
`REBIND_DENIED_UNDER_R10=2` is its repair in the operating system's own words: on both vectors,
against both rebinding documents, Windows refused the rewriter. `CONSERVED_R9_VERDICTS=6` is round
9's repair surviving intact behind the new binding. `M1_GATE_FIRED=1`, `M2_EXCLUSION_MEASURED=1`
and `M3_POST_RUN_GATE_FIRED=1` are the three new gates shown firing on the published instrument
with the stated substitutions, not published as branches nothing ever takes.
`D026_PIN_PRECONDITION ... PIN_TAKEN=0 WINERROR=32` is the fail-closed direction: an object this
wrapper cannot pin is an object it will not launch a child over.

`D026_WRITEPATH` is round 9's measurement carried, reproduced on the real artifact: this
document's own first `powershell` fence holds 110 LF and zero CRLF, the round-8 write path puts
110 CRLF on disk, and the round-10 write path puts the fence's bytes on disk.

### 13c. The published round-10 wrapper
...
```python
#!/usr/bin/env python3
"""Extract every fenced powershell block from SELF_QA_SEC102_R10.md AS BYTES, write it to a
temporary .ps1 inside a directory tree this process PINS OPEN, prove the file the interpreter
will open is byte-identical to the fence WHILE HOLDING A HANDLE THAT DENIES EVERY WRITE, DELETE
AND RENAME TO EVERY OTHER PROCESS, run it from a working directory OUTSIDE the repository, prove
the child run completed over the pinned object, and only then compare its output with the
published transcript.

Round 10 repairs the defect `SEC102_CODEX_T1_AUDIT_R9_2026-08-12.md` found in the round-9
wrapper: round 9 wrote the temporary script, read it back BY NAME, compared, and then launched
`powershell.exe -File <pathname>` - which resolves that name a SECOND time, after the equality
decision.  Between the two resolutions a concurrent process running as the same principal could
replace or modify the object, so the child could execute bytes the parent never compared while
`SCRIPT_BYTES_IDENTICAL=1` and the published SHA-256 still described the earlier bytes.  The
defect is not the newline translation of round 8 and not the status ordering of round 7: it is
TEMPORAL REBINDING - a check bound to a NAME rather than to an OBJECT.

The repair binds the checked bytes to the executed bytes by removing the interval, not by
shrinking it:

  * every component of the script's path - the leaf file's directory and every ancestor up to
    the volume root - is opened and HELD with share mode FILE_SHARE_READ, so no process can
    rename, delete or replace any component of the name the child will resolve;
  * the script object itself is opened and HELD the same way BEFORE it is verified, so the
    verification happens under the lock rather than before it;
  * the bytes are read back THROUGH THE HELD HANDLE, never by re-opening the name;
  * the exclusion is MEASURED, not asserted: the wrapper itself tries to open the name for
    write and for delete and requires ERROR_SHARING_VIOLATION (32) from Windows on both;
  * the name the child is given is resolved once more and its object identity (volume serial
    number + file index) must equal the pinned object's, so the name is proved to reach the
    pinned bytes;
  * the handle stays open across the child's ENTIRE lifetime, and after the child exits the
    name-to-object binding and the pinned object's bytes are BOTH re-checked, so anything the
    lock cannot prevent is still detected before one byte of stdout is interpreted.

Seven tests now run in a fixed order.  Tests 1-3 are round 8's and round 9's, conserved
verbatim; 0a-0c and 4 are round 10's:

  0a. every path component is pinned with write and delete sharing denied
                                                    -> otherwise NO CHILD IS EVER LAUNCHED
  ...
  0c. the pathname the child will be given resolves to the pinned object
                                                    -> otherwise THE CHILD IS NEVER LAUNCHED
  ...
  4.  after the child exits, the name must still resolve to the pinned object and the pinned
      object's bytes must be unchanged               -> otherwise STDOUT IS NEVER READ
  3.  only then: every published line must appear in the real stdout (subset check)

The mechanism is `CreateFileW` with a share mode that denies writers - but the mechanism is not
the evidence.  The evidence is that Windows is ASKED to break the lock and refuses, that the
name is resolved to an object identity rather than trusted, and that both are re-measured after
the child exits, because "I passed the right share mode" is exactly the class of claim round 9
made about `write_bytes` and could not support.
...
# ---------------------------------------------------------------------------------------------
# Win32.  Nothing here is exotic: one API, one share mode, and the errors Windows returns.
# ---------------------------------------------------------------------------------------------
K32 = ctypes.WinDLL("kernel32", use_last_error=True)
...
# THE PIN.  Read sharing is granted so the interpreter can open the script; write and delete
# sharing are withheld, which is what makes a concurrent modify, replace, rename or delete
# impossible for as long as the handle is held.  One line, and it is load-bearing: section 13b
# flips exactly this constant and shows the rebinding come back.
...
def win_open(path, access, share, flags=FILE_ATTRIBUTE_NORMAL):
    """Open an existing object.  Returns (handle, 0) or (None, win32 error code)."""
    ctypes.set_last_error(0)
...
def object_identity(handle):
    """The object's true identity: volume serial number + NTFS file index.  A name that has
    been rebound to a different object cannot reproduce this triple."""
...
def name_identity(path):
    """Resolve the pathname exactly as the child will and report which object it reaches.
    Read access only, and read sharing granted, so this cannot itself disturb the pin."""
...
def exclusion_is_real(path):
    ...
    delete handle on the name, and require it to refuse both with ERROR_SHARING_VIOLATION.  A
    wrapper that only claims to have passed the right share mode is making the round-9 mistake
    one level up."""
...
def write_script(command: bytes, path: pathlib.Path) -> None:
    ...
    round 9's repair, carried; round 10 does not take its word for it either - the file is
    pinned first and read back THROUGH THE PIN afterwards."""
    path.write_bytes(command)


# ---------------------------------------------------------------------------------------------
# (0a) Pin the whole name.  A pinned file object is not enough on its own: a name is resolved
# component by component, so a directory anywhere above the leaf could be renamed away and a
# substitute put in its place.  Every component from the volume root down to the directory that
# holds the script is opened and held with the same share mode, which denies the rename and the
# delete of each one.  If any component cannot be pinned the run refuses to launch ANY child.
# ---------------------------------------------------------------------------------------------
WORK = pathlib.Path(tempfile.mkdtemp(prefix="sec102_r10_")).resolve()
path_pins = []
unpinned = []
for component in [WORK] + list(WORK.parents):
    handle, err = win_open(component, GENERIC_READ, PIN_SHARE, FILE_FLAG_BACKUP_SEMANTICS)
    if handle is None:
        unpinned.append((str(component), err))
    else:
        path_pins.append(handle)
components = len([WORK] + list(WORK.parents))
path_pin_complete = not unpinned
print(f"PATH_PIN_COMPONENTS={components} PATH_PIN_HELD={len(path_pins)} "
      f"PATH_PIN_COMPLETE={int(path_pin_complete)} PIN_SHARE_DENIES=WRITE|DELETE")
if not path_pin_complete:
    for component, err in unpinned:
        print(f"    PATH_PIN_FAILED WINERROR={err} {component}")
    print("PATH_PIN_INCOMPLETE CHILD_NOT_LAUNCHED")
...
for index, (command, published) in enumerate(blocks, 1):
    ...
    script = WORK / f"block_{index:02d}.ps1"
    write_script(command, script)

    # (0b) THE ROUND-10 REPAIR.  The object is pinned BEFORE it is verified, the bytes are read
    # back THROUGH the pin rather than by re-opening the name, and Windows is asked to break the
    # pin and refuses.  From here to the child's exit no other process can modify, replace,
    # rename or delete this object, so the bytes compared below are the bytes that run.
    pin, pin_err = win_open(script, GENERIC_READ, PIN_SHARE)
    if pin is None or not path_pin_complete:
        rejected += 1
        byte_rejects += 1
    ...
    print(f"BLOCK={index:02d} PIN_HELD=1 READ_THROUGH_PIN=1 "
          ...
          f"WINERROR={write_err}/{delete_err} NAME_BOUND_TO_PINNED_OBJECT={int(name_bound)}")
    if not same:
        K32.CloseHandle(pin)
        script.unlink(missing_ok=True)
        rejected += 1
        ...
        continue
    # (0c) the name the child is about to be given must reach the pinned object, and the object
    # must be provably closed to writers.  Either failure is terminal before the child exists.
    if not (excluded and name_bound):
        K32.CloseHandle(pin)
        script.unlink(missing_ok=True)
        rejected += 1
        binding_rejects += 1
    ...
    # (4) THE PIN IS STILL HELD.  Re-resolve the name and re-read the object through the pin:
    # what the lock prevents is now also measured, and anything a lock cannot reach - an
    # exotic rebinding of the volume itself - is detected here instead of being believed.
    ...
    if err and index not in STDERR_CONTRACT:
        ...
        continue
    # (4) the executed instrument, re-measured after the child exited.  A block whose object was
    # rebound or rewritten while it ran is terminal here, with its output never interpreted.
    if not after_bound:
        rejected += 1
        binding_rejects += 1
    ...
    # (3) byte identity, name binding and execution completeness are proved; only now may the
    # transcript be interpreted.
    if not published:
        rejected += 1
...
print(f"BLOCKS={len(blocks)} SCRIPT_BYTES_IDENTICAL_ALL={identical} PINNED_ALL={pinned} "
      f"NAME_BOUND_ALL={bound} POST_NAME_BOUND_ALL={post_bound} "
      f"POST_BYTES_UNCHANGED_ALL={post_same} REJECTED_ON_BYTES={byte_rejects} "
      f"REJECTED_ON_BINDING={binding_rejects} STATUS_PROVED_COMPLETE={complete} "
...
sys.exit(0 if rejected == 0 and path_pin_complete else 1)
```

### 13d. Real byte identity, binding and status for all eleven blocks

Write the section-13c fence to `verify_selfqa_r10.py` in a directory outside the repository and
run it as `python -B verify_selfqa_r10.py <path-to-this-file> <that-directory>`. Real output:

```text
PATH_PIN_COMPONENTS=7 PATH_PIN_HELD=7 PATH_PIN_COMPLETE=1 PIN_SHARE_DENIES=WRITE|DELETE
...
BLOCK=02 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=03 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=04 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=05 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=06 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=07 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=08 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=09 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=10 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
...
BLOCK=11 FENCE_BYTES=18571 SCRIPT_BYTES=18571 LF=339 CRLF=0 NONASCII=0 SCRIPT_BYTES_IDENTICAL=1 SHA256=2779497454b970302c6d3786e7f945376ec8ccd08ebcfa09fecfefd993cf796a
BLOCK=11 PIN_HELD=1 READ_THROUGH_PIN=1 WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32 NAME_BOUND_TO_PINNED_OBJECT=1
BLOCK=11 RC=0 STDERR_BYTES=0 POST_NAME_BOUND=1 POST_BYTES_UNCHANGED=1 STATUS_OK PUBLISHED_LINES=24 MISSING_FROM_REAL_OUTPUT=0 OK $base='MTC_COMMAND_CENTER\11_TRIAGE\WPI_PREREG_DRAFT_ROUND1'
BLOCKS=11 SCRIPT_BYTES_IDENTICAL_ALL=11 PINNED_ALL=11 NAME_BOUND_ALL=11 POST_NAME_BOUND_ALL=11 POST_BYTES_UNCHANGED_ALL=11 REJECTED_ON_BYTES=0 REJECTED_ON_BINDING=0 STATUS_PROVED_COMPLETE=11 REJECTED_ON_STATUS=0 REJECTED_ON_STDERR=0 COMPARED=11 MISMATCHED=0 REJECTED=0 CWD=C:\Users\BARSEM~1\AppData\Local\Temp\claude\C--LAB-Tradingview-LAB-CLEAN\75451042-6dac-4c8b-80ec-8b5d7066e97a\scratchpad\outside
```

Every one of the eleven children was launched over an object that was **pinned before it was
verified** (`PINNED_ALL=11`), proved byte-identical to its fence through that pin
(`SCRIPT_BYTES_IDENTICAL_ALL=11`, `REJECTED_ON_BYTES=0`), proved to be the object its pathname
reaches (`NAME_BOUND_ALL=11`), and still both of those things after it exited
(`POST_NAME_BOUND_ALL=11`, `POST_BYTES_UNCHANGED_ALL=11`, `REJECTED_ON_BINDING=0`). Each returned
process status `0` with **zero** stderr bytes, and only then had its output compared.
`WRITE_OPEN_DENIED=1 DELETE_OPEN_DENIED=1 WINERROR=32/32` on every block is Windows refusing, once
per block, the two opens that could have rewritten the script — the exclusion measured eleven
times rather than asserted once. `NONASCII=0` on every block, so nothing above turns on how the
interpreter decodes a non-ASCII byte; that boundary stays the disclosed Pattern-4 residual. The
`LF`/`CRLF` columns are the composition of this file as it exists on the disk this transcript was
produced on — a checkout that rewrote them would print different numbers and different digests
...
last prose edit, and is byte-identical to the run published there. What is *not* claimed: that the
eleven children could not have done something this document failed to ask about, that the bytes
proved here are the bytes a fresh clone would materialise (section 11, round-9 residual 1, carried
as round-10 statement 9), or that a rebinding of the volume beneath the pinned path is prevented
rather than detected (round-10 statement 1). Round 10 proves each child was handed this page's
bytes, that they were still this page's bytes for as long as the child lived, and that the child
ran to completion before its output was believed — nothing more, and the round-9 finding is that
"was still this page's bytes" had never previously been established at all.
