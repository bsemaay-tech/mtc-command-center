...
## Helper change record — OD-20260906-11 (extension 21:3x)
At 21:17 and 21:30 two lanes were discarded on `Changed:.git\objects\pack\pack-90d0a308….pack` (a repack/gc by another app touched a
pre-existing 121 MB pack). Pack-store files are Git objects storage; the SAME authorized rule (pre-existing at lane start AND byte-identical
afterwards) is now applied to `.git\objects\pack\pack-*.{pack,idx,rev,mtimes,promisor,bitmap}` by SHA-256 snapshot-and-compare. New pack
files (`Created`) and changed bytes still abort. Helper SHA-256 5ce687a1… → **97b7db5fc787f48b14b752feb554acbbef62dbb0ce471f7b4235eeae5ba89c35**
(`extend_gemini_helper_pack_freshen.py`; PARSE_OK; match semantics verified); launchers re-pinned. Recorded as an interpretation of
OD-11's wording, not a new authorization — the owner may narrow it.

## Helper change record — OD-20260906-11 (original)
