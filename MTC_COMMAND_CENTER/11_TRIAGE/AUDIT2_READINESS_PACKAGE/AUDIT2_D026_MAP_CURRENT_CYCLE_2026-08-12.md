...
| RP6-11 | R15 dynamically resolved inventory-mutation target | **CLOSED by round 17** *(row corrected 2026-08-12 ~18:05 — it still read `UNLOCATED — supplemental` / `OPEN` after the summary had been updated for r17; the inconsistency was caught by the independent count re-derivation, not by an auditor)* | `RP6_CODEX_T0_AUDIT_R15_2026-08-12.md` F3: an admitted variable-mutating builtin whose target is resolved dynamically and contains no literal protected inventory name | **RED:** `D026_RED_WEAKENED_R16 mutant=eval rc=0 summary=PASS` and `mutant=dot_source rc=0 summary=PASS` — a temporary r16 fence with **only** the indirect-execution refusal removed, labelled explicitly rather than implied as a live defect (the shipped r16 fence already refuses `eval`/`source`/`.` at `SELF_QA_RP6.md:16763-16765`) | **GREEN:** `D026_GREEN_R17 … refused rc=1` for both class members, with `dynamic_targets` converted from a hardcoded literal into a measured field (`r17_literal_zero_measurements=0`). Lead verbatim run: `R17_DYNAMIC_TARGETS_SUMMARY cases=15 pass=15 fail=0`, carried r16 grammar `50/50`, block identity unchanged `5132bacd…` | **FULLY CLOSED.** Two structurally different class members exercised, so this is a class closure rather than an `eval` patch. Historical note retained: the r16 structural PASS alone did not manufacture the missing falsification — round 17 supplied it. |
...
*(Counts corrected 2026-08-12 ~18:05 after an independent re-derivation — `AUDIT2_D026_MAP_COUNT_RECHECK_2026-08-12.md` — found one row misclassified: `RP6-11` was resolved by round 17 but its table row still read UNLOCATED/OPEN. Pre-correction figures shown struck through.)*

- Total closure/evidence rows mapped: **39** (unchanged).
- Fully closed with located RED + GREEN on the stated final bytes: ~~28~~ → **29**.
...
- Unlocated/supplemental rows: ~~11~~ → **10 evidence-quality flags** — exactly `TR-01`..`TR-09` plus `PS-09`. The former eleventh was `RP6-11`, which was double-counted (both OPEN and unlocated) and has now moved to fully closed, removing the overlap entirely. **There is no longer any double-count in these figures.**
...
- Freeze-relevant D026 result: ~~**one open current-audit finding** remains (`RP6-11`)~~ → **ZERO open current-audit findings.** `RP6-11` was closed by round 17 (see its row and the disposition note). Transport F1 is separately owner-ratified ACCEPT-WITH-DISCLOSURE and is not counted as open. *(Line corrected 2026-08-12 ~19:25 — the ~18:05 correction updated the summary and the RP6-11 row but MISSED this sentence, leaving the document self-contradictory for ninety minutes. Caught by an independent second re-derivation, not by an auditor. This is the third instance today of the same class: summary updated, a detail left behind.)*
