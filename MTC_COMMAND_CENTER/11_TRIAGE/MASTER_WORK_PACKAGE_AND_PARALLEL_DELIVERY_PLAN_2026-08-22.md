...
- **Inputs — Lane A (`INTERNAL_PAPER`):** the **`TESTNET_ELIGIBLE` admission decision**, which admits to `INTERNAL_PAPER` and `EXCHANGE_TESTNET` and to nothing else (WP-V2A-10, brief §11.5); the frozen **`deployment_identity_hash`** the soak will be evidenced against; **a pre-registered soak plan approved before the window opens**, naming the immutable start date, the strategy set, the reconciliation procedure and the stop conditions; the `MockBroker` fill source (brief §6.4); the three-way reconciliation tooling. **No credentials and no venue path of any kind.**
