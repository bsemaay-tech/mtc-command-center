...
| A25 | **Unattended overnight orchestrator ÖLÜP kalabilir.** 18:30 (2026-07-02) run'ı Stage A ortasında crash etti, restart yoktu → ~2h idle. In-process retry süreç ölünce yetmez. | (1) Per-stage RETRY loop (checkpoint-resume ucuz); (2) **external Windows Task Scheduler watchdog** (~15dk, ölmüşse relaunch — hem orchestrator hem AI session ölse de yaşar); (3) Startup reboot hook. `overnight_resilient_2026-07-02_2100.ps1` + `watchdog_resilient_2100.ps1` referans. | LESSONS_2026-07-02 A25 |
| A26 | **Watchdog CommandLine-match liveness FLAKY.** `Get-CimInstance CommandLine -match` transient null/miss verir → canlı orchestrator'ı ölü sanıp **ikinci instance** başlattı → aynı `--resume` checkpoint'e iki yazıcı = corruption riski. | Orchestrator start'ta `$PID`'i `orchestrator.lock`'a yazar + lock PID canlıysa çalışmayı reddeder (single-instance); watchdog yalnız lock PID ölüyse (`Get-Process -Id`) relaunch eder. PID-liveness güvenilir. | LESSONS_2026-07-02 A26 |
...
### CHANGELOG
- 2026-07-02 — İki gece: 18:30 run crash-restart'sız ÖLDÜ (~2h idle). 21:00 **dirençli** run (20 worker, per-stage retry + PID lockfile + external watchdog + reboot hook): STG001/STG002 confirmation → 8 varyant → v2 23 tamamlandı → CPCV/PBO = **11,781 hücre, robust_final 0**. Tüm executable evren (~51 archetype) non-robust; micro-price crypto compounding artifact (C8) leaderboard'u domine ediyor. **A25** (crash-restart+watchdog şart) + **A26** (PID lockfile, CommandLine-match flaky) eklendi. Yol: yeni strateji logic'i (varyant/grid değil). LESSONS_2026-07-02.
