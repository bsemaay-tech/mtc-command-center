...
> ## ▶ NEWEST CHECKPOINT — Claude Opus 5 post-Gate local run-kit design (documentation only) (2026-08-09)
>
> **PICK UP EXACTLY HERE:** (1) The five **COMMAND GAP** procedures now have an implementation-ready
> design contract — `11_TRIAGE/POST_GATE_WPL_WPI_RUN_KIT_DESIGN_2026-08-09.md`. It is a
> **specification, not an executable**; every command block in it is marked **NOT EXECUTED** and
> **no stage script exists or may be created** until the budget blocker clears *and* a named
> authority lift covers the stage. (2) Next safe local unit: resolve the two **UNRESOLVED-INPUTs**
> read-only — `EXPECT_HOSTNAME` (`GATEA-STAGING` is the Hyper-V **VM name**, not a verified guest OS
> hostname; read the cloud-init seed record) and `MANIFEST_SHA256` for candidate `2ce41e34…321b`
> (the `bfefea2f…ced02` in `WPI_READINESS_RECORD:9` belongs to the earlier `1adf9ae5…` candidate;
> read `install_manifest.json` `release_manifest_sha256` or the A-2 install log). **Do not invent
> either.** (3) Keep `GATEA-STAGING` **retained, active, credential-free DISARMED — do not discard
> it** (needed through WP-A). (4) **No server execution** until a human budget re-plan / ceiling
> extension resolves the budget-evidence blocker (exact 50 h balance NOT REPRODUCIBLE) **and** a
> named lift covers any WP-V/KVM2/credential/broker/ARM/order/TESTNET-mainnet/economic action.
>
> **Unit scope:** `claude-opus-5` effort `xhigh`, isolated local worktree, documentation HEAD
> `851d2aa5`, candidate `2ce41e34…321b` unchanged. Exactly five files written. **No** SSH, sudo,
> systemctl, reboot, service, package/install, pytest, broker/network/exchange, credential,
...
> boundary:** **B** post-start read-only evidence · **C1** post-SIGTERM no-dangling-state · **C2**
> post-reboot subcheck · **C3** WAL capture → verify → restore-into-temp · **C4** rollback
> stop+mask-only. Normative rules: one authority per artifact; no stage invokes another (C1 never
> restarts, C1 never calls C3, C4 never performs a recovery start); read-only stages accumulate and
> fail closed once while mutating stages abort at first mismatch; the quiesced window is declared in
> the authorising sentence, never discovered at runtime. Ordering that spends **one** stop and keeps
> all the evidence: **C1 → C3-b → C4**.
>
> **Three findings a future implementer must not lose:**
> 1. **Stage B asserts `systemctl is-enabled` = exactly `static`.** `verify.sh:212-215` accepts
>    `masked|disabled|static`, which post-start would silently accept a *masked* host — a different
>    state from the accepted one. `static` proves unmasked **and** un-enableable in one token; pair it
>    with a direct mask-symlink-absence check and an empty-`DropInPaths` check (a drop-in can override
>    `Restart=`/`ExecStart=`/`TimeoutStopSec=` while the unit hash still matches `538c1c60…279bd`).
> 2. **C1 hard ordering rule:** capture the sidecar snapshot (`bridge.db-shm` absent, `-wal` absent or
>    0 B, no open fd) **before any DB read, including a read-only one** — a read-only SQLite open can
>    materialise a zero-byte WAL/SHM (`wal_state_bundle.py:602-604`, `:491-500`). Reorder it and the
>    verifier fails on state it created itself.
> 3. **`wal_state_bundle.py` has no `restore` subcommand** (only `create`/`verify`, `:1062-1080`).
>    Restore-into-temp is composed: copy the bundle db → `create` against the copy → assert the
>    re-derived **`invariants_sha256`** matches. Assert invariants, **never** `bundle_db_sha256` — a
>    re-capture is a fresh online backup, so the file hash legitimately differs.
>
> **Stale node G4 refreshed (narrow authorisation).** `WP0_SCOPE_BASELINE_RECORD_2026-07-31.md` I-R2
> no longer cites `test_kill_restart_after_request_commit_keeps_killed_and_resumes_once` (re-verified
> absent this unit); the two surviving symbols got path:line (`tests/test_api.py:61`,
> `tests/test_window_state.py:82`). A dated correction note records the removal rather than rewriting
> history, and records that the same symbol still sits in §9.1 line 308 — **left in place on purpose**,
> outside this unit's named write authorisation. Evidence-map defect, **not** a product defect;
> classification unchanged; no replacement symbol invented.
>
> **Re-derived independently here:** `requirements.lock` SHA-256
> `40873556a7f4586d77f165b985863138c9fc95b095da64ac52456b8c49098ec3`, 56 `==` entries, 1345 hash
> lines — matches the Lead's gap-matrix value. Also confirmed for the design: `meta(key, value)` with
> `app_state` (`bridge/store/db.py:654-657, :4313-4315`; `bridge/app.py:109-110`), so the "not ARMED"
> predicate is a real query. Recorded discrepancy: canonical `mtc-bridge.env` / `bridge.err.log`
> (`common.sh:18`, templates) vs the inventory's `bridge.env` / `bridge.err` — **shorthand, not an
> observed host fault**; no host read occurred.
>
> **Not designed on purpose:** C5 runtime egress (needs credentials + broker/TESTNET network
> authority — notably **not** ARM, which stays forbidden); release-rebind rollback (unmet
...
>
> **Stop conditions:** executing any block from the design record; creating any stage script as an
> executable before both blockers clear; running `verify.sh` wholesale post-start; inventing
> `EXPECT_HOSTNAME`, `MANIFEST_SHA256`, a rollback target SHA, a replacement test symbol, or any hash
> for an artifact that does not exist; destructively testing or repairing
> `/var/lib/mtc-bridge/bridge.db`; discarding `GATEA-STAGING`; any observed drift from the recorded
> host invariant (second release, extra listener, non-loopback bind, ARM enabled, credentials present,
> MainPID change under `Restart=no`) — investigate read-only, report, take no corrective action.

> ## ▶ PREVIOUS CHECKPOINT — GLM-5.2 post-Gate preregistration & gap matrix (WP-L Phase 2 → WP-I → Audit 2 → WP-A) (read-only) (2026-08-09)
>
...
> authority lift covers any WP-V/KVM2/credential/broker/ARM/order/TESTNET-mainnet/economic action.
>
> **Conclusion:** the correct post-Gate sequence **`WP-L Phase 2 → WP-I staging verification → Audit 2 →
> WP-A`** is verified from source (roadmap §23a steps 3–5 + §"Audit 2" lines 863/972–973/1199;
> runbook `:137`). Its obligations, reusable evidence, and unresolved command gaps are explicitly
> mapped; it is **not execution-ready**. **Do not start WP-V; do not rerun Gate A.** Gate-A A-0..A-9
> PASS is staging acceptance only — reuse its immutable evidence where predicates overlap, but it is not
> WP-L/WP-I/WP-A completion. Full record:
> `11_TRIAGE/GATE_A_POST_GATE_PREREGISTRATION_GAP_MATRIX_2026-08-09.md`.
>
> **Matrix groups:** A reusable immutable Gate-A evidence (no host action) · B read-only post-start host
...
> state: plain reboot from the current unmasked state expects inactive+unmasked; inactive+masked
> requires a separate authorised pre-reboot mask step. Either path must prove no process/listener/order
> and DB not ARMED; do not yet call the missing `[Install]` a defect. (G2) full `verify.sh` is a
> masked/unstarted verifier and **intentionally fails post-start** —
> use bounded subchecks, do not prescribe it now. (G3) rollback stop+mask is feasible but release-rebind
> has an **unmet prerequisite** (only candidate `2ce41e34…321b` installed; old install already absent) —
> do not invent a target. (G5) A-5 proved SIGKILL/restart/integrity/DISARMED, **not** graceful SIGTERM or
> reboot; A-6 empty-broker startup ≠ queue/full-reconcile (schema-4 disables it). (G6) README "never
> executed" text is stale after Gate A (cite historically). (G7) exact 50 h balance NOT REPRODUCIBLE →
> all host execution blocked.
>
...
> a new named lift. The broad standing authorisation does **not** override the narrower budget/safety hold.
>
> **Routing:** Tier 4 protected Bridge safety/evidence preregistration; GLM-5.2 via Z.AI Coding Plan;
> owner exact-model request. Worker scope: GLM-5.2 edited only the four task-named files (this prepend
> plus `_AI_MEMORY/NEXT_STEPS.md`, `_AI_MEMORY/GLOBAL_HANDOFF.md`, and the new gap-matrix record) and ran
> no SSH/Gate-A-script/sudo/systemctl/reboot/test/package/Git/staging-mutation/credential-read/
> broker-network command. Note: this prepend shifts the line numbers of the older checkpoints below it
> further down; locate cited blocks by their stable header text, not raw line numbers.
>
...
> work against contingency vs outside-budget, **without inventing hours**; (2) build the post-Gate
> preregistration/gap matrix for **WP-L Phase 2 + WP-I staging verification + Audit 2 + WP-A** from
> existing records and the exact candidate/service state; (3) **no server execution** until that package
> proves command scope, evidence outputs, stop conditions, and budget/authority fit; (4) keep
> `GATEA-STAGING` **retained and credential-free DISARMED — do not discard it.** Continue independent safe
> units rather than asking routine questions.
>
> **This was read-only discovery — no staging command was run** (no SSH, Gate-A script, scan, sudo,
> service, package, Git, staging mutation, credential read, or broker/network command). Starting HEAD
> `51e666b0`; product candidate remains `2ce41e34bceb599d80af24c5c33d835820ec321b`.
>
> **Conclusion: WP-V is NOT next.** Gate A A-0..A-9 PASS is **staging acceptance only**. Plan §23a exact
> sequence after Gate A: 3 one named expendable Ubuntu staging action; 4 Audit 2 after WP-L Phase 2 + WP-I
> staging verification; 5 WP-A on the retained host; 6 discard host only after WP-A evidence; 7 freeze
> final exact SHA/artifact; 8 Audit 3 Gate-5 + Gate-6; 9 Gate B; 10 WP-V only after deployment approval;
> 11 Gate C. Runbook `GATE_A_PREREGISTRATION_AND_STAGING_RUNBOOK_2026-08-01.md:137`:
> `Gate A verification → WP-L Phase 2 → WP-I staging → Audit 2 → WP-A`, all DISARMED — and **Audit 2
> restores the flagship acceptance floor.** **No record proves WP-L Phase 2, WP-I staging verification,
> Audit 2, or WP-A completed after the final Gate-A pass.**
>
> **Owed evidence:** `WPL_PHASE1_VERIFICATION_RECORD_2026-08-01.md:17` (no Ubuntu execution in Phase 1;
> Ubuntu evidence owed in Phase 2 / WP-A); `WPI_READINESS_RECORD_2026-08-01.md:45-46` (Phase 2 had not
> occurred) and `:154-168` (retained-host evidence owed: exact 56-entry lock parity; masked/inactive
> install + DISARMED start; reboot DISARMED; systemd/SIGTERM; SQLite backup/restore; rollback; actual
> egress / no mainnet; WP-A restart/reconnect/stale-data invariants).
>
> **Host:** `GATEA-STAGING` is the named clean expendable host
> (`GATE_A_STAGING_HOST_PROVENANCE_2026-08-02.md:105-117`); current inventory proves it **still exists,
> active/running, credential-free DISARMED, only candidate `2ce41e34…321b` installed — not discarded.**
>
> **Authority (conservative):** `OWNER_AUTH_50H_EXECUTION_PROMPT_2026-07-31.md:20-56` authorizes WP-L,
> WP-I, WP-A, WP-R, WP-V, Ubuntu staging, the named expendable host, KVM2 deployment, and pre-grants
> WP-V / ARM / first-TESTNET approval — **subject to every objective prerequisite.** Narrower later
> constraints control this window: `CODEX_TAKEOVER_HANDOFF_2026-08-02.md:261-263` (no master, KVM2, WP-V,
> or deployment beyond `GATEA-STAGING`) and **this file's "Hard stop — unchanged, needs a new explicit
> instruction from Barış" block** (was `:1452-1454` at HEAD `51e666b0`; shifted down by this prepend) —
> WP-V/deployment/KVM2 and other protected/economic actions need a new explicit instruction. A generic
> continue instruction does not
> name or lift those stops. **Authorized now:** read-only/local preparation, evidence reconstruction,
> scoped docs, prerequisites planning. **Not authorized now:** WP-V, KVM2, master merge, credential load,
> broker/exchange access, ARM, orders, TESTNET/mainnet, economic action, old-payload deletion. **Do not
> infer WP-V authority from Gate-A PASS or from generic continue wording.**
>
> **Budget blocker:** this file's `## Budget` section (was `:1489-1492` at HEAD `51e666b0`; shifted down
> by this prepend) — ≈14–17 h remained before that session; WP-A+WP-R+WP-V alone
> total 17 h; Gate-A repairs/rebuild/audits were unbudgeted; re-plan before committing to the remainder.
> Exact current hour ledger **not reconstructed**; hard 50 h ceiling, no silent overrun. **This does not
> require idling** — the read-only unit above proceeds now.
>
> **Stop conditions:** any request to execute WP-V/KVM2/master/ARM/credentials/broker/orders/economic
> action without an explicit named lift; any required Phase 2 / WP-I / WP-A evidence that would need a
...
> `openai_token`, `anthropic_token`, `xai_token`, `telegram_bot_token`, `ethereum_private_key` —
> every line exactly `rc=1 matches=0`; `A9_any_hit=0`; one `A-9 PASS`; one `A9_TRAP_EXIT rc=0`; zero
> `A9_FAIL`, zero path blocks, zero grep-error blocks. No matched path, text, or value existed or
> was printed. Exact scan roots recorded: the release candidate root and `/etc/mtc-bridge`; venv and
> `/home/gatea` excluded. A-9 truthfully read bytes including the root-readable env file while
> `grep -l` emitted no matched content; no secret value entered Lead output. Independent postcheck
> rc0 artifact `C:\WPI_ARTIFACTS\postcheck_gatea_a9_d.out` (stderr empty): evidence hash/bytes, all
> nine exact rc1/matches0 lines, aggregate hit0/PASS/trap/no-fail/no-path/no-error, zero A9
...
> PASS.** A-5 used accepted run-kit E; A-6 through A-9 used accepted run-kit D; the candidate
> remained `2ce41e34…321b`. Current staging remains safe: active/static (Restart=no), PID189813,
> NRestarts0, loopback-only `127.0.0.1:8790`, exact credential-free DISARMED `state_version=1`, all
...
> task-named files) and ran no SSH, Gate-A script, scan, sudo, test, service, package, Git, staging,
> cleanup, or network/broker/exchange command. **Next, in default autonomous order:** (1) read-only
> post-Gate transition inventory — reconstruct the exact A-0..A-9 reports/hashes, identify the exact
> old masked installation targets versus the accepted current candidate, verify current
> systemd/release/symlink/package state without reading secrets, and write a cleanup/cutover scope
> checkpoint, with no deletion or mutation in that unit; (2) only after exact-target verification
> and a fresh `_AI_MEMORY` checkpoint, perform any already-authorized old-install cleanup using
> explicit paths and recoverable/safe ordering, preserving the accepted candidate and evidence, and
> record a blocker rather than guess where authorization scope is not explicit; (3) do not rerun
> Gate A, ARM, load credentials, connect broker/exchange, place orders, merge master, or begin
> TESTNET/mainnet/economic action merely because Gate A passed. Record:
> `11_TRIAGE/GATE_A_A9_PASS_FINAL_2026-08-09D.md`.
>
> ---
...
> remote/local SHA-256 identical
> `a7ef34a18145aee61196110dda6882c80992e189573003eb7fbf1119f829f0d7`, 1087 B; exactly one
> `A-8 PASS`, one `A8_TRAP_EXIT rc=0`, one `RESULT=PASS`, zero `A8_FAIL`/`RESULT=FAIL`. In-script:
> `ss_rc=0`, `listener_count` 1, `local_addresses` exactly `127.0.0.1:8790`,
> non-loopback/wildcard/VM-IP listener lists all empty, `A8_ufw_rc=0`; IP and UFW evidence captured
> in the log, raw payload deliberately not reproduced. Host: one run of the accepted packaged path
...
> and LF-only, with the fixed VM/candidate/timeout values and the same booleans; `A8_HOST_PASS` is a
> command stdout marker and is not stored in the evidence log. Independent postchecks, both rc0 with
> empty stderr: `C:\WPI_ARTIFACTS\postcheck_gatea_a8_remote_d.out` (remote evidence
> hash/bytes/markers and binding assertions, exact credential-free DISARMED API, production
> active/running PID189813, Restart=no, NRestarts0, one loopback listener) →
> `A8_REMOTE_POSTCHECK=PASS`; `C:\WPI_ARTIFACTS\postcheck_gatea_a8_host_d.out` (host evidence
> hash/bytes/no BOM/CR0, command stdout includes `A8_HOST_PASS`, independent `TcpClient` reprobe
> port22 True and port8790 False) → `A8_HOST_POSTCHECK=PASS`. Combined acceptance required remote
> `A-8 PASS` plus host rc0 with all required booleans and `A8_HOST_PASS`; both held, so A-8 PASS.
> Contract held: no `/api/arm`, env file not opened, no credential content, no
> broker/exchange/order/economic action, read-only networking and firewall evidence only. **Gate
> state A-0..A-8 PASS; A-9 NOT RUN. Next:** preflight the accepted D A-9 script for identity and
> syntax, absence of the remote A-9 log, safe service, exact scan roots and command permissions, and
> the output-redaction contract — A-9 truthfully reads bytes under the release directory and
> `/etc/mtc-bridge` including the environment file, but may emit only category counts and matching
> paths, never matched text or values; update `_AI_MEMORY` before execution; then execute A-9 exactly
> once, only after the preflight checkpoint, preserving and hashing evidence and inspecting only
> counts and paths, never matched content. A genuine A-9 hit or failure is BLOCK/FAIL and stops Gate
> A completion. Record: `11_TRIAGE/GATE_A_A8_PASS_2026-08-09D.md`.
>
> ---
...
> empty (script redirects to its no-clobber log). Accepted candidate `2ce41e34` unchanged. Evidence
> `/home/gatea/gatea-A7-20260808D.log` preserved to `C:\WPI_ARTIFACTS\gatea-A7-20260808D.log`;
> remote/local SHA-256 identical `09443b51…2bbf5`, 4269 B; one `A-7 PASS`, one `A7_TRAP_EXIT rc=0`,
> one `RESULT=PASS`, zero `A7_FAIL`/`RESULT=FAIL`. API: HTTP200, DISARMED,
> credential_free_disarmed, state_version1, reconcile_ready False (expected, not required true),
> reconcile_error None, all network/exchange/credential/ARM flags off. Production DB via
> preregistered read-only sudo: quick_check ok, app_state DISARMED, schema_version4, explicit
...
> begin/end; `A7_journal_credgrep=not performed (forbidden by contract)`, raw payload deliberately
> not printed into Lead output. Accepted postcheck rc0
...
> because it over-strictly required the current mutable `bridge.log` hash to equal A-7's
> point-in-time snapshot; independent GET status checks append benign lines (1554 → 1616 → 1678 B,
> current hash at accepted v2 `d6bb3a2a…5b13ab`, `bridge.err.log` unchanged at 597 B/same hash) — a
> verifier-design defect, not an A-7 failure. V2 validates the authoritative snapshot identity
> inside the immutable A-7 evidence plus current logs regular/non-empty, instead of demanding a live
...
> **Gate state A-0..A-7 PASS; A-8..A-9 NOT RUN (A-8 not executed). Next:** preflight both accepted D
> A-8 scripts (`gatea_A8.sh` remote and `gatea_A8_host.ps1` Windows host) — exact hashes/syntax,
> remote and host evidence paths absent, service safe, required host/SSH connectivity — without
> executing A-8; update `_AI_MEMORY`; then execute the preregistered A-8 remote+host sequence
> exactly once, preserve/hash both evidence logs, independently postcheck. Do not run A-9 on genuine
> A-8 FAIL. Record: `11_TRIAGE/GATE_A_A7_PASS_2026-08-09D.md`.
>
> ---
...
> remote/local SHA-256 identical `75ed4262…488c`, 2007 B; one `A-6 PASS`, one `A6_TRAP_EXIT rc=0`,
> four `RESULT=PASS`, zero `A6_FAIL`/`RESULT=FAIL`. Production unchanged before/after: active,
> MainPID189813, exact HTTP200 DISARMED, credential_free_disarmed, state_version1, all
> network/exchange/credential/ARM flags off. Isolated temp app: engine present,
...
> `RESULT=PASS`; temp DB quick_check ok, DISARMED, schema_version4. Scope is empty-broker
> startup/no raise/no hang/no leftover queue — not queue-drain-under-load, not full reconcile
> (schema4 baseline disables it). Temp `/home/gatea/gatea-A6-temp.FLfBfh` cleaned; zero
> `/home/gatea/gatea-A6-temp.*` leftovers. Accepted postcheck rc0
...
> credential-free DISARMED API. The first postcheck added one out-of-contract read-only open of
> `/var/lib/mtc-bridge/bridge.db` as unprivileged `gatea` after all prior assertions passed; SQLite
> returned `unable to open database file` — verifier permission defect, not an A-6 failure (A-6
> targets only its temp DB; production PID/API unchanged). Probe removed, v2 accepted; A-7
> preregisteredly uses sudo for the production persisted-state check. Hardening held: no `/api/arm`,
> env file not opened, six process env keys removed/discarded before bridge imports with no values
> printed/copied/persisted/retained, MockBroker `bars=[]` blocked credential resolver and
> broker/exchange network, notifier absent/disabled bound into PASS. **Gate state A-0..A-6 PASS;
> A-7..A-9 NOT RUN. Next:** verify A-7 kit identity/syntax, A-7 log absent, service safe and
> noninteractive preregistered sudo without printing protected content; checkpoint memory; then run
> `bash /home/gatea/gatea-run-kit-20260808D-2ce41e34/gatea_A7.sh` once, preserve/hash, postcheck.
> Do not run A-8 on genuine A-7 FAIL. Record: `11_TRIAGE/GATE_A_A6_PASS_2026-08-09D.md`.
>
> ---

> ## ▶ NEWEST CHECKPOINT — A-6 preflight PASS; execute A-6 D next (2026-08-09)
>
> Lead-performed read-only A-6 preflight at checkpoint `e48cba48`; accepted candidate `2ce41e34`
> unchanged. Remote D tar `/home/gatea/gatea-run-kit-20260808D-2ce41e34.tar` (SHA-256
> `e8a52e3c…e0d3`, 71680 B) and extracted kit verify: seven SHA256SUMS OK, A-6 syntax rc0, A-6 script
> SHA-256 `4bd3cbc3…6625` / 13863 B / CR0. A-6 log absent, no A-6 temp leftover. Production safe:
> active/running PID189813, Restart=no/NRestarts0, one `127.0.0.1:8790` listener, exact HTTP200
> credential-free DISARMED state_version1, all external/ARM flags off; systemctl resolves exactly
> `MTC_BRIDGE_START_MODE=credential_free_disarmed`. First verifier stopped on `kill -0` "Operation
> not permitted" (root PID; read-only verifier defect; no Gate-A script ran); replaced with
> `test -d /proc/189813`, rerun rc0 `A6_PREFLIGHT=PASS`. **Gate state A-0..A-5 PASS; A-6..A-9 NOT
> RUN (A-6 not executed). Next:** execute A-6 once with
> `bash /home/gatea/gatea-run-kit-20260808D-2ce41e34/gatea_A6.sh`, then preserve/hash
> `/home/gatea/gatea-A6-20260808D.log`, postcheck, memory before A-7. Record:
