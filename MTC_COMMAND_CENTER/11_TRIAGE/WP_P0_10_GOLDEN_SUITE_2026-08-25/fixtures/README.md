...
The declaration-inventory hash checks that this list is exactly the reviewed, pinned list: a declaration cannot be added, removed, swapped, renamed, or moved between assertions or companion scenarios without refusal. Input-path order inside one assertion is intentionally ignored. This is declaration integrity, not value integrity: the hash does not compare the values at those paths, identify which record changed, or see a configuration field that no assertion declares. A declared value can change without changing this hash, and an undeclared field is outside it.
...
It executes the clean corpus and isolated temporary modified copies covering the inherited coherence/hash checks, the audited B1/B2 probes, family 20's embedded mirror expected values, singular companion selectors, missing declared inputs, C32/C36/C37 master-gate deletion, OHLCV element deletion, OHLCV key deletion, bar-count deletion, indexed bar reordering, index scrambling, malformed bar members, selector re-homing, manifest-count corruption, the seven declaration-inventory variants, and the declaration-order control under normal and optimized Python.
