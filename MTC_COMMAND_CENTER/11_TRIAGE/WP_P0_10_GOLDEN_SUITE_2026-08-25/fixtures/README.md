...
The verifier contains no economic implementation. It validates the complete manifest and file set with explicit, optimization-safe checks; validates that each cited line range exists and has a structurally matching WP-P0-09 C/GF, cross-cutting-rule, or explicit-non-decision label; verifies the LF-normalized authority hash against a verifier-owned constant (`EXPECTED_AUTHORITY_TEXT_LF_SHA256`) and the two manifest-local hashes for each fixture; checks the stored output/state hashes; checks the full declaration inventory against the verifier-owned `DECLARATION_INVENTORY_SHA256`; classifies every declared companion `config.*` path into exactly one of PINNED / SIBLING_VARIANT / AUTHORITY_SILENT; and checks the named arithmetic and idempotence relationships for families 4, 5, 22, and 24.

Every expected assertion must declare a checked input source. Fixture-local assertions carry `input_paths` that resolve to non-empty values inside the same JSON fixture. Companion legacy assertions are accepted only when their required input paths resolve inside a literal top-level `companion_scenarios` run. C32, C36, and C37 companion assertions must explicitly declare the `config.tw_audit_semantics_mode` master gate and the scenario must hold the required `research` value. Declared OHLCV sequences are pinned to hardcoded bar counts and to the local `frozen_metadata.ohlcv_fields` declaration `open,high,low,close,volume`; every declared OHLCV bar must be an object with an `ohlcv` list, and indexed OHLCV sequences must keep `index == position`. Cross-row imports are not part of this corpus; any `cross_row*` assertion field is rejected. The verifier accounts for 241 assertion sources and 2,660 concrete declared input paths.

The declaration-inventory hash checks that this list is exactly the reviewed, pinned list: a declaration cannot be added, removed, swapped, renamed, or moved between assertions or companion scenarios without refusal. Input-path order inside one assertion is intentionally ignored. This is declaration integrity, not value integrity: the hash does not compare the values at those paths, identify which record changed, or see a configuration field that no assertion declares. A declared value can change without changing this hash, and an undeclared field is outside it. The inventory check runs before the per-family validation, so when a modified corpus breaks both, the reported reason is `declaration_inventory_hash_mismatch` and the more specific per-family reason is not reached. Nothing becomes acceptable, but a reader diagnosing a failure should re-run against the previous verifier, or temporarily disable the inventory check, to recover the specific reason.
...
Those two fixture hashes are self-consistency checks, not an external trust anchor: both stored hashes live in `manifest.json`. A coordinated declared-value edit plus recomputation of both hashes is therefore not detected unless one of the independent coherence checks rejects it. The declaration-inventory hash does not close that value gap; it closes only changes to which paths each assertion declares. At audit fixed point `c05e5968`, coherence validation covered 24 of 230 expected values and left 206 unchecked. This repair adds 11 coverage assertions without extending `validate_coherence`, so the current scope is 24 of 241 expected values; a self-consistent declared-value rehash outside those 24 values is not detected. Named follow-up `WP-P0-10-COHERENCE-217` covers the 217 current values outside coherence validation.
...
It executes the clean corpus and isolated temporary modified copies covering the inherited coherence/hash checks, the audited B1/B2 probes, family 20's embedded mirror expected values, singular companion selectors, missing declared inputs, C32/C36/C37 master-gate deletion, OHLCV element deletion, OHLCV key deletion, bar-count deletion, indexed bar reordering, index scrambling, malformed bar members, selector re-homing, manifest-count corruption (that case is also run under optimized Python, and is the only one that is), the seven declaration-inventory variants, and the declaration-order control.

`gen_declaration_inventory.py` is a developer tool, never called by the verifier. It prints a digest for whatever corpus it is pointed at, including a modified one, and verifies nothing. Re-pinning `DECLARATION_INVENTORY_SHA256` from its output is therefore a change to the verifier's own authority and requires the same review as changing the corpus itself.
