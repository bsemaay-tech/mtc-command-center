...
6. ~~**NEW — the rebuilt artifact dropped two accepted WP-I documents.**~~ **DECIDED BY BARIŞ
   ...
   never landed on `origin/master`. Neither path is referenced by Bridge source or by the Gate A
   runbook, so this is **documentation drift, not a functional blocker** — but it is an owner/Lead
   decision: accept the drift, or land the docs on the build line and rebuild (new SHA, this
   artifact's identity invalidated). Do not silently rebuild.
