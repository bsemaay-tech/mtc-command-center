...
| 7 | ~~`run_p0.sh` wires none of the five `P0_ATTESTED_*` inputs~~ → **the five `P0_ATTESTED_*` inputs are WIRED but UNFILLED** | open | **OPEN — and the blocker was mis-stated.** *Corrected 2026-08-12 ~20:45, Lead-verified against the bytes.* `run_p0.sh` **does** wire all five: defined at `:151-155`, exported at `:237-240`, and logged at `:241-243` with `origin=deploy_channel_grant_6_never_learned_from_this_login`. The wiring is complete; the **values** are `<PIN-AT-FREEZE>` markers. This is a **fill** problem, not a missing-code problem — the fix is supplying attested values at freeze time, not writing wiring. The old phrasing would have sent an implementer to write code that already exists |
