...
Stage 2 immutable preregistration did not complete. The exact-model counterpart timed out after 904 seconds with only three partial, unaccepted scripts present. The full failure inventory is in `02_PREREG/STAGE2_PREREG_FAILURE_RECORD.md`.

First-FAIL stopping applies. With no immutable preregistration, RP0 forbids transport, so the VM was not contacted and no test was run. The partial scripts are preserved byte-for-byte as evidence and are explicitly **not authorized to execute**.
...
Resume only after the exact counterpart can complete and self-QA a fresh immutable Stage 2 preregistration. Do not reuse or execute the three partial scripts; preserve them as the failed-attempt record. A resumed attempt needs a new preregistration checkpoint before any remote invocation.
