# SELF QA - B3-GAP-ENV Option 1 design repair, round 5 (DOCUMENTATION ONLY)

Subjects: `round5/RP1-B3.sh` (662 lines), `round5/RPD-VERIFY.sh` (775 lines).
...
## 0. What round 5 is, and what it is not

Round 5 changes exactly ONE file: this one. `RP1-B3.sh`, `RPD-VERIFY.sh` and
`DESIGN_NOTES.md` in `round5/` are byte-identical copies of the `round4/`
versions. Verified in this round, not asserted:

```
$ cd round5
$ cmp -s RP1-B3.sh      ../round4/RP1-B3.sh      && echo IDENTICAL_RP1
IDENTICAL_RP1
$ cmp -s RPD-VERIFY.sh  ../round4/RPD-VERIFY.sh  && echo IDENTICAL_RPD
IDENTICAL_RPD
$ cmp -s DESIGN_NOTES.md ../round4/DESIGN_NOTES.md && echo IDENTICAL_DN
IDENTICAL_DN
```

Round 5 exists to close the one REQUIRED survivor of `audit4/AUDIT4_REPORT.md`:
finding 2, a D026 evidence-recording defect. Round 4's QA supplied several
closure tests as a parameterized recipe plus a value table (`<FIX>`, `<CASE>`,
`<PRE>/<SRC>/<FILE>/<FN>/<FX>`), and one block (the item-1 cwd RED) depended on
an `arm.sh` that a later block in the same document had overwritten. A recipe
plus a table is reproducible; it is not the exact executable command D026
requires. This round replaces every such recipe with the literal command text and
makes every block self-contained. Those five tokens are named here, in prose,
because that is what was wrong; none of them appears in any command block in this
file, and neither does any other placeholder.

**Round 5 re-runs no arm and changes no recorded output.** Every transcript below
is the round-4 transcript, unedited. That is checkable rather than convenient:
the bytes each round-4 arm was extracted from are the bytes now in `round5/`, and
the three `cmp` results plus the two hashes above prove it. Where a command block
names `$R4`, that is the literal path the recorded run used. `$R5/RP1-B3.sh` and
`$R5/RPD-VERIFY.sh` are byte-identical to it, so substituting `$R5` for `$R4` in
any block below extracts the same bytes and reproduces the same output.

Round 5 also corrects one arithmetic slip found while sweeping the counts
(section 3, the 6.10 row) and narrows one over-broad limitation sentence
(audit-4 nit 2; section 3 item 5 and section 8 gap 10).
...
Run in THIS round, against the `round5/` copies:
...
The two hashes above are the hashes audit 4 recorded for round 4, and the hashes
of the bytes every arm below was extracted from.

The deliverable set is exactly four files. `find round5 -mindepth 1` was checked
...
`round4` -> `round5` is empty: the files are byte-identical (section 0).
...
Round 5 adds nothing to either column: it re-drove no arm.
...
and by construction could not have acted on one: it is closed to code.
...
2. Round 4's document, not its harness: several closure tests were written down
   ...
   finding 2. That is what this round repairs; no run was affected, only the
   record of it.

## 4. QA declarations, in full
...
R2="$B/round2"; R3="$B/round3"; R4="$B/round4"; R5="$B/round5"
...
`R5` is declared for completeness only. No recorded run used it; the runs used
`$R4`, whose two script files are byte-identical to `$R5`'s (section 0).

`<repo>` in the `B=` line is the fourth and last path normalization in this file,
alongside `$QA`, `$QAW` and `$QA_PY`: it stands for the absolute path of the
repository checkout root on the QA host, normalized for the same reason the audit
normalizes the scratch root - it is a machine-local path that says nothing about
the subjects. It appears in a declaration, never inside a command block; every
command block reaches the repository through `$R2`, `$R3` or `$R4`.

**How `RC=` is recorded.** In every transcript below, the lines above `RC=<n>`
are the command's combined stdout and stderr passed through `emit`, and `RC=<n>`
is that command's exit status. `RC=` is a recording notation - neither block ever
prints it. Where a command block ends in the helper `arm`, that helper performs
the capture and its definition is above. Where a block ends in a run line of its
own (an `env`/`PATH` prefix, or a `( cd ... )` subshell), the identical capture
was applied to the line as written:

```
out="$(<the run line> 2>&1)" || rc=$?; emit "$out"; printf 'RC=%s\n' "$rc"
```
...
case and its fixture is the absence itself.
...
The only things any block assumes are: the declarations of section 4
(`R2`, `R3`, `R4`, `QA`, `QAW`, `QA_PY`, `GOODREL`, `GOODMAN`), the helper
definitions of sections 3 and 4 (`exfn`, `ex1`, `exc`, the six preludes, `emit`,
`arm`, `pinstub`), and the stubs and fixtures built by the blocks in 4.1. All of
those are recorded verbatim above, and no arm rewrites any of them.
...
PATH="$QA/fix2/stub:$PATH" env 'STUB_SELF_U=user:[4026531837]' 'STUB_SELF_M=mnt:[4026531840]' \
  STUB_ROOTFS=2049:2 'RPD_EXPECT_NS_USER=user:[4026531837]' 'RPD_EXPECT_NS_MNT=mnt:[4026531840]' \
  RPD_EXPECT_ROOTFS_ID=2049:2 bash "$QA/arm.sh"
PATH="$QA/fix2/stub:$PATH" env 'STUB_SELF_U=user:[4026531837]' 'STUB_SELF_M=mnt:[4026999992]' \
  STUB_ROOTFS=2049:2 'RPD_EXPECT_NS_USER=user:[4026531837]' 'RPD_EXPECT_NS_MNT=mnt:[4026531840]' \
  RPD_EXPECT_ROOTFS_ID=2049:2 bash "$QA/arm.sh"
PATH="$QA/fix2/stub:$PATH" env 'STUB_SELF_U=user:[4026531837]' 'STUB_SELF_M=mnt:[4026531840]' \
...
( cd "$R4" && \
  ...
  bash -c "unset RPD_EXPECT_NS_USER; bash RPD-VERIFY.sh" )
( cd "$R4" && \
  RPD_CANDIDATE_SHA=$GOODREL RPD_RELEASE_MANIFEST_SHA256=$GOODMAN \
  RPD_EXPECT_NS_USER='user:[4026531837]' RPD_EXPECT_NS_MNT='mnt:[4026531840]' \
  RPD_EXPECT_ROOTFS_ID=2049:2 \
  bash -c "unset RPD_EXPECT_NS_MNT; bash RPD-VERIFY.sh" )
( cd "$R4" && \
  RPD_CANDIDATE_SHA=$GOODREL RPD_RELEASE_MANIFEST_SHA256=$GOODMAN \
  RPD_EXPECT_NS_USER='user:[4026531837]' RPD_EXPECT_NS_MNT='mnt:[4026531840]' \
  RPD_EXPECT_ROOTFS_ID=2049:2 \
  bash -c "unset RPD_EXPECT_ROOTFS_ID; bash RPD-VERIFY.sh" )
```
...
PATH="$QA/fix3/stub:$PATH" env STUB_OWN=0:999   bash "$QA/arm.sh"
```
...
```
( cd "$R4" && env -u B3_SVC_UID B3_SWEEP_BUDGET_S=120 B3_SVC_GID=1500 bash RP1-B3.sh )
( cd "$R4" && env B3_SWEEP_BUDGET_S=120 B3_SVC_UID=abc  B3_SVC_GID=1500 bash RP1-B3.sh )
( cd "$R4" && env B3_SWEEP_BUDGET_S=120 B3_SVC_UID=0    B3_SVC_GID=1500 bash RP1-B3.sh )
( cd "$R4" && env -u B3_SVC_GID B3_SWEEP_BUDGET_S=120 B3_SVC_UID=1500 bash RP1-B3.sh )
( cd "$R4" && env B3_SWEEP_BUDGET_S=120 B3_SVC_UID=1500 B3_SVC_GID=0    bash RP1-B3.sh )
```
...
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=twoline   bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=oneline2  bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=wrapper   bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=wrongpath bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=eacces    bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=statx     bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=enoent    bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=eio       bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=empty     bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=nonprint  bash "$QA/arm.sh"
PATH="$QA/fix3/stubdir:$PATH" STUB_CASE=ok        bash "$QA/arm.sh"
```
...
```
( cd "$R4" && env -u B3_SWEEP_BUDGET_S B3_SVC_UID=1500 B3_SVC_GID=1500 bash RP1-B3.sh )
( cd "$R4" && env B3_SWEEP_BUDGET_S=abc B3_SVC_UID=1500 B3_SVC_GID=1500 bash RP1-B3.sh )
( cd "$R4" && env B3_SWEEP_BUDGET_S=0   B3_SVC_UID=1500 B3_SVC_GID=1500 bash RP1-B3.sh )
( cd "$R4" && env B3_SWEEP_BUDGET_S=120 B3_SVC_UID=1500 B3_SVC_GID=1500 bash RP1-B3.sh )
```
...
```
( cd "$R4" && env RPD_CANDIDATE_SHA=$GOODREL RPD_RELEASE_MANIFEST_SHA256=$GOODMAN \
    RPD_EXPECT_NS_USER='user:[4026531837]' RPD_EXPECT_NS_MNT='mnt:[4026531840]' \
...
```
( cd "$R4" && env -u RPD_CANDIDATE_SHA -u RPD_RELEASE_MANIFEST_SHA256 \
...
6. **Nothing reads a credential.** Neither block references a token, key,
   ...
   `command -v python3`. Round 4 adds no command and round 5 changes no byte.
...
13. **Round 5 re-drove nothing.** Every transcript in this file is the round-4
    transcript. What makes that sound is stated in section 0 and checked there:
    the three non-QA files in `round5/` are byte-identical to `round4/`, verified
    by `cmp -s` and by the two recorded hashes, so the bytes these arms were
    extracted from are the delivered bytes. Round 5's own fresh evidence is the
    section 0 `cmp` output and the section 2 `bash -n` / encoding / hash /
    line-count block, both run in this round against `round5/`.
