...
#
# ROUND 4 (audit-3 finding 1) - THE READ-ERROR ARM, repaired identically in both
# blocks. Round 3 still classified an EMPTY nonzero `read` as ordinary EOF.
# Opening a directory for input SUCCEEDS on Linux and the FIRST `read` then
# reports `read error: Is a directory`, returns nonzero and populates nothing, so
# the round-3 loop broke at record zero and printed a false no-mount admission
# with `records=0`, rc 0 - the exact false pass final-list item 5 requires to
# STOP. A nonzero read that populated NOTHING and that consumed NO record before
# it is not an end of input: a mount table has at least one record, so zero
# records means the source was never read at all. That is
# `mount_table_read_error`, rc 3. An empty nonzero read AFTER at least one record
# is left exactly as it was - that is what a well-terminated table produces - and
# the populated unterminated-record arm below is untouched.
# HONEST LIMIT, stated rather than hidden (SELF_QA section 8): bash `read`
# returns 1 for BOTH end of input and a read(2) failure, and the diagnostic goes
# to this process's stderr, not to a status the shell can test. A read error
# raised MID-table, after at least one well-formed record, is therefore
        ...
        if [ "$rrc" -ne 0 ]; then
            if [ -z "$f1$f2$f3$f4$f5$f6$extra" ]; then
                if [ "$records" -eq 0 ]; then
                    exec 9<&-
                    rpd_stop "mount_table_read_error path=$MOUNTS records=$records read_rc=$rrc detail=nonzero_read_populated_no_field_and_consumed_no_record"
                fi
                break
            fi
            truncated=1
