...
**Version:** 2.1 — owner-ratified decisions, peer-reviewed, Gate-5 audited, **repeatedly repaired and since materially amended. `G1` architecture acceptance was SATISFIED on 2026-08-23 for the exact candidate commit `c81aacb83f89ebc2454ddc80b4fd619ae8fd57f5` (cycle C2-R1) and attaches to that commit only — §0.7 is that historical record. THIS document is the amended set that followed it, and its fresh `G1` acceptance has NOT been granted at the time this candidate is written — §0.8 governs current amendment status, and §0.7 plus §0.8 must be read together.**
**Date:** 2026-08-21 (v2.0) · repaired 2026-08-22 (v2.1, **five** repair rounds — the fifth is the round recorded in §0.6 / C-29; the header previously said four and undercounted it. The historical round records in §0.3, §0.4 and §0.5 are unchanged; §0.6 retains its original blocker statements, and its **R1**, **R3**, **R9** and **R16** *Repair*, *Where* and pointer cells were **extended append-only** by later correction passes). **Amended further on 2026-08-23 and 2026-08-24 by the nine owner-ratified wayfinder folds (including the final red-team fold) and the bounded three-round G1 audit cycle, with repair passes before rounds 2 and 3 — indexed in §0.8, which is the current amendment ledger. No amendment is an acceptance, and the version stays v2.1.**

> ## STATUS — read before using this document
> | Field | Value |
> |---|---|
> | **Document state** | **v2.1 REPAIRED** — the 2026-08-21 Gate-5 findings are repaired (Appendix E), a second owner-authorized T2 documentation round applied the repairs listed in §0.3, a third owner-authorized T2 documentation round applied the repairs listed in §0.4, a **fourth owner-authorized T2 documentation round** applied the repairs listed in **§0.5** (owner acknowledgement of D-01…D-12, the D-02 and D-07 clarifications, and removal of a reserved future safeguard identifier), and a **fifth owner-authorized documentation round** repaired the **seventeen blockers R1 … R17 raised by the G1 architecture acceptance audit round 1 (T0)**, listed in **§0.6**. **The version remains v2.1; no repair round is an acceptance.** **A subsequent status-only record, §0.7 (2026-08-23), carries the acceptance and acknowledgement position AS AT THAT DATE and changes no architecture text, count, package, gate or authorization.** **Since §0.7 the document set has been materially amended — the nine owner-ratified wayfinder folds (including the 2026-08-24 final red-team fold) and the bounded 2026-08-24 three-round G1 audit cycle, with repair passes before rounds 2 and 3 — and `§0.8` is the current amendment and provenance ledger for all of them. `§0.7` is historical to 2026-08-23; `§0.8` governs what the set is now.** |
> | **Current counts (§0.6 governs the totals; §0.7 is the 2026-08-23 status record; §0.8 governs current amendment status)** | **60 tracked requirements = 44 owner outcomes (O-01…O-44, unchanged) + 16 derived safeguards (D-01…D-16)**. **All 16 are now owner-acknowledged** — D-01…D-12 on **2026-08-22** (D-02 and D-07 with their recorded clarifications), **D-13…D-16 on 2026-08-23 AS WRITTEN**. **They remain DERIVED permanently, their safeguard text is unchanged, and they never become original owner outcomes.** **76 work-package definitions** *(69 before the 2026-08-23 wayfinder decision fold — C-31; map #37 added six, bringing the count to 75, and the map-#79 execution-architecture fold added the seventh, **WP-P0-31 Lifecycle Ledger and Registrar** — §10.4, §17.1 — bringing it to the canonical 76; maps #78 and #95 amend existing carriers only; **§0.8.1 indexes all nine folds and their package effect**)*. Counts quoted inside §0.3, §0.4 and §0.5 are historical to those rounds and are not the current totals, **and the "69 work packages" recorded in §0.7 is likewise historical — it is the count as it stood at the accepted candidate commit `c81aacb8…`, not a current total.** **The requirement total 60 = 44 + 16 is unchanged then and now.** |
> | **AUDIT ACCEPTANCE** | **HISTORICAL — SATISFIED for gate G1, at the exact candidate commit `c81aacb83f89ebc2454ddc80b4fd619ae8fd57f5` (parent / fixed point `e71248a2576132b3e3363df00b7f2ce5425902aa`), cycle C2-R1, 2026-08-23** — two fresh, independent, read-only flagship audits at `xhigh` (`claude-opus-5` **PASS**; Codex `gpt-5.6-sol` **PASS**), supplemental Standards and Spec axes **PASS**, **no unresolved required finding**, final audited worktree clean. **The acceptance attaches to that candidate commit only; a later status-record commit is not itself audited.** **THIS document is not that commit.** Since it, the set has been materially amended (§0.8), and **a fresh `G1` acceptance of the amended set has NOT been granted at the time this candidate is written: no accepting audit verdict is claimed here, and none may be inferred. A later status-only record may record an accepted exact SHA.** Acceptance remains governed by the **current root `AGENTS.md`**: the **tier-required independent audit(s)** at the **highest applicable tier at the actual gate**, with the auditor identity, effort and count that policy names **at the time of that gate**. This document does **not** decide a permanent tier, an auditor count or a model identity. The 2026-08-21 Codex Slot-B `REQUEST_CHANGES` on **v2.0** is **historical provenance only**. **G1 acceptance is not implementation authorization: `G1-IA` is unsatisfied, every work package — and every named milestone and stage within one — is NOT STARTED, and no deployment, host, credential, testnet or live act is authorized.** Appendix E §E.7 is the canonical record of that historical acceptance; **§0.8.3 states the current position, in which items 1 and 2 are deliberately kept apart** |
...
| **Q6** | **Account binding is DECIDED — subaccounts preferred: one subaccount plus its own dedicated agent wallet per risk bucket by default, per-strategy where capacity allows; virtual books inside one account are the specified fallback, on the recorded triggers only, with the fallback spec accepted before `WP-V2B-03` starts. `WP-P0-28` (VEN-A) verifies the remaining venue facts and does not reopen the decision** *(index aligned 2026-08-24, G1 final repair before audit round 3, with the canonical §21.1 Q6 and §10.2 — both of which already carry the owner's settled 2026-08-23 wayfinder decision, ticket #40. This index line still read "decided after WP-P0-28 exchange verification" and still asserted that "§21.1 Q6 reads 'decide after verification'", which had stopped being true when §21.1 Q6 was aligned: the same document then had Q6 both settled and pending. **§21.1 remains canonical, the owner's decision is unchanged and is neither re-made, widened nor narrowed here, and no venue fact is invented.** Carrier name previously repaired 2026-08-24, map #123 fold, from the retired "Package 7")* | §10.2, §17.2, **§21.1 Q6 (canonical)** |
...
| C-32 | **Post-acceptance amendment span (2026-08-23 → 2026-08-24), indexed rather than restated.** Everything that changed the document set **after** the accepted candidate `c81aacb8…`: the **nine owner-ratified wayfinder folds** — maps **#37, #54, #67, #78, #79, #95, #96, #97** and the **#118 / #123 / #124 final red-team fold** — which took the package count from the historical **69** to the canonical **76** (`WP-P0-26` … `WP-P0-31`, `WP-V4-09`), and the **bounded three-round G1 audit cycle of 2026-08-24, with repair passes before rounds 2 and 3** against the reproduced required findings of a fresh `T0` G1 audit of the amended set. **§0.8 is the ledger; each fold's own record is the authoritative account of it.** **The requirement total stays 60 = 44 OWNER + 16 DERIVED, all sixteen safeguards remain DERIVED, no O- or D- row was added, removed, reworded or renumbered, no gate or tier was created or renumbered, `OPEN-01` stays `[OPEN]`, and every unratified number stays `[OPEN]` as a draft proposal with its qualitative obligation binding. `G1-IA` remains unsatisfied; every package, milestone and stage remains NOT STARTED. A fresh `G1` acceptance of the amended set has NOT been granted at the time this candidate is written, and no accepting verdict is claimed.** **Extended 2026-08-24: the owner-authorized post-cycle closure repairs (`8882af26…`, `b2850ac4…`, `fa4c02e2…` — provenance wording, the stale `cancel_all()` clause, the declared `WP-V4-01` dependency edge and the F-14/F-15 pointer re-anchors) are indexed in §0.8.2 and changed no count, requirement, package, gate, tier or authorization.** | Wayfinder maps #37–#124, the 2026-08-24 G1 audit and its owner-authorized closure repairs, documentation-only rounds | **Amendments folded / errors corrected** |
...
| **R16** | The brief invented a `BrokerAdapter` protocol while `IBKR_PAPER_BRIDGE/bridge/broker/base.py` already defines `Broker`, `PartialRecoveryBroker` and `FullReconciliationBroker` (**including `funding_evidence`**), with `HyperliquidBroker` and `MockBroker` implementing them. *(Line pointers re-anchored 2026-08-24, G1 final repair before audit round 3. **As recorded at this round in 2026-08-22 they were `base.py:154` / `:230` / `:293` / `:320-323`, `hyperliquid.py:105`, `mock.py:68`** — accurate then, stale now. **At planning candidate baseline `01e0725890e456d079bca8967625ccb09c66b889` they are `base.py:156` / `:234` / `:347` / `:374-378`, `hyperliquid.py:115`, `mock.py:70`**; §2 F-9a and Appendix A carry the full re-anchored set. **The blocker itself, the symbols and the repair are unchanged, and these pointers locate repository source only — deployed runtime identity remains `UNVERIFIED` pending `G9`.**)* | The **existing boundary is recorded as a fact** and a **reuse / extend / deliberately replace-or-rename decision** is required **before** any new adapter protocol is named. Carrier **WP-P0-25** (decision only, no implementation). **Completed in the R1 correction pass: the `FullReconciliationBroker` method inventory was missing `funding_evidence`, and WP-P0-25 is reclassified T0 and mapped to G3** — protected/cross-cutting architecture takes the highest tier even when it edits no runtime code. **The existing structural `Protocol` seam remains the starting point, and no `BrokerAdapter` is invented** | **§2 F-9a (new)**, §4.1, §4.2, §13.3, §17.5; plan **WP-P0-25**, **WP-V5-01**; register **O-14**, **O-17** |
| **R17** | The authenticated kill/flatten path had **no redundancy requirement**. *(Statement corrected in the R1 correction pass: the prior plan did **not** implement or specify a solitary authenticator. §12.5 required WebAuthn/FIDO2 step-up and said nothing about credential count; it did **not** require redundant authenticators or an out-of-band recovery drill, and the Bridge source in this repository has **no WebAuthn path at all**; deployed runtime behaviour remains `UNVERIFIED` pending G9. The defect is the missing requirement, not a specified single key — and it is enforceable: an implementation satisfying §12.5 with one credential would be compliant and would be a single point of failure on the one control that stops the system.)* | **At least two independently registered authenticators** required, **plus documented and drilled out-of-band venue-side access and flatten recovery**. Native exchange stops remain **mitigation, not a substitute**. Carried by protected packages **WP-V2B-10** (semantics and runbook), **WP-V2B-06** (two authenticators, proven locally) and **WP-V2B-07** (the authorized testnet drill evidence), and made a **live-gate dependency**. New safeguard **D-16**. **Completed in the R1 correction pass:** the drill and the completed-authentication obligations are placed on the packages that can actually satisfy them, in the order **B01 → B10 → B06 → B05 → B07 → WP-V4-01**, so no package is required to prove something that does not exist yet | **§12.5**, **§12.5a**, §10.2, §19 **A-15b**, **A-21**; plan **WP-V2B-10**, **WP-V2B-06**, **WP-V2B-05**, **WP-V2B-07**, **WP-V4-01**, gates **G4**, **G5**; register **D-16** |
...
**Read as of the close of that round.** Every status sentence in §0.3, §0.4, §0.5 and §0.6 — including "acceptance is still PENDING", "not owner-acknowledged" and the totals — is **historical to the round it describes and was true at that round's close**. **The acceptance and acknowledgement position as at 2026-08-23 is §0.7; the current amendment and acceptance position, which §0.7 predates, is §0.8** *(pointer corrected 2026-08-24, G1 final repair before audit round 3: this read "the current … position is §0.7 alone", which stopped being true once the set was materially amended after that date)*.

## 0.7 Status and provenance record — 2026-08-23 (status-only, T3) — **historical to that date; §0.8 governs current status**

**Read §0.7 with §0.8** *(pointer added 2026-08-24, G1 final repair before audit round 3; this subsection's heading previously read "Current status", and its content still ends at 2026-08-23 while the set has since been materially amended)*. **Everything below is exactly as recorded on 2026-08-23 and is unchanged: it is the true status of the accepted candidate commit `c81aacb8…` and of nothing else.** **§0.8 is the current amendment and provenance ledger**, and it is where the nine wayfinder folds (including the 2026-08-24 final red-team fold), the bounded 2026-08-24 G1 audit cycle with repair passes before rounds 2 and 3, and the present acceptance position are recorded.
...
## 0.8 Current amendment and provenance ledger — 2026-08-23 / 2026-08-24

**Why this subsection exists.** §0.7 is a **status record dated 2026-08-23** and its last entry is that date. **Material amendments landed after it** — the nine owner-ratified wayfinder folds (including the 2026-08-24 final red-team fold), and the bounded 2026-08-24 G1 audit cycle with repair passes before rounds 2 and 3 — **and until this subsection existed no current ledger recorded them**, so a reader arriving at §0.7 could reasonably believe the document set had not moved since the accepted candidate. It has. **§0.8 is a provenance index only. It changes no normative architecture text, no requirement, no count, no work package, no dependency, no tier, no gate and no authorization**, and it **cross-references the standing fold records rather than restating them** — each record named below is the authoritative account of its own fold.

### 0.8.1 The nine wayfinder folds — owner-ratified, applied to existing carriers

All nine are owner-decision records and plan-amendment passes. **Together they added the seven work packages that take the total from the historical 69 to the current canonical 76, and changed no requirement: the total stays 60 = 44 OWNER + 16 DERIVED, all sixteen safeguards remain DERIVED, and no O- or D- row was added, removed, reworded or renumbered.** **Every one of them states, in its own record, that it authorizes nothing (D-12).**

| # | Map | Fold record (in `MTC_COMMAND_CENTER/11_TRIAGE/`) | Package effect |
|---|---|---|---|
| 1 | **#37** | `WAYFINDER_DECISION_FOLD_2026-08-23.md` | **+6** — `WP-P0-26` … `WP-P0-30`, `WP-V4-09`; thirteen existing packages amended in place |
| 2 | **#54** | `WAYFINDER_LIFECYCLE_FOLD_2026-08-23.md` | none — lifecycle, post-live tail, re-entry and succession doctrine onto existing carriers (§6.3, §6.9) |
| 3 | **#67** | `WAYFINDER_KERNEL_FOLD_2026-08-23.md` | none — kernel and economic-honesty governance (§9.7) |
| 4 | **#78** | `WAYFINDER_EXPLORER_FOLD_2026-08-23.md` | none — explorer scope, display doctrine and build order (§11.6); the trade-truth chart leaves Phase 0 for `WP-V3-11` |
| 5 | **#79** | `WAYFINDER_EXECUTION_CRITIQUE_FOLD_2026-08-23.md` | **+1** — `WP-P0-31` Lifecycle Ledger and Registrar (§10.4); execution-critique carriers assigned |
| 6 | **#95** | `WAYFINDER_OPERATOR_SURFACE_FOLD_2026-08-23.md` | none — operator display, control, execution-chart and mobile decisions (§12.7); includes the read-only-seam repair that splits `WP-V2B-05` and `WP-V2B-06` into named milestones |
| 7 | **#96** | `WAYFINDER_SAFETY_OPERATIONS_FOLD_2026-08-23.md` | none — emergency-control, incident/backup/recovery, credential/access and live-readiness doctrine (§12.6) |
| 8 | **#97** | `WAYFINDER_REPOSITORY_CONTEXT_DELIVERY_FOLD_2026-08-23.md` | none — repository topology, AI-context and versioning/migration/delivery doctrine (§14.6, §15.5) |
| 9 | **#118 / #123 / #124** | `WAYFINDER_FINAL_RED_TEAM_FOLD_2026-08-24.md` | none — the final red-team review, applied as coherence repairs and reuse-first amendments |

**Every unratified numeric value the folds touch stays `[OPEN]` as a draft proposal, with its qualitative obligation unchanged and binding.** **`OPEN-01`** — a zero-build vanilla-JS execution UI versus a framework/build step for optional table/query libraries — **is deliberately left `[OPEN]` and undecided.**

### 0.8.2 The 2026-08-24 amendments in this brief

**Three distinct 2026-08-24 workstreams have dated amendments in this document, and they are not the same thing.** **(a) The final red-team fold (map #123)** applied the owner-resolved coherence and reuse-first findings. **(b) The bounded G1 audit cycle**, run as **three audit rounds with repair passes before rounds 2 and 3** against reproduced required findings of a **fresh `T0` G1 audit** of the amended set. **(c) The owner-authorized post-cycle closure repairs** — recorded in the dated entry at the end of this subsection, each site carrying a note labelled *"owner-authorized closure repair"* (or, for §10.2a, the dated clause-correction note). Each amendment is marked at its own site with its date and its round, and the sites are, in document order: **§0.1** (the Q6 index), **§0.6 / §0.6.1** (round records extended append-only), **§2 F-9a, F-14, F-15 and F-16**, **§4.1**, **§6.3 and §6.5**, **§10.2 / §10.2a**, **§12.5 / §12.5a / §12.6 / §12.7**, **§14.3**, **§16 M9b**, **§17.1 / §17.2**, **§18**, **§20**, **§21.1**, **§22**, and **Appendices A, C and E.7**.

**What the final repair before G1 audit round 3 changed here, 2026-08-24** *(the repair responding to round-2 findings before the third and final audit round permitted by the `T0` cap; the delivery plan §0 and the register §1 carry their own matching entries)*: **§0.1's Q6 index is aligned** with the canonical §21.1 Q6 and §10.2, which already carried the owner's settled 2026-08-23 decision — subaccounts preferred, one subaccount plus its own agent wallet per risk bucket by default, virtual books the specified fallback, `WP-P0-28` verifying venue facts without reopening it; **the KILL baseline is corrected and re-anchored** in §10.2a and Appendix A, where a false `engine.py:391-404` citation and a false `cancel_all()` / "cancels all working orders indiscriminately" claim are replaced with the verified source behaviour, the still-true one-endpoint fact preserved; **every stale broker pointer is re-anchored** in §0.6 R16, §2 F-9a, §4.1 and Appendix A; and **source, repository-record and deployed-truth are separated** in §16 M9b, §17.1, §18 and §22, which no longer assert a running deployed soak as established fact. **This round added no requirement, no safeguard, no package, no gate, no tier, no milestone, no stage and no owner decision, and reworded and renumbered none.**

**The owner-authorized post-cycle closure repairs, 2026-08-24** *(recorded here per this ledger's own convention that every repair round records itself; the delivery plan §0 and the register §1 carry their own matching entries)*. The bounded cycle's final round was interrupted by an owner-requested shutdown and returned no verdict; the owner then explicitly authorized, in-session on 2026-08-24, a rerun of that final round and bounded closure rounds after it. Three bounded repair commits landed, each fixing only reproduced required findings of those owner-authorized rounds: **`8882af26…`** corrected the two line-5 provenance status paragraphs in `OWNER_MASTER_PLAN_2026-08-22.md` and `PROJECT_STARTING_POINT_AND_MAIN_OBJECTIVE_2026-08-22.md` (their earlier miscount — which described the two repair passes as three full repair rounds — replaced with the ledger-accurate bounded-cycle wording; the superseded wording is deliberately not quoted here); **`b2850ac4…`** removed a stale `cancel_all()` clause from the §10.2a deployed-truth paragraph; and **`fa4c02e2…`** declared the `WP-V4-09 Milestone 1` dependency edge in `WP-V4-01`'s `Depends on` cell in the delivery plan and re-anchored the F-14/F-15 pointers (`db.py:268`; `engine.py:70,133`) at their four sites here. The commit that records this entry is itself part of that closure sequence and is provenance-only. **No requirement, safeguard, package, milestone, stage, gate, tier, count or owner decision changed in any of them: the totals stay 60 = 44 OWNER + 16 DERIVED and 76 work packages, `OPEN-01` stays `[OPEN]`, every package, milestone and stage remains NOT STARTED, and no accepting audit verdict, no fresh `G1` acceptance and no `G1-IA` is claimed by this entry or by any of those commits.**

**The V-Next deferral ratification, 2026-08-24** *(the delivery plan §0 and the register §1 carry their own matching entries)*. After the fresh `G1` acceptance of `a00446a6…` (§0.8.3 item 3), the owner **ratified in full — Parts 1, 2 and 3** — the deferral package drafted at his instruction *"don't descope — move the non-critical things to future versions"* (`WAYFINDER_VNEXT_DEFERRAL_DRAFT_2026-08-24.md`). What it changes in this document set: **(Part 1)** eleven packages gain a **`V-NEXT`** tag in the delivery plan — WP-V3-01, WP-V3-11, WP-V2B-08, WP-V3-07, WP-V4-03, WP-V4-06 Stages 2–3 only, WP-V3-08, WP-V3-10, WP-P0-17, WP-V2B-09, WP-P0-16 — each a **move, not a deletion**: ID, contract, tier, gates and dependencies unchanged, not authorizable before first live unless the owner pulls it forward; the owner approved the consequent **O-02/O-29 delay**, with an interim QuantStats tearsheet carried by WP-P0-14. **(Part 2)** the **paper-admission split**: a new **`PAPER_ELIGIBLE`** decision issued by the Environment Admission Authority (§11.5), admitting **`INTERNAL_PAPER` only**, requiring the paper lane's genuine needs and **not** the Milestone-0 drills, credentials or command-surface chain; `TESTNET_ELIGIBLE` unchanged; WP-V2B-07's dependencies restated per lane, Lane B unchanged; **D-14's decision enumeration is extended under this explicit owner ratification** — the only requirement-text edit this ratification makes, with the safeguard's principle, class and the counts unchanged. **(Part 3)** the `V2B-11 → V2B-07 Lane A` paper-release checkpoint edge is drawn, and V2B-07's "runs alone" rule is scoped (no other venue-contacting work; nothing touching the frozen identity/soak host/environment; dependency-ready V3 research work may continue); Part 3's other two items were dashboard-side defects carried by the owner-issued round-2 dashboard fix prompt. **The totals stay 60 = 44 OWNER + 16 DERIVED and 76 work packages; no package, gate, tier, milestone, stage, lifecycle state or environment is added, removed or renumbered; no protected gate is weakened; `OPEN-01` stays `[OPEN]`; D-12 binds — this ratification authorizes no implementation, host contact, credential, deployment, testnet, live or trading action by itself.** **The amendment is material to the `a00446a6…` acceptance: a fresh dual-flagship `G1` round on the amended exact SHA is required, and no acceptance of the amended set is claimed by this entry.**
...
**The owner-authorized V-Next closure repair, 2026-08-24** *(the delivery plan §0 carries its matching entry)*. After the three-round cap closed without joint acceptance, the owner explicitly authorized one bounded closure commit and one closure audit round. In this brief it completes the propagation of the ratified content: **A-18** gains the `PAPER_ELIGIBLE` enumeration; **`V-NEXT` tags** are completed at the Stage-2 ordering line, the three-mode dragging table, the mobile-doctrine command stages, the §13.2 NautilusTrader and optuna-dashboard rows, **§22 item 8**, and the §16 M10/WP-V2B-09 migration surfaces. The two owner documents' D-14/admission wordings gain the same enumeration under explicit owner-attribution brackets, and the delivery plan's §2 gains the per-milestone reading of the combined `B07` arrows. **Counts unchanged (60 = 44 + 16; 76); no gate weakened; nothing authorized; no acceptance claimed — one owner-authorized closure audit round on this exact SHA follows.**

**The second owner-authorized V-Next closure repair, 2026-08-24** *(the delivery plan §0 carries its matching entry)*. The first closure round's dual REQUEST_CHANGES shared one required finding: **§22 item 8 was in the first closure's mandate, was not edited, and the first closure's ledger entries wrongly recorded it as tagged** — the §13.2 table row was tagged instead. The inaccuracy is acknowledged, not edited away; **§22 item 8 now carries its `V-NEXT` tag**, as do the §16 operative M10/WP-V2B-09 sentences, the §11.1 step-3 and §11.4 catalog sites; the §17.3 nested-bold defect is fixed; the plan's WP-V3-03 non-goals enumeration gains paper. **Counts unchanged; no gate weakened; nothing authorized; no acceptance claimed — a fresh dual-flagship round on this exact SHA follows.**

**The third owner-authorized V-Next closure repair, 2026-08-24** *(the delivery plan §0 carries its matching entry)*. Iteration 2's converging union closed: in this brief, the **§11.4 WP-V3-11 catalog heading**, **§11.6 item 2**, **§12.2's execution-reuse WP-V3-11 mention** and **Appendix E.6's O-11 Due cell** gain their `V-NEXT` tags; in the owner plan, the wave-1 "isolated proofs of concept" phrase gains the planning-status note excluding WP-P0-17. **Counts unchanged; no gate weakened; nothing authorized; no acceptance claimed — a fresh dual-flagship round on this exact SHA follows.**

**Status-only acceptance record, added 2026-08-24 (V-Next amendment cycle): GRANTED.** A fresh dual-flagship `G1` acceptance was granted on 2026-08-24 for the exact candidate commit **`3b088fba01a1f73e6ccda3a8cbfd622c1d976bb1`** — the V-Next-amended set (owner-ratified Parts 1–3, three audited repair rounds, three owner-authorized closure iterations): fresh independent read-only `claude-opus-5` at `xhigh` returned **PASS-WITH-NITS** (two optional tag-consistency nits, zero required findings) and fresh independent read-only Codex `gpt-5.6-sol` at `xhigh` returned **PASS** (zero findings), with the audited worktree clean before and after both audits. **The acceptance attaches to that exact commit only; the commit recording this note is not itself an audited artefact. It changes no package, milestone, stage, gate, tier, requirement or count; every package not separately owner-authorized remains NOT STARTED; and nothing beyond the owner's separately given authorizations is authorized by this record.**

**P0-31 M1 / P0-14 sequencing and authority, owner decision 2026-09-12.** Fixture-only P0-31 Milestone 1 engineering may precede accepted P0-13, but M1 acceptance still requires exact accepted P0-04/P0-13 provenance, resolution of its retained lifecycle-contract blockers, and the complete fresh T0 review path. An owner-ratified worthiness check-set version is instead an operating precondition for writing any real `CAPTURED -> TRIAGED` record; until then that write remains fail-closed. Early engineering supplies no authoritative receipt, runtime integration, admission, or acceptance. M1 remains before integrated Bridge admission and neither authorizes nor satisfies WP-V2A-01 or another consumer gate. M2 remains seed import only; M3 remains consumer repair/cutover plus exact-target retirement, de-tracking, and deletion. Each milestone is separate, unstarted until its own authority exists, and accepts or authorizes no later milestone. P0-14 remains OPEN; P0-31's small provenance-first read-only report is temporary continuity, not P0-14 completion or acceptance. This is a narrow P0-14-only sequencing exception, not a blanket Phase-0-before-V2A waiver or operational, deployment, trading, migration, retirement, deletion, or acceptance authority. Current implementation, evidence, review status, and exact blockers live only in `11_TRIAGE/P031_M1_SCOPE_AND_STATUS.md`.
...
1. **Historical `G1` acceptance stands where it was earned.** It is **SATISFIED for the exact candidate commit `c81aacb83f89ebc2454ddc80b4fd619ae8fd57f5`** (§0.7), **and for nothing else.** The **counts recorded there — 60 requirements and 69 work packages — are historical to that commit.**
2. **The current planning set is the amended one**, carrying all nine folds and the 2026-08-24 amendments above. **Its canonical totals are 60 tracked requirements = 44 OWNER + 16 DERIVED, and 76 work-package definitions.**
3. **A fresh `G1` acceptance of this amended set has not been granted at the time this candidate is written.** Every fold and repair entry recommends one, and **this subsection claims no accepting audit verdict, no `G1` acceptance and no result for any audit round that may follow.** **A later status-only record may record an accepted exact SHA; until such a record exists, none may be inferred.** **Status-only record, added 2026-08-24: GRANTED.** A fresh `G1` acceptance was granted on 2026-08-24 for the exact candidate commit **`a00446a63c586cf7c55aaf7d59987f9e7ad80500`**: a fresh independent read-only **`claude-opus-5`** at `xhigh` returned **PASS**, a fresh independent read-only Codex **`gpt-5.6-sol`** at `xhigh` returned **PASS**, no required finding remains unresolved, and the audited worktree was clean before and after both audits. **The acceptance attaches to that exact commit only; the commit recording this sentence is not itself an audited artefact and must not be presented as one; and item 4 below is unchanged in force — `G1-IA` remains UNSATISFIED, every package, milestone and stage remains NOT STARTED, and nothing is authorized by this record.**
4. **`G1-IA` remains UNSATISFIED.** **Every work package, every named milestone and every named stage is NOT STARTED**, and each separately requires the owner's explicit implementation authorization at the time it starts. **Implementation, code, host contact, credential use, testnet action, live action, deployment, repository push, trading and spending all remain UNAUTHORIZED**, and gates `G2` … `G9` each still require the owner's word in its own act.
...
|---|---|---|
| **`Broker`** | `IBKR_PAPER_BRIDGE/bridge/broker/base.py:156` | The accepted abstract exchange interface: `connect`, `account`, `positions`, `open_orders`, `historical_bars`, `subscribe_bars`, `subscribe_user_events`, `planned_cloids`, `place_bracket`, `submission_recovery_evidence`, `modify_stop`, `cancel`, `cancel_all`, `flatten`, `reprotect_position` |
| **`PartialRecoveryBroker`** | `base.py:234` | Deliberately **separate** opt-in protocol (TS-P1-004): `lot_unit`, `symbol_snapshot`, `query_order`, `cancel_order_by_cloid`, `place_protective_stop`, `flatten_reduce_only`. Every result is typed and conservative — transport failure maps to `UNKNOWN`, never to claimed success. `OrderManager` feature-detects it and treats absence as *"recovery unavailable"* |
| **`FullReconciliationBroker`** | `base.py:347` | Separate **read-only** opt-in protocol (TS-P1-005): `lot_unit`, `portfolio_evidence`, `open_orders_evidence`, `fills_evidence`, **`funding_evidence` (`base.py:374-378`, implemented by both `HyperliquidBroker` and `MockBroker`)** — *the fifth method was missing from this inventory and was added in the R1 correction pass; an incomplete inventory of an existing boundary is exactly what makes "we need a new one" look reasonable.* Incomplete pagination fails closed rather than reporting an empty-but-accepted component |
| **`HyperliquidBroker`** | `bridge/broker/hyperliquid.py:115` | The live concrete implementation |
| **`MockBroker`** | `bridge/broker/mock.py:70` | The local concrete implementation, and the `INTERNAL_PAPER` fill source of §6.4 |
| Failure types | `PartialRecoveryUnavailable` `base.py:226` · `FullReconciliationUnavailable` `base.py:339` | Each carrying a machine-readable `reason_code` |

**Pointer provenance — re-anchored 2026-08-24 (G1 final repair before audit round 3).** Every line reference in the table above is **verified directly against planning candidate baseline `01e0725890e456d079bca8967625ccb09c66b889`**. *(They previously read `base.py:154` / `:230` / `:293` / `:320-323` / `:222` / `:285`, `hyperliquid.py:105` and `mock.py:68` — accurate when first recorded on 2026-08-22 and stale since. **No symbol, protocol, method inventory, finding or decision changes with the re-anchoring**; only the pointers do.)* **These pointers locate repository source at that candidate baseline. They are not a claim about the deployed runtime**, whose version, configuration, schema version and behaviour remain **`UNVERIFIED`** pending gate **`G9`** (F-14, F-15, §12.6.5 item 7). **`WP-P0-25`'s acceptance gate already requires the boundary to be cited by file and line; those citations are re-verified against the exact commit in force when that package starts, not taken from this document.**
...
| **A** | Broker Adapter | Venue mechanics: precision, tick/lot, order types, reduce-only, error mapping. **This is the *existing* Bridge boundary — `Broker` / `PartialRecoveryBroker` / `FullReconciliationBroker` (`bridge/broker/base.py:156,234,347`, re-anchored 2026-08-24 against candidate baseline `01e0725890e456d079bca8967625ccb09c66b889` from the stale `:154,230,293`; source location only, deployed runtime `UNVERIFIED` pending G9) with `HyperliquidBroker` and `MockBroker` — not a new protocol.** Whether to reuse, extend or deliberately replace it is decided by **WP-P0-25** before any new adapter protocol is named (F-9a, §0.6 R16). |
...
**Ratified reading — map #54 fold (2026-08-23).** The lifecycle below is now read through the owner-ratified canonical state machine (wayfinder map [#54](https://github.com/bsemaay-tech/mtc-command-center/issues/54), tickets #59–#65; `WAYFINDER_LIFECYCLE_FOLD_2026-08-23.md`): **one pipeline, two named domains, one door.** The **RESEARCH FUNNEL** (`CAPTURED` → `TRIAGED` → `CANDIDATE` → `FROZEN` | `PARKED` | `REJECTED`, with `DECLINED` at triage; every rejection/park/decline re-entry-eligible under §6.9) begins **before candidate creation** — the dead-drop intake folder is replaced by recorded front-door states, and the `CAPTURED`→`TRIAGED` decision is where the owner-gated worthiness checklist applies. **Checklist status, stated exactly (2026-08-24, G1 round-1 repair): the only worthiness checklist that exists is `WAYFINDER_LIFECYCLE_FOLD_2026-08-23.md` §5 v0.1, which that record states is a DRAFT for the owner's ratification and which "binds nothing until ratified". This brief previously read as though it were already being applied. It is not. The v0.1 checklist is INACTIVE, and `CAPTURED`→`TRIAGED` is `BLOCKED` until the owner ratifies an exact checklist version, which then becomes the `check_set_version` that transition records.** **Nothing else ratifies it:** `G1` acceptance of this brief is document acceptance on the audit axis and **cannot** ratify an owner-gated definition (**D-12**); nor may ratification be inferred from the owner's acceptance of the **nine overnight workflow recommendations**, from any wayfinder fold, from a carrier clarification, or from this repair. **This follows §6.5's own doctrine that an unset threshold or an unavailable check yields `BLOCKED`, never `PASS`** — and it applies to the checklist itself, not only to the checks inside it. **No checklist content is invented, added, altered, weakened or ratified here, and no requirement, safeguard, count, package, gate, stage or owner decision changes.** `FROZEN` (steps 7/14b) is **the only door** to the **EVIDENCE LADDER** (`SHADOW` → `TESTNET` → `LIVE_CANDIDATE`, automatic bookkeeping → `LIVE` via the signed gate — **the only human signature in the machine** → the §6.9 tail). **Step 17's serial placement is abolished: portfolio-fit is a parallel evidence lane** whose verdict is an input to the live gate (the test's content belongs to the Bridge V2 map; this section fixes only its position). **Step 21 is superseded by the designed post-live tail (§6.9).** The plan vocabulary is the ONLY lifecycle vocabulary: the legacy 7-stage promotion ladder is retired via the fold's one-time mapping table (fold §4), the nine diagnostic labels survive as descriptive tags only (they say WHY, never WHERE), and `MTC_ENGINE_VALIDATED` is dropped as a state. Pre-freeze research artifacts are permanently navigational — no retroactive hash blessing. The diagram is retained as drawn; where it and this banner differ, the banner governs.
...
2. **Criteria changes are owner-gated; applications never are.** Changing a check set, the slot-ranking rule, or the worthiness checklist is a register-change-control act with the owner's word. Applying them per candidate is the machine's job, forever. **And a definition that has never been ratified has nothing to apply** *(stated 2026-08-24, G1 round-1 repair, consistently with the §6.3 banner):* the **worthiness checklist v0.1 is a DRAFT and is INACTIVE**, so **`CAPTURED`→`TRIAGED` is `BLOCKED`** — automatic application of that checklist begins at the owner's ratification of an exact version and never before. **`BLOCKED` is not `FAIL` and is never `PASS`** (rule 1's doctrine, above).
...
| **Failure mode 2** | *Second controller returns.* Someone repopulates the routing fields, or an old copy survives. **Mitigation:** delete the `alert()` **emission**, not merely the codes. A file with neither `alert(` nor `alertcondition(` cannot use those guarded Pine alert forms regardless of configuration. CI grep guard with an allowlist that is empty by default. |
...
4. **Across the entire active source tree, zero `.pine` files contain either `alert(` or `alertcondition(`, under an allowlist that is empty.** That is what makes the guard satisfiable.
...
6. Add a repo-guard check over the **entire active source tree**: any `.pine` file containing either `alert(` or `alertcondition(` must appear in an explicit allowlist, and **the allowlist is empty** — so the check passes only when **no active `.pine` file contains either byte needle anywhere**. The frozen tag is outside the active tree and is not scanned. **Per D026 the guard must be shown RED when either byte needle is deliberately reintroduced anywhere in the active tree, and GREEN after its removal**, before it counts as protection.
...
| Evidence | The owner has a package specific enough to authorize or refuse | Freeze tag exists **before** the edit; **zero `.pine` files in the active tree contain either `alert(` or `alertcondition(` under an empty allowlist**, with the guard proven **RED when either byte needle is deliberately reintroduced anywhere in the active tree and GREEN after removal**, per D026; the visualization-only source renders identically to the frozen original on a fixed chart and reference dataset; a **rollback path walked at least once**, restoring the controller from its tag |
...
**What the repository source shows, stated at source level.** *(Reworded 2026-08-24, G1 round-1 repair. This paragraph was headed "What the running Bridge actually does today" and asserted facts about "the running Bridge". **Nobody has contacted the host, and nobody is authorized to**, so nothing here can be an observation of a running process. The evidence behind every claim below is the **repository source**, and that is now what it is stated as. The claims themselves are unchanged — only the axis they are asserted on is corrected.)* The first repair said the running Bridge *"treats them as separate"*. **It does not — and the accurate correction is a source-level one.** **In the V1 source as read in this repository, the Bridge exposes one `POST /api/kill` path with an optional `flatten` parameter** (`IBKR_PAPER_BRIDGE/bridge/api/routes.py:113-123`, with `/api/kill/ack` at `:125`) **and no `/api/flatten`**. **There are no separate KILL and FLATTEN API operations in that source.** What the source gets right is that flatten is **not** implicit; what it does not have is the **separation** — one path, one parameter, one confirmation. **The V2 plan creates that separation (WP-V2B-10); it does not describe something already present in the source.**

**What the source's kill path actually does, corrected and re-anchored 2026-08-24 (G1 final repair before audit round 3), verified against planning candidate baseline `01e0725890e456d079bca8967625ccb09c66b889`.** *(This paragraph replaces a citation and a claim that were both wrong. The prior text cited `bridge/engine/engine.py:391-404` as `kill(flatten: bool = False)` "latch `KILLED` and `cancel_all()`". **`engine.py:391-404` is the DISARM region, not the kill path, and `Engine.kill()` does not call `cancel_all()` anywhere.** The **one-endpoint fact above is unaffected and stands**; what follows corrects the semantics beneath it. **No requirement, safeguard, package, gate, tier or owner decision changes with this correction, and it removes no V2 obligation.**)*

- **`Engine.kill()` begins at `IBKR_PAPER_BRIDGE/bridge/engine/engine.py:421`**; `acknowledge_kill()` is at **`:539`**.
- **On an older store schema** — `store.kill_evidence_enabled()` false — **it latches `KILLED` durably and performs no broad mutation at all**, returning at **`engine.py:450-455`**. It cancels nothing and flattens nothing. The source comment there states the reason: v9 is opt-in, and an older store can never acknowledge without the durable evidence contract.
- **With schema-v9 kill evidence enabled, the source already implements selective, protection-preserving semantics.** The kill episode **cancels only the orders it has classified as risk-increasing** at **`IBKR_PAPER_BRIDGE/bridge/engine/orders.py:1787-1797`**. With an **open position and no `flatten`**, it **requires and preserves qualifying owned reduce-only protection and returns `SAFE_RETAINED`** at **`orders.py:1853-1874`** — and **fails closed** as `UNRESOLVED / EXACT_OWNED_PROTECTION_MISSING` when that protection is absent, rather than proceeding. **Controlled flatten begins at `orders.py:1882`**, and only when the caller passed the flag.
- **So the accurate statement is:** **the source does not cancel all working orders indiscriminately.** Under enabled v9 it **already** performs selective risk-order cancellation with protection preservation; under an older schema it **only latches**. **What the source does not have is a distinct authenticated FLATTEN operation with its own API path, confirmation and audit record**, nor the operator-facing separation, scope/precedence model, command lifecycle and activation/evidence contract. **Those remain real V2 work under `WP-V2B-10`, and nothing here reduces them** — but no document may describe the source as lacking semantics it already implements.
- **Which of those two paths the deployed process is on is a separate and unanswered question**, governed by the paragraph immediately below: the **deployed schema and runtime remain `UNVERIFIED`** pending **`G9`**.

**What the deployed runtime does is a separate question, and this section does not answer it.** Per **F-14, F-15 and §12.6.5 item 7**, the **deployed version, configuration, schema version and runtime behaviour remain `UNVERIFIED`** — including which schema-dependent path above is active; everything above is what the source says and **not** a statement about what is running. *(Clause corrected 2026-08-24, owner-authorized closure repair: an earlier version of this sentence still referenced a `cancel_all()` behaviour that the 2026-08-24 correction above had already established the source does not have; §0.8.2 records the closure repairs.)* **No host was contacted here and none may be.** **No design, gate, acceptance or evidence claim in this document set may rest on deployed behaviour**: it is independently verified under **G9** first, or it stays **`UNKNOWN`**. The map-#96 amendment at the end of this section states the same separation and is not weakened by anything above.
...
| **Phase availability** *(stated at source level 2026-08-24, G1 round-1 repair: every cell in this row describes the **V1 source in this repository**, never the deployed runtime, which stays **`UNVERIFIED`** pending G9. The cells previously read "Exists today" / "Does not exist … today", which reads as a host observation nobody has made)* | **Present in the V1 source**; carried through V2 | **Present in the V1 source as the single `POST /api/kill` path** *(cell corrected 2026-08-24, G1 final repair before audit round 3: it read "which as written cancels working orders indiscriminately", which the source contradicts — see the corrected source paragraph above)*. **Under an older store schema that path only latches `KILLED` and performs no broad mutation (`engine.py:450-455`); under enabled schema-v9 kill evidence it already cancels only classified risk-increasing orders (`orders.py:1787-1797`) and already requires and preserves qualifying owned reduce-only protection, returning `SAFE_RETAINED` (`orders.py:1853-1874`).** **What V2B adds is not the idea of protection-preserving cancellation but the operator-facing separation, the scope/precedence model, the command lifecycle, the retirement of the `flatten` parameter from this path and the activation/evidence contract** — and **which schema the deployed process runs on stays `UNVERIFIED`** | **Absent from the V1 source as an operation** — that source carries only an optional `flatten` parameter on the one `/api/kill` path, and no `/api/flatten`. It **becomes a separately named operation with its own API path, confirmation and audit record** in V2B | **Absent from the source, and not built in V2 or V3.** Design study only, WP-V3-10 |
...
**Account binding — [OWNER — Q6], DECIDED:** the confirmed preferred default is **one subaccount plus its own dedicated agent wallet / worker per risk bucket**, per-strategy where capacity allows under the venue's subaccount cap. **Virtual books inside one account are the specified fallback**, activated only on the recorded triggers — volume-gate ineligibility, cap exhaustion, venue restriction — and the fallback spec **must be accepted before `WP-V2B-03` starts**, not written during an incident. **`WP-P0-28` (VEN-A) verifies the remaining venue facts before implementation and does not reopen this decision.** *(Aligned 2026-08-24, G1 round-1 repair, with the owner's 2026-08-23 decision fold, ticket #40 — §21.1 Q6 governs; this line previously read "decide after WP-P0-28's official Hyperliquid verification, then prefer …", which contradicted the settled decision recorded elsewhere in this same document set. Carrier re-pointed 2026-08-24, map #123 fold, from the retired 2026-08-17 "Package 7" name that WP-P0-28 absorbed; **the owner decision itself is unchanged and nothing here authorizes an account, a credential or any venue contact**.)*
...
3. **Compare the two search regimes on frozen strategies and frozen datasets**, measuring: trials to reach the same best OOS metric, coverage of the space, reproducibility under re-run, and the resulting DSR/BH-FDR family size. *(This comparison is WP-P0-16 — `V-NEXT`, owner-ratified deferral 2026-08-24; the sequence governs how it runs when it runs.)*
...
**ADVANCED EXPLORER — V3** *(WP-V3-01 — `V-NEXT`, owner-ratified deferral 2026-08-24; the Minimum Explorer WP-P0-14 stays pre-live)*
...
**PRODUCTION CHART AND RESULT-VISUAL SURFACE — V3, `WP-V3-11`.** *(`V-NEXT` — owner-ratified deferral 2026-08-24; the owner approved the O-02/O-29 delay, and the interim QuantStats tearsheet in WP-P0-14 covers the gap pre-live.)* *Added 2026-08-22 (§0.6 R10).*
...
| **Environment Admission Authority** (WP-V2A-10) | issue `SHADOW_ELIGIBLE`, `PAPER_ELIGIBLE` *(paper-admission split, owner-ratified 2026-08-24)* and `TESTNET_ELIGIBLE` admission decisions from accepted eligibility evidence | admit anything to mainnet or `LIMITED_LIVE`; produce eligibility evidence itself |
...
**Staged admission.** Admission is split into four immutable, identity-bound decisions produced by two authorities *(three until the owner-ratified paper-admission split of 2026-08-24 added `PAPER_ELIGIBLE`)*, in order:
...
| **`PAPER_ELIGIBLE`** *(paper-admission split, owner-ratified 2026-08-24)* | Environment Admission Authority | **V2B** (WP-V2B-07 Lane A) | An accepted `TESTNET_PAPER_ELIGIBLE` verdict set, plus the paper lane's genuine needs — frozen `deployment_identity_hash`, pre-registered soak plan, accepted OPS-A (WP-P0-26), worker reconciliation machinery (WP-V2A-02), portfolio divergence report (WP-V2B-03). **NOT the Milestone-0 drills, NOT credentials, NOT the command-surface completion chain** — those bind `TESTNET_ELIGIBLE`, which is unchanged | `INTERNAL_PAPER` only |
...
4. **No decision widens another.** `SHADOW_ELIGIBLE` never admits to testnet; **`PAPER_ELIGIBLE` never admits to anything beyond `INTERNAL_PAPER`** *(paper-admission split, owner-ratified 2026-08-24)*; `TESTNET_ELIGIBLE` never admits to mainnet; and **nothing except a Promotion Authority decision admits to real capital.** An escalation is a new decision with new evidence, not a re-reading of an old one.
...
2. **Layout ratified from the prototype (#94):** **Inspector** — chart + right-rail per-trade evidence card (entry/exit/stop/TPs, R, duration, MAE/MFE, fees/funding) + a compact linked trade list (`prototype/result-chart-mock-94`, `ce7d227a`/`578f3cfd`). **Read-only display of the strategy's own indicators is IN scope** — requiring the indicator series to be carried in the §11.2 per-trial artifacts. *(This item is WP-V3-11 content — `V-NEXT`, owner-ratified deferral 2026-08-24, as item 5 below already records.)*
...
5. **Phase-0 minimum vs Stage-2/V3 (#83):** P0 = family tree with lifecycle states; per-candidate "where is it and why" (rejection reasons + re-entry eligibility); screens-vs-evidence labelling; basic summary result views **including the TV-style KPI row**. **The trade-truth chart (+ indicator overlays + MAE/MFE/fees) is the FIRST Stage-2 addition, carried by WP-V3-11 — it leaves Phase-0 scope** *(WP-V3-11 is `V-NEXT`, owner-ratified deferral 2026-08-24; the ordering governs when the V-Next work eventually runs, and the interim QuantStats tearsheet in WP-P0-14 covers the gap pre-live)*. Daily home screen = the family tree + a ledger-derived "what changed since yesterday" strip. V3 analytics order: cross-candidate comparison → parameter importance → Pareto/parallel-coordinates/3-D surfaces.
...
**Three modes, gated in this order.** *(All three carriers — WP-V2B-08, WP-V3-07, WP-V4-03 — are `V-NEXT`, owner-ratified deferral 2026-08-24: buttons instead of dragging pre-live. The gating order below governs when they eventually run; it is not a pre-live schedule.)*
...
- **And the Bridge source in this repository contains no WebAuthn path at all** — step-up authentication is a V2 design commitment, not an existing capability. *(Stated at source level 2026-08-24, G1 round-1 repair: this read "the current Bridge has no WebAuthn path", which reads as a claim about the deployed host.)* **The deployed version, configuration, schema version and runtime behaviour remain `UNVERIFIED`** (F-14, F-15, §12.6.5 item 7); on either axis there is no deployed single-key design to describe.
...
5. **Mobile and remote doctrine (#103).** One responsive private web surface serves phone and computer; no separate native app initially. Mobile starts with a read-only 60-second incident view, then gains DISARM after accepted authentication evidence and KILL/FLATTEN only after map-#96 design plus the required local/testnet drill evidence *(the DISARM and KILL/FLATTEN steps are WP-V4-06 Stages 2–3 — `V-NEXT`, owner-ratified deferral 2026-08-24; the read-only incident view stays on the pre-live road)*. ARM and protective dragging stay laptop/desktop-only unless separately re-decided. Private mesh, approved devices, WebAuthn/FIDO2 and authenticator redundancy remain binding. Background resume forces a fresh snapshot and reconciliation before controls enable; offline/stale screens are visibly frozen and read-only. Notifications deep-link to authenticated incident detail but never contain trading controls or clear the underlying fault.
...
|---|---|---|---|---|
| [**NautilusTrader**](https://github.com/nautechsystems/nautilus_trader) | LGPL-3.0-only | ~26.8k ★ | `POC_ONLY` in V3 — **`V-NEXT` (WP-V3-08, owner-ratified deferral 2026-08-24)** | **[OWNER — Q12] POC first; do not replace the Bridge in V2.** Ships **stable** adapters for **both Hyperliquid and Interactive Brokers**, and documents that *"the same execution semantics and deterministic time model operate in both research and live systems."* That matches your multi-broker + parity requirement exactly. POC scope: an independent backtest of one promoted candidate as a second opinion. Any later production use requires a documented licensing review of the linking boundary. |
...
- **No Bridge runtime code path changes in Phase 0.** The Bridge does not construct, consume or act on `OrderIntent` at runtime, its behaviour is byte-for-byte what it is today, and **Bridge V1 — including any separately authorized V1 soak — is unaffected** *(reworded 2026-08-24, G1 final repair before audit round 3: this read "the V1 soak is unaffected", which presupposes a running soak on an unverified host; **the deployed state is `UNVERIFIED` pending `G9`** — F-14, F-15 — and **the guarantee is unchanged: Phase 0 changes nothing about V1 either way**)*.
...
5. **Large run outputs and evidence leave ordinary Git storage** for **indexed, immutable, hash-verified artifact storage**; **Git keeps source, contracts, small manifests and pointers**. This direction is independent of the split question. **It is carried by existing packages — no new package is created for it:**
   ...
   - **WP-V2B-09** *(`V-NEXT`, owner-ratified deferral 2026-08-24)* performs the **in-repository workflow cutover** so that **new** writes land at those destinations and **Git holds the hash and index pointers** rather than the payloads. **It deletes no history and de-tracks no historical path.**
...
| **In-repository** routed cutover — the terminal topology state through Phase 0–V3 | **WP-V2B-09** *(`V-NEXT`, owner-ratified deferral 2026-08-24)* |
...
flowchart TD
    ...
    M9b --> M10["M10 WORKFLOW CUTOVER - inside this repository only (carrier V-NEXT)"]
...
| **Explicit non-goals** | No new trading features. No multi-strategy runtime. No broker work **— WP-P0-25 decides the broker boundary on paper and implements nothing**. **No Bridge behaviour change. No execution-dashboard rework** — clarified 2026-08-22 (§0.3 item 9): the non-goal is the *execution* surface, because the research-side **Minimum Explorer is explicitly in scope** above. Building a read-only research viewer is not "dashboard rework". **Bridge V1 is untouched, and any separately authorized V1 soak is untouched with it** *(reworded 2026-08-24, G1 final repair before audit round 3: this read "**The V1 soak is untouched**", which presupposes a running soak on a host nobody has contacted. **The deployed state is `UNVERIFIED` pending `G9`** — F-14, F-15 — and **the non-goal is unchanged and not weakened: Phase 0 touches V1 either way**)*. **No repository split, no topology migration and no topology implementation** — *reworded 2026-08-23 (map #97 fold): this read "No repository topology decision", which is stale, because the macro topology **is** decided (§14.4, §14.6): the stage-routed monorepo is the ratified default through Phase 0–V3. What Phase 0 excludes is the **act** — no second repository, no tree moved between repositories, no archival of this one; Phase 0 builds the routed structure in place (WP-P0-05) and produces the measurement that a later conditional-split trigger would be evaluated against.* No Bridge runtime wiring of the contracts package (§14.3). **No production chart surface — that is WP-V3-11, deliberately not pulled forward.** |
...
| **OSS to evaluate** | Parquet/PyArrow, DuckDB (adopt); Optuna vs grid comparison (§11.1) **— `V-NEXT` (WP-P0-16, owner-ratified deferral 2026-08-24: the optimizer study waits until after first paper)**; optuna-dashboard **only as an optional ≤ 2-day isolated POC** (§13.2) **— `V-NEXT` (WP-P0-17, same ratification)** — never a second maintained viewer; **retirement or removal is separately owner-authorized, after its findings, measurements and decision record are preserved** |
| **Owner decisions needed** | none outstanding |

**Narrow sequencing exception, owner decision 2026-09-12.** Core Bridge engineering may proceed while P0-14 remains OPEN and deferred, but integrated Bridge admission and every consumer gate retain their prerequisites. P0-31 M1 stays before that admission; fixture implementation alone satisfies neither M1 acceptance nor WP-V2A-01. The temporary P0-31 read-only report preserves provenance/status continuity and is not the Minimum Explorer.
...
| **Owner decisions needed** | **None outstanding for Q6 — it is decided** (subaccounts preferred, one subaccount plus its own agent wallet / worker per risk bucket; virtual books the specified fallback, frozen before `WP-V2B-03`). **What remains is `WP-P0-28` (VEN-A)'s verification of the outstanding venue facts, which is implementation input and not a reopening of the decision** *(row aligned 2026-08-24, G1 round-1 repair — §21.1 Q6 governs; this cell previously read "Q6 resolves after WP-P0-28 (VEN-A)", contradicting the "Q6 is decided" statement two rows above it. Carrier re-pointed 2026-08-24, map #123 fold, from the retired "Package 7" name)* |
...
| | |
|---|---|
| **Scope** | **V-Next ratification note (owner-ratified 2026-08-24, delivery plan §0): the `V-NEXT`-bracketed items below keep their contracts but leave the pre-live road; V3's pre-live remainder is the artifact tier, the Promotion Authority chain, portfolio backtesting, correlation gates, divergence reporting, the Missing-Rule Ledger and testnet Multi-TP.** Advanced Explorer: parallel coordinates, 3-D response surfaces with plateau/needle detection, parameter importance, Pareto, comparison, `SIGNAL_EDGE`/baseline/enriched A/B — **`V-NEXT` (WP-V3-01; the Minimum Explorer WP-P0-14 stays)**. **Added 2026-08-22 (§0.6 R10): the production chart and result-visual surface (WP-V3-11) — named position, SL, TP, Multi-TP and trailing-stop visual state plus every promised result visual and statistic, on real artifacts, with one-click variant navigation and timeframe/strategy filtering — `V-NEXT` (owner approved the O-02/O-29 delay; an interim QuantStats tearsheet ships pre-live inside WP-P0-14's result views).** Full artifact tier + deterministic replay + 20 GB retention (Q9). Promotion Authority + immutable decision artifacts. Portfolio backtesting with allocator and Guardian simulated. Correlation gates: promotion, runtime, live monitor. Live-vs-backtest divergence reporting. Missing-Rule Ledger in the promotion packet. **Multi-TP: testnet execution, partial fills, reconciliation.** Drag-and-drop **paper/testnet mode** — **`V-NEXT` (WP-V3-07)**. **NautilusTrader POC (Q12)** — **`V-NEXT` (WP-V3-08)**. |
| **Explicit non-goals** | No live drag-and-drop. No new brokers in production. No ML. No Guardian resizing without a versioned, backtested allocation policy. **No emergency existing-exposure reduction capability is built here either: under the D-02 clarification (§0.5) that would be a distinct, separately authorized, explicit and tested safety policy, and V3 carries only the WP-V3-10 design study of Appendix E O-11 and O-12 — itself `V-NEXT` (owner-ratified deferral 2026-08-24).** |
...
| | |
|---|---|
| **Scope** | Drag-and-drop **live mode** with the full §12.4 safety chain and human-override accounting — **`V-NEXT` (WP-V4-03, owner-ratified deferral 2026-08-24; moves with WP-V3-07)**. Signed live-trading gate **(Q10)**. **`LIMITED_LIVE`: one strategy, ≤ 1 % of account (Q11) — owner clarification 2026-08-22: the 1 % is the MAXIMUM CAPITAL ALLOCATED to that strategy; loss-at-stop / risk-to-stop carries a SEPARATE and LOWER cap that must be defined and evidenced before live authorization, and no number for it is set here (register D-07, §0.5).** Multi-bucket operation (day/swing/position) with calibrated allocations. Mobile monitoring — **the read-only incident view (WP-V4-06 Stage 1) stays; the mobile command stages 2–3 are `V-NEXT` (owner-ratified deferral 2026-08-24: commands come from the laptop)**. TradingView divergence alarm as a scheduled job. **Live Multi-TP after chaos and restart drills.** |
...
| **Dependencies** | **V3's pre-live packages complete — its `V-NEXT`-tagged rows (WP-V3-01, WP-V3-07, WP-V3-08, WP-V3-10, WP-V3-11) are NOT preconditions for V4** *(V-Next ratification, owner-ratified 2026-08-24; this cell previously read "V3 complete", which made deferred work a live-gate prerequisite)*; reconciliation active; auth hardened; **signed live gate with all FOURTEEN canonical hard preconditions evidenced together, in one dated decision, against the exact `deployment_identity_hash`** (corrected 2026-08-22, §0.6 R1/R2 — earlier text said "six") |
...
| R-5 | TradingView second controller resurfaces | Medium | Severe | Delete `alert()` emission; CI alert-family grep guard with an empty allowlist |
...
| R-18 | Bridge V1 — and any separately authorized V1 soak — disturbed by V2 work *(reworded 2026-08-24, G1 final repair before audit round 3; the deployed state is `UNVERIFIED` pending `G9`, and the mitigation is unchanged)* | Medium | High | V2 in separate branches/worktrees; V1 candidate frozen; standing rule unchanged |
...
| **DL-10** | **Account binding is decided: subaccounts preferred — one subaccount plus its own dedicated agent wallet / worker per risk bucket; virtual books are the specified fallback on the recorded triggers, frozen before `WP-V2B-03`.** **WP-P0-28 (VEN-A)** verifies the remaining venue facts before implementation and does not reopen the decision *(row aligned 2026-08-24, G1 round-1 repair, with the owner's 2026-08-23 decision fold, ticket #40 — §21.1 Q6 governs; carrier re-pointed 2026-08-24, map #123 fold, from the retired "Package 7" name; the prior wording "decided after WP-P0-28 verification; prefer Hyperliquid subaccounts where reliably supported" is retained here as the historical text it was)*. *(Q6)* |
...
| **Q6** | Subaccounts or virtual books? | **DECIDED — subaccounts are the confirmed preferred default.** *(Canonical text aligned 2026-08-24, G1 round-1 repair, with the owner's 2026-08-23 wayfinder decision fold, ticket #40 — `WAYFINDER_DECISION_FOLD_2026-08-23.md`. The decision is the owner's and is unchanged; it is not re-made, widened or narrowed here. This row previously still read "decide after verification", so the same document set had Q6 both decided and undecided.)* The default unit is **one subaccount plus its own dedicated agent wallet / worker per risk bucket**, per-strategy where capacity allows under the venue's subaccount cap. **Virtual books inside one account are the specified fallback**, activated only on the recorded triggers — **volume-gate ineligibility, cap exhaustion, or venue restriction** — and **the fallback spec must be accepted before `WP-V2B-03` starts**, never authored during an incident. **`WP-P0-28` (VEN-A) verifies the remaining venue facts before implementation** — same-symbol netting / hedge-mode wording, cross-plus-isolated margin coexistence, testnet volume-gate behaviour, agent-wallet expiry duration, and whether venue IP restriction exists — **and it does not reopen this settled preference-and-fallback decision.** *The original 2026-08-21 answer — "**Decide after verification**, then prefer subaccounts where Hyperliquid supports them reliably." — is preserved here as the historical answer it was.* **This is a decision on the D-12 decision axis only: it authorizes no implementation, no account creation, no credential and no venue contact.** |
...
| 6 | **De-fang TradingView (Q3) — two packages, not one.** **(a) WP-P0-19, design only, T2:** produce the exact file/line list, the **in-place transformation plan naming the one maintained active Pine source**, the empty-allowlist CI-guard specification, the divergence-alarm specification and the rollback path. Writes no Pine and no config. **(b) WP-P0-23, implementation, T0:** tag the controller, **transform the active Pine source in place into visualization-only Pine**, delete the two `alert()` emissions and the **13 `wt_*`** inputs/keys, and land the CI guard so that **zero `.pine` files in the active tree contain either `alert(` or `alertcondition(` under an empty allowlist**. **The alert-capable original survives only in the frozen tag — it does not remain active.** **The 7 `tw_*` keys are excluded — they are live, not inert (F-8a, §0.6 R12), and belong to the kernel chain WP-P0-09 → WP-P0-12.** | (a) **DOCUMENTATION ONLY** · (b) **WRITE — protected surface. See the authorization note below.** | (a) T2 · (b) **T0** |
...
| 8 | **`V-NEXT` (WP-P0-17, owner-ratified deferral 2026-08-24 — NOT a pre-live next action; this row describes how the POC runs when the V-Next work runs).** **Optional, timeboxed `optuna-dashboard` POC.** ≤ 2 days, **only if** it demonstrably saves work on the Minimum Explorer. Isolated by construction — see §13.2. Not a decision to adopt Optuna (O-2). **Skip it entirely if the Minimum Explorer is already close.** Once the Minimum Explorer renders `TrialRecord`, development and maintenance stop; **stopping maintenance does not authorize deletion** — cleanup is a **separate, explicitly owner-authorized act performed after findings, measurements and the decision record are preserved.** | **WRITE — isolated local POC tooling** | **T1 if POC code is built.** If it is skipped and produces **only a documented skip decision**, that closure artefact alone is **T2** *(matches WP-P0-17)* |
...
| 11 | **Leave Bridge V1 isolated and untouched, and leave any separately authorized V1 soak isolated too.** Nothing above requires touching the frozen V1 candidate. *(Reworded 2026-08-24, G1 final repair before audit round 3: this read "**Keep Bridge V1 soaking, untouched**", which asserts a running soak on the deployed host as established fact. **Repository records report that state; the deployed version, configuration, schema version and runtime behaviour are `UNVERIFIED` pending `G9`** — F-14, F-15, §12.6.5 item 7 — **and nothing here verifies a soak, starts one, or authorizes contacting the host to find out.** **The protective constraint is unchanged and is not weakened: V1 is untouched either way**, and no soak referred to anywhere in this document set counts as live-gate precondition 4 evidence, which is `WP-V2B-07` Lane A's alone.)* | **NO ACTION — protective constraint** | — |
...
| **The existing broker boundary** *(added 2026-08-22, §0.6 R16; **pointers re-anchored 2026-08-24, G1 final repair before audit round 3, against planning candidate baseline `01e0725890e456d079bca8967625ccb09c66b889`, from the stale `base.py:154` / `:230` / `:293` / `:320-323` / `:222` / `:285`, `hyperliquid.py:105`, `mock.py:68`. Repository-source locations only; deployed runtime identity remains `UNVERIFIED` pending `G9`*)* | `IBKR_PAPER_BRIDGE/bridge/broker/base.py:156` `Broker(Protocol)`, `:234` `PartialRecoveryBroker(Protocol)`, `:347` `FullReconciliationBroker(Protocol)` **including `funding_evidence` at `base.py:374-378`, implemented by both concrete brokers *(added in the R1 correction pass)***, `:226` `PartialRecoveryUnavailable` / `:339` `FullReconciliationUnavailable`, the two reason-code types; `bridge/broker/hyperliquid.py:115` `HyperliquidBroker`; `bridge/broker/mock.py:70` `MockBroker`. See F-9a |
...
| **The V1 SOURCE has ONE kill path with an optional `flatten` parameter — it does NOT have separate KILL and FLATTEN operations** *(added 2026-08-22, §0.6 R8; heading corrected in the R1 correction pass, which previously claimed the running Bridge already separates them; **restated at source level 2026-08-24, G1 round-1 repair — the heading still said "the running Bridge", which reads as an observation of the deployed host that nobody has made. Every cell in this row cites repository source; the deployed version, configuration, schema version and runtime behaviour remain `UNVERIFIED` pending G9**)* | **Re-anchored and corrected 2026-08-24 (G1 final repair before audit round 3) against planning candidate baseline `01e0725890e456d079bca8967625ccb09c66b889`.** `bridge/api/routes.py:113-123` exposes `POST /api/kill` with an optional `flatten` parameter, `:125` exposes `/api/kill/ack` — **and there is no `/api/flatten`**. `bridge/engine/engine.py:421` `Engine.kill(flatten: bool = False)`; `:539` `acknowledge_kill()`. **`Engine.kill()` does not call `cancel_all()`:** on an older store schema it **latches `KILLED` and performs no broad mutation**, returning at `engine.py:450-455`; with **schema-v9 kill evidence enabled** it **cancels only classified risk-increasing orders** at `bridge/engine/orders.py:1787-1797`, and with an open position and no flatten it **requires and preserves qualifying owned reduce-only protection, returning `SAFE_RETAINED`** at `orders.py:1853-1874`, with controlled flatten beginning at `orders.py:1882`. `bridge/app.py:109-110` sets `DISARMED` unless already `KILLED`. *(The prior cell cited `engine.py:391-404` — the DISARM region — and claimed `kill()` "calls `cancel_all()`"; both were wrong. **The row's headline claim is unchanged and still true: one kill endpoint, no separate FLATTEN operation.**)* **In that source flatten is not implicit and cancellation is not indiscriminate, but FLATTEN is still not a separate operation: that separation, its API path, its confirmation and its audit record are created by the V2 plan (WP-V2B-10). Nothing in this row is an observation of the deployed runtime, whose schema and behaviour remain `UNVERIFIED` pending `G9`.** See §10.2a |
...
| # | Item | Due |
|---|---|---|
| **O-11** | Whether reducing *existing* exposure may be treated differently from sizing *new* entries under the no-runtime-resize rule. **Distinction RESOLVED 2026-08-22 by the owner's D-02 clarification (§0.5, §5.5, §10.2 rule 7): it may — but only as a separately authorized, explicit, tested safety policy, never as silent resizing, and nothing authorizes building or using one. The remaining open part is the policy design itself.** | V3 allocation-policy design — **WP-V3-10, design only** *(`V-NEXT`, owner-ratified deferral 2026-08-24 — no longer a pre-live due date)* |
...
**Scope of this acceptance, stated so it cannot be stretched.** It attaches to **that candidate commit and nothing else**. A later commit that merely records this status is **not itself an audited artefact** and must not be presented as one. **Acceptance is not implementation authorization**: `G1-IA` remains unsatisfied, **every work package remains NOT STARTED** and separately requires the owner's explicit authorization, and implementation, deployment, host contact, schema activation, credential use, testnet action, live action, repository push, trading and spending all remain **UNAUTHORIZED**. **No requirement, count, safeguard wording, work package, dependency, tier or gate changed** — the totals stay **60 = 44 + 16**, and the package count at that accepted candidate commit was **69**. *(Repaired 2026-08-24, map #123 fold: this sentence read as a current total. **69 is the historical, pre-wayfinder figure at `c81aacb8…`; the current canonical total is 76** — the §0 STATUS block and **§0.8** govern. Later folds added definitions after that commit without changing the requirement count.)* **Amendment status, added 2026-08-24 (G1 final repair before audit round 3): the document set has been materially amended since this acceptance — the nine owner-ratified wayfinder folds (including the 2026-08-24 final red-team fold) and the bounded three-round G1 audit cycle, with repair passes before rounds 2 and 3, all indexed in §0.8. This appendix therefore records a HISTORICAL acceptance of an earlier candidate. A fresh `G1` acceptance of the amended set has NOT been granted at the time this candidate is written, no accepting audit verdict is claimed anywhere in this document, and none may be inferred; a later status-only record may record an accepted exact SHA.**
