...
| **Phase availability** *(stated at source level 2026-08-24, G1 round-1 repair: every cell in this row describes the **V1 source in this repository**, never the deployed runtime, which stays **`UNVERIFIED`** pending G9. The cells previously read "Exists today" / "Does not exist … today", which reads as a host observation nobody has made)* | **Present in the V1 source**; carried through V2 | **Present in the V1 source as the single `/api/kill` path**, which as written cancels working orders indiscriminately; the **protection-preserving, entry-only cancellation semantics are new in V2B** | **Absent from the V1 source as an operation** — that source carries only an optional `flatten` parameter on the one `/api/kill` path, and no `/api/flatten`. It **becomes a separately named operation with its own API path, confirmation and audit record** in V2B | **Absent from the source, and not built in V2 or V3.** Design study only, WP-V3-10 |
