...
# 2. PBO (uncapped is safe at 15 splits)
python probabilistic_pbo.py --cpcv "$RUN_DIR/cpcv15/cpcv_results.json" --out-dir "$RUN_DIR/pbo" --max-combinations 0 >/dev/null 2>&1
...
python build_evaluation_artifact.py --mega "$MEGA" $EXTRA --out-dir "$RUN_DIR/evaluation_artifacts" >/dev/null 2>&1
NART=$(ls "$RUN_DIR/evaluation_artifacts"/*.eval.json 2>/dev/null | wc -l)
log "eval artifacts: $NART"

# 4. Gate2
python score_gate2.py --in-dir "$RUN_DIR/evaluation_artifacts" --out-dir "$RUN_DIR/gate2_scorecards" >/dev/null 2>&1
log "gate2 scorecards: $(ls $RUN_DIR/gate2_scorecards/*.json 2>/dev/null | wc -l)"

# 5. All-gate evidence (emits Gate1/1B/3 sections)
python build_all_gate_evidence.py --eval-dir "$RUN_DIR/evaluation_artifacts" --mega "$MEGA" --out-dir "$RUN_DIR/all_gate" >/dev/null 2>&1
...
python enrich_gate3_evidence.py --in-dir "$RUN_DIR/all_gate" --out-dir "$RUN_DIR/all_gate_g3enriched" --run-id "$RUN_ID" 2>&1 | tail -1
python score_gate3.py --in-dir "$RUN_DIR/all_gate_g3enriched" --out-dir "$RUN_DIR/gate3_scorecards" >/dev/null 2>&1
...
python score_all_gates.py --in-dir "$RUN_DIR/all_gate_g3enriched" --out-dir "$RUN_DIR/scorecard_v2" 2>&1 | grep SUMMARY | tail -1

# 8. Alpha vs buy&hold (canonical)
MEGA_OUTPUT_DIR="$RUN_DIR" python alpha_vs_buyhold.py >/dev/null 2>&1
log "alpha: $([ -f $RUN_DIR/alpha_summary.json ] && echo OK || echo MISSING)"

# 9. Morning report
python heavy_night_report.py --run-dir "$RUN_DIR" --night-id "$RUN_ID" --out "$RUN_DIR/HEAVY_TIER_MORNING_REPORT.md" >/dev/null 2>&1
...
PYTHONPATH="$TOOLS/$API_REL" python -c "
