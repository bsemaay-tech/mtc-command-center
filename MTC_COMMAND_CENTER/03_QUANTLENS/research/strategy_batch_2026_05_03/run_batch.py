...
BATCH_ROOT = ROOT / "06_QUANTLENS_LAB" / "research" / "strategy_batch_2026_05_03"
...
STRATEGIES = [
    ...
    ("12_ema20_50_two_retests_baseline", "QL_EMA20_50_TWO_RETEST_BASELINE_v0", "QUANTLENS_EMA20_50_RETEST_CRITICAL_INTAKE_REPORT.md", "crypto", ["1D", "4h", "1h"]),
]
...
def write_batch_docs(index: pd.DataFrame, data_report: str, commands: list[str], git_before: str, git_after: str) -> None:
    ...
    (BATCH_ROOT / "README.md").write_text("# Strategy Batch 2026-05-03\n\nPython-first QuantLens research batch. Research-only; no Pine/MTC production edits.\n", encoding="utf-8")
...
def main() -> int:
    ...
    rows = []
    for slug, strategy_id, source, kind, timeframes in STRATEGIES:
        rows.append(run_strategy(slug, strategy_id, source, kind, timeframes))
    index = pd.DataFrame(rows).sort_values(["final_classification", "aggregate_pf"], ascending=[True, False]).reset_index(drop=True)
