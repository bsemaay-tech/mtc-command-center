...
def classify_item(doc: SourceDoc | None, intake: SourceDoc | None, faithfulness: str, corrected_path: str = "") -> dict:
    ...
    elif any(x in title_lower for x in ["sell rule", "sell rules", "when to take trading profits"]):
        classification = "EXIT_MODULE"
    elif any(x in title_lower for x in ["risk management", "position sizing", "sizing"]):
        classification = "SIZING_OR_RISK_MODULE"
    elif any(x in title_lower for x in ["vwap", "avwap", "relative strength", "stage analysis", "canslim", "market regime"]):
