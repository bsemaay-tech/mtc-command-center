...
def classify_item(doc: SourceDoc | None, intake: SourceDoc | None, faithfulness: str, corrected_path: str = "") -> dict:
    ...
    intake_text = (intake.text if intake else "").lower()
    ...
    elif any(x in title_lower + intake_text[:4000] for x in ["sell rule", "exit module", "profit taking", "risk management", "position sizing", "sizing"]):
        classification = "SIZING_OR_RISK_MODULE" if any(x in title_lower + intake_text[:4000] for x in ["risk", "sizing", "position size"]) else "EXIT_MODULE"
    elif any(x in title_lower + intake_text[:4000] for x in ["vwap", "avwap", "relative strength", "stage analysis", "canslim", "market regime", "sector"]):
        classification = "FILTER_MODULE"
    else:
        scoring_text = title_lower + "\n" + intake_text[:8000]
        day_score = sum(1 for x in ["low float", "small cap", "small-cap", "intraday", "day trading", "day-trading", "opening range", "halt", "premarket"] if x in scoring_text)
        swing_score = sum(1 for x in ["swing", "vcp", "pullback", "breakout", "wedge", "flag", "base", "retest", "relative strength"] if x in scoring_text)
        position_score = sum(1 for x in ["weekly", "position trading", "position_trading", "tqqq", "longer-term", "long term", "stage 2", "stage analysis"] if x in scoring_text)
        if "swing" in title_lower:
            swing_score += 3
        if "weekly" in title_lower or "position" in title_lower:
            position_score += 3
        if "day trader" in title_lower or "day trading" in title_lower:
            day_score += 3
        best = max(day_score, swing_score, position_score)
        if best >= 2 and day_score == best:
            classification = "DAY_TRADE_STRATEGY"
        elif best >= 2 and position_score == best:
            classification = "POSITION_TRADING_STRATEGY"
        elif best >= 2 and swing_score == best:
            classification = "SWING_TRADE_STRATEGY"
        elif any(x in scoring_text for x in ["automation", "workflow", "scanner", "ai"]):
            classification = "PROCESS_OR_WORKFLOW_ONLY"
        elif any(x in scoring_text for x in ["psychology", "mental", "process", "discipline", "overtrading", "jared tendler"]):
