...
DEFAULT_TRAIN_CASE = "configs/cases/supertrend_sl_wf_train_20260308.json"
DEFAULT_TARGET1_CASE = "configs/cases/supertrend_sl_wf_target1_20260308.json"
DEFAULT_TARGET2_CASE = "configs/cases/supertrend_sl_wf_target2_20260308.json"
PROFILE_SPACE_FILES = {
    "atr": "configs/spaces/supertrend_sl_walkforward_atr_20260308.json",
    "pct": "configs/spaces/supertrend_sl_walkforward_pct_20260308.json",
    "swing_plus_atr": "configs/spaces/supertrend_sl_walkforward_swing_plus_atr_20260308.json",
    "swing_atr": "configs/spaces/supertrend_sl_walkforward_swing_atr_20260308.json",
}
...
def run_profile(profile: str, args: argparse.Namespace, outdir: Path) -> int:
    ...
    print(f"SL walk-forward profile: {profile}")
...
def write_aggregate(base_outdir: Path, rows: list[dict]) -> tuple[Path, Path]:
    csv_path = base_outdir / "sl_profile_summary.csv"
    json_path = base_outdir / "sl_profile_summary.json"
        ...
        if not df.empty:
            ...
            if candidate_name:
                ...
                if candidate_path.exists():
                    ...
                    stop_loss = candidate.get("config", {}).get("stop_loss", {})
                    params = candidate.get("params", {})
                    payload["stop_loss.mode"] = stop_loss.get("mode", params.get("stop_loss.mode"))
                    payload["stop_loss.atr_len"] = stop_loss.get("atr_len", params.get("stop_loss.atr_len"))
                    payload["stop_loss.atr_mult"] = stop_loss.get("atr_mult", params.get("stop_loss.atr_mult"))
                    payload["stop_loss.percent"] = stop_loss.get("percent", params.get("stop_loss.percent"))
                    payload["stop_loss.swing_basis"] = stop_loss.get("swing_basis", params.get("stop_loss.swing_basis"))
                    payload["stop_loss.swing_lookback"] = stop_loss.get("swing_lookback", params.get("stop_loss.swing_lookback"))
                    payload["stop_loss.swing_atr_len"] = stop_loss.get("swing_atr_len", params.get("stop_loss.swing_atr_len"))
                    payload["stop_loss.swing_atr_mult"] = stop_loss.get("swing_atr_mult", params.get("stop_loss.swing_atr_mult"))
    return payload


def main() -> int:
    parser = argparse.ArgumentParser(description="Run Supertrend stop-loss walk-forward optimization.")
    ...
    parser.add_argument("--mode", choices=["grid", "random", "bayes"], default="random")
    ...
    parser.add_argument("--candidate-source", choices=["pareto", "train_top_score"], default="pareto")
    ...
    if args.outdir:
        ...
        base_outdir = (PROJECT_ROOT / "results" / "walkforward" / f"supertrend_sl_{stamp}").resolve()
    ...
    if rows:
        csv_path, json_path = write_aggregate(base_outdir, rows)
