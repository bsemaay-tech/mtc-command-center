        ...
        *,
        ...
        return {
            ...
            GATE_CANDLE_PATTERN: evaluate_candle_pattern_gate(
                self.config,
                # Include the current bar so the window matches Pine's
                # `ta.barssince(pattern) < candle_pattern_lookback` semantics,
                # which counts from bar[0] (current bar) inclusive.
                # The deque holds at most (lookback+1) previous bars; adding
                # the current bar and slicing to the last (lookback+1) entries
                # gives exactly the same window Pine evaluates.
                recent_bars=(
                    list(self._candle_history) + ([bar] if bar is not None else [])
                )[-(int(self.config.get("candle_pattern_lookback", 5)) + 1):],
            ),
