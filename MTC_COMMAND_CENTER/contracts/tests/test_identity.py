...
def test_environment_lineage_is_excluded_from_all_three_hash_formulae():
    ...
    evaluation_hash = compute_evaluation_run_hash(
        package_hash=package_hash,
        dataset_manifest_sha="2" * 64,
        cost_model_json={},
        simulator_class="CANONICAL",
        simulator_version="1",
        evaluation_config_json={},
    )
    deployment_hash = compute_deployment_identity_hash(
        package_hash=package_hash,
        allocator_code_sha="3" * 64,
        allocation_policy_version="allocation-v1",
        guardian_code_sha="4" * 64,
    ...
    assert len({package_hash, evaluation_hash, deployment_hash}) == 3
    for function in (
        compute_package_hash,
        compute_evaluation_run_hash,
        compute_deployment_identity_hash,
        ...
        with pytest.raises(TypeError):
            function(environment_lineage={"os": "Windows"})
