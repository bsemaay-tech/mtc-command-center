...
## 🔷 STRATEGY PARAM-SPEC REGISTRY — Faz 1 DONE 2026-07-01 (Claude Opus 4.8), Faz 2/3 pending
Declarative per-strategy param spec built read-only on branch `feature/strategy-param-specs` (3 commits, NOT merged/pushed). Generator `03_QUANTLENS/tools/build_strategy_param_specs.py` (code=truth) + overlay `05_REGISTRY/STRATEGY_PARAM_SPEC_ANNOTATIONS.json` → `05_REGISTRY/STRATEGY_PARAM_SPECS.json` (20 strat, 1122 combos, 1,201,662 cases). Surfaced in dashboard Strategy Detail §4 (optimizable grid + case count + fixed/missing knobs + exec model). API 112 passed, render verified.
**Next [AI: Barış]:** approve **push + PR** for Faz 1. **Then [AI: Claude, approval-gated]:** Faz 2 = fill `mtc_v2_parity` map (QuantLens param → Pine `input()`, parity_required flag) — touches Pine/MTC_V2 reasoning, needs approval. Faz 3 = missing-knob R&D (opposite-channel exit, short, promote fixed knobs like TRIPLE_EMA 5/13/50) as NEW variants in VARIANT_LOG with fresh validation — new logic, approval + two-stage (broad→pre-registered confirmation) required. Also [AI: Any]: write the canonical `case = Σgrid × cells × folds` + "case vs cell vs eval" definition into the runbook (was undocumented).
