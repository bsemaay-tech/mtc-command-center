...
1. **Night-runs → MCC** `[AI: Codex]` — `full_sweep_2026-06-07` (122) and `batch023_034_2026-06-07` (111) enriched into `scorecard_v2` on 2026-06-08 by Codex GPT-5; MCC scorecard reader now sees 593 total cards / 46 distinct strategies. Remaining: `night_1m_2026-06-07` (122) still needs export diagnosis because the run dir has `gate2_scorecards/` but no top-level `MEGA_walk_forward_results.json`, so `mcc_night_tail.sh` cannot run directly.
2. **UI Round-2 remainder** `[AI: Codex]` — R2-13-deep (per-sub-score reason, backlog), R2-04/05 (verdict/badge ladders, backlog). R2-31 fixed 2026-06-08 by Codex GPT-5: Strategy Detail now surfaces the selected scorecard file timestamp, falling back to snapshot timestamp only when no scorecard is linked. R2-36 closed 2026-06-08 by Codex GPT-5 as a no-code audit: Gate2 tooltip references real emitted `metrics.wfo_pass`, not a ghost requirement. Then Barış screenshot-re-check. Plan: `_AI_MEMORY/UI Reviev/ROUND2_PLAN.md`.
