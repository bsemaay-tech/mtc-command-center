...
4. **Phase 4 — research/UI pilots (branch-isolated)** `[AI: Claude|Barış]` — Claude-Video, Impeccable, Design-Extract, Taste-Skill on `feature/ui-*` only; no data-contract/registry/backtest change.
   ...
   - Remaining Phase 4: **Claude-Video** (needs a real strategy video).
   - OPEN THREAD (2026-06-22): two doc-only branches are UNMERGED — their pilot/audit docs are not on master yet. Merge to master (keep master's per-tool §5 rows on conflict, take the branch's pilot doc), then delete both (local+remote): `feature/ui-design-extract` (→ `pilots/design-extract_pilot.md`) and `feature/audit-second-eyes` (→ `09_DOCS/AI_TOOLING/SECOND_EYES_AUDIT_2026-06-22.md`). Also clean `C:\tmp` leftovers (design_extract_out, second_eyes_*).
