...
4. **Phase 4 — research/UI pilots (branch-isolated)** `[AI: Claude|Barış]` — Claude-Video, Impeccable, Design-Extract, Taste-Skill on `feature/ui-*` only; no data-contract/registry/backtest change.
   - DONE: **Impeccable** (Strategy Detail polish, `feature/ui-impeccable-pilot`, merged to master).
   - DONE 2026-06-22: **Design-Extract** (`designlang` npm; Node24/MIT/Playwright `--system-chrome`; no key) piloted on Linear(dark) on `feature/ui-design-extract` → **KEEP on-demand for inspiration only (not copy)**. 35+ artifacts (DTCG tokens/Tailwind/Figma/motion/agent DESIGN.md) to `C:\tmp` (git-noise → out of repo). Cosmetic post-step crash (exit 1, outputs land); landing-page sampled not app dashboard → for data-density point at a real app screen via `--cookie-file`. Overlaps browser MCPs + our DESIGN.md but adds token extraction. Report `09_DOCS/AI_TOOLING/pilots/design-extract_pilot.md`. Dashboard application DEFERRED to a scoped follow-up UI task. Remaining Phase 4: Taste-Skill, Claude-Video (needs real video).
