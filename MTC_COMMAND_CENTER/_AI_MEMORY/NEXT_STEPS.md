...
## ✅ CRYPTO PAPER BRIDGE P2 — ARMED, DAY 0 v4 = 2026-07-15T12:02:42.856537Z [AI: Any]

Outage-tolerance fix live in `C:\P2RT` (detached `1465f8f0`): reconcile N=3 consecutive-strike
tolerance + ~5-min reconnect budget before DATA_STALE + Telegram notify-threshold. Run
`paper-20260715105547`, exactly one ARM, zero RECONCILE_FAILED / _TOLERATED / DATA_STALE since,
positions/orders `[]`, 130 tests both CWDs. **All PRs #16–#19 MERGED → master `8721bce0`.**
Full record: GLOBAL_HANDOFF `[Claude Fable 5] 2026-07-15 — DEPLOY (Day 0 v4) + PR MERGE AUDIT`.

- **[AI: Any]** daily read-only D3 check: state/reconcile freshness, WARN+ events, equity,
  positions/orders + native stops, pinned identity (`git -C C:/P2RT log -1` = detached
  `1465f8f0`, clean). Benign: ~10-min DISCONNECT→RECONNECT attempt=1→DATA_RESTORED (now
  suppressed from Telegram). A single `RECONCILE_FAILED_TOLERATED` (WARN, no disarm) during a
  real outage is the CORRECT new behaviour.
- **[AI: Claude|Codex]** any safety anomaly: preserve evidence, DISARM if needed, diagnose; no
  re-ARM without a fresh complete gate incl. verified fresh bars.
- **PC uptime + VPS (Barış 2026-07-15):** ON → Jul 18 (~2h off) → Jul 20 (~2h off) → 6 days →
  pattern; **VPS end of month.** No pre-VPS window reaches ≥10 uninterrupted days → **Day 0 v4 is
  policy VALIDATION; the definitive ≥10-day D3 clock starts on the VPS.** Planned PC-offs are
  window boundaries, NOT safety incidents.
- Prior P2 window chain (v1 13:00Z auto-disarm, v2 15:17Z race-death, v3 06:48Z outage-death → v4)
  is fully in GLOBAL_HANDOFF; superseded NEXT_STEPS task sections trimmed here 2026-07-15.
- **[AI: Barış, when ready]** exit-aware gauntlet tooling (FAZ 3B forward prerequisite) — plan
  written `11_TRIAGE/EXIT_AWARE_GAUNTLET_TOOLING_PLAN_2026-07-15.md`; approve to build.
