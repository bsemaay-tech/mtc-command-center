...
> **Z.AI GLM routing:** when sub-delegating via Z.AI Coding Plan (GLM-4.7 / GLM-5.2 / GLM-5-Turbo) instead of DeepSeek/Grok, follow `AGENTS.md` §GLM SUPPLEMENTAL ROUTING (canonical). Same two-tier enforcement: implementer-tier only; Lead does not dispatch GLM directly; GLM does not replace the mandatory audit roster.
