...
- **Mandatory Gemini comparison (OD-20260829-1):** every T0/T1/T2 Gate 5 model-audit dispatch also sends a fresh `gemini-3.7-flash-high` review through `Invoke-GeminiProReadOnly.ps1`, in parallel and with the same scope, allowed/forbidden paths, actual diff/files, test evidence, and repo rules. The Lead records common findings, Gemini-only findings, contradictions, and the independently reproduced disposition of each required finding. A missing Gemini run blocks audit completion unless Barış waives it. Gemini is read-only and cannot run the mandated suite, so label its report `SUPPLEMENTAL_UNEXECUTED`; that limitation alone does not block the acceptance auditors or replace their acceptance role. Because its launcher reads only the canonical checkout, an audit of another worktree must provide Gemini the exact SHA plus literal, safety-redacted diff and relevant file slices; never substitute canonical-checkout contents for the audited source. T3 remains self-verification-only unless a later owner task explicitly requests a model audit.
...
**Gemini comparison:** under OD-20260829-1, dispatch the same Gate 6 review package in parallel to fresh `gemini-3.7-flash-high` through `Invoke-GeminiProReadOnly.ps1`; compare its findings with the required auditors and reproduce each required finding before disposition. Its read-only, no-suite-execution report is supplemental and never replaces T0 acceptance.
