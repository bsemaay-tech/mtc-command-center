...
HER backtest / optimizasyon icin (in-day tek strateji, sprint, overnight - fark etmez) **zorunlu pre-read iki dosya:**
...
Akis: `..\04_SHARED\prompts\05_ai_workflow\08_backtest_launch.md`. Single-strategy 5dk run dahi 4-gate (rolling WF + bootstrap+BH-FDR + DSR + multi-window) + buy&hold karsilastirmasi olmadan promotable degil.

**Veri + calistirma (kanonik):** Hangi datayi kullanacagini bilmeden backtest calistirma. Otorite envanter: `..\03_QUANTLENS\data\README.md` (tum native bundle'lar - sembol/timeframe/asset-class/bar sayisi + crypto data konumlari). US equities / ETF / multi-asset / 10m **var**; engine'in hardcoded default manifest'i eski crypto arsivi, guncel data DEGIL. Engine data secimi: `MEGA_BUNDLE_MANIFEST` env -> bir bundle'in `manifests\dataset_manifest.json`'u + `--symbol`/`--tf`. Kanonik tek-kosu: `python ..\03_QUANTLENS\tools\mega_walk_forward.py --strategy <id> --symbol <SYM> --tf <tf>` (research engine; `walk_forward_processor.py` alt-seviye/custom). Birincil bundle: `native_multiasset_alpaca_2026-06-28`.
