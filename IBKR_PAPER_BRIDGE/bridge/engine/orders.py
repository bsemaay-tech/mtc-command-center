    ...
    async def submit_plan(
        ...
        try:
            ...
            if hasattr(self.store, "insert_event"):
                self.store.insert_event(
                    ...
                    f"decision_uid={original_decision_uid} intent_id={intent_id} error={type(exc).__name__}: {exc}",
                )
            raise
        finally:
        ...
        try:
            ...
            self.store.insert_event(
                ...
                f"intent_id={intent_id} decision_uid={original_decision_uid} error={exc.code if hasattr(exc, 'code') else type(exc).__name__}",
            )
            raise
        except Exception as exc:
            self.store.insert_event(
                ...
                "ERROR",
                "IDENTITY_FINALIZE_FAILED",
                f"intent_id={intent_id} decision_uid={original_decision_uid} error={type(exc).__name__}: {exc}",
            )
            raise
