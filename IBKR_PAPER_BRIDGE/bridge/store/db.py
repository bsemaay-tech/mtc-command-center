...
def compute_intent_identity(
    ...
    Requires timezone-aware signal_ts; rejects naive datetimes.
    """
...
class Store:
    ...
    def _migrate_v2_to_v3(self) -> None:
        ...
        inside a single BEGIN IMMEDIATE transaction. On any error the rollback
        leaves schema_version=2, all legacy data unchanged, and no order_identity
        table/index residue.
        """
    ...
    def _migrate_v2_to_v3_in_tx(self) -> None:
        ...
        if not fp_rows:
            # No fingerprints to backfill — just bump version and commit
            self.conn.execute(
        ...
        for fp_row in fp_rows:
            ...
            # Validate SIGNAL and order_plan semantics agree
            ...
            # Look for orders with this exact decision_uid AND a valid trade
            ...
            if order_rows:
                # Verify each order's trade exists and is consistent
                for o_row in order_rows:
                    if o_row["trade_id"] is not None:
                        trade = self.conn.execute(
                            "SELECT entry_decision_uid FROM trades WHERE trade_id = ?",
                            (o_row["trade_id"],),
                        ).fetchone()
                        if trade is None:
                            raise MigrationError(
                        ...
                        # Verify trade's entry_decision_uid matches
        ...
        # All rows migrated successfully → bump version and commit
        self.conn.execute(
            "INSERT OR REPLACE INTO meta(key, value) VALUES ('schema_version', '3')"
        )
        self.conn.commit()
