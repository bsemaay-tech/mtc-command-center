...
class Store:
    ...
    def _migrate_v2_to_v3(self) -> None:
        ...
        # Ensure all v2 tables exist before we start reading them
        self._create_tables_v3()

        # Check if identity table already has rows (partial migration guard)
        existing_count = self.conn.execute(
            "SELECT COUNT(*) FROM order_identity"
        ).fetchone()[0]
        if existing_count > 0:
            raise MigrationError(
                "order_identity table already has rows but schema_version is 2; "
                "aborting to prevent duplicate migration"
            )

        with self.conn:
            # Read all signal_fingerprints
            ...
            # We need the strategy id for backfill. The current single strategy.
            ...
            for fp_row in fp_rows:
                ...
                # 1. Find SIGNAL decision payload
                sig_row = self.conn.execute(
                    """SELECT payload_json FROM decisions
                       WHERE decision_uid = ? AND stage = 'SIGNAL'
                       ORDER BY id ASC LIMIT 1""",
                    (decision_uid,),
                ).fetchone()
                if sig_row is None:
                    raise MigrationError(
                        f"No SIGNAL decision found for decision_uid={decision_uid} "
                        f"(run_id={run_id}); cannot backfill identity"
                    )
                try:
                ...
                # Extract required fields from SIGNAL payload
                ...
                # 2. Find RISK_PASS decision payload
                risk_row = self.conn.execute(
                    """SELECT payload_json FROM decisions
                       WHERE decision_uid = ? AND stage = 'RISK_PASS'
                       ORDER BY id ASC LIMIT 1""",
                    (decision_uid,),
                ).fetchone()
                if risk_row is None:
                    raise MigrationError(
                        f"No RISK_PASS decision found for decision_uid={decision_uid}; "
                        f"cannot reconstruct request identity"
                    )
                try:
                ...
                request_id, request_preimage, request_version = compute_request_identity(
                    ...
                    ref_price=float(plan_ref_price),
                    qty=float(plan_qty),
                ...
                # 3. Check for existing identity row with same intent_id but different request
                existing = self.conn.execute(
                    "SELECT request_id, intent_preimage, request_preimage FROM order_identity WHERE intent_id = ?",
                    (intent_id,),
                ).fetchone()
                if existing is not None:
                    ...
                    # Same intent_id + same request_id → skip (already migrated)
                    continue

                # Also check request_id uniqueness
                ...
                # 4. Determine state: LEGACY_SUBMITTED vs LEGACY_RESERVED
                order_rows = self.conn.execute(
                    "SELECT cloid FROM orders WHERE decision_uid = ? LIMIT 1",
                    (decision_uid,),
                ).fetchall()
    ...
    def finalize_submission(
        ...
        All steps execute in one SQLite transaction. If any step fails, the
        entire transaction rolls back. The already-committed reservation
        remains RESERVED.

        Returns the new trade_id.
        """
        with self.conn:
            # 1. Verify identity is still RESERVED
            ident = self.conn.execute(
                "SELECT state FROM order_identity WHERE intent_id = ?",
                (intent_id,),
            ).fetchone()
            if ident is None:
                raise IdentityCollisionError(
            ...
            # 4. Transition RESERVED → SUBMITTED
            now = _to_iso(self._clock())
            self.conn.execute(
    ...
    def _insert_order_in_tx(
        ...
        Collision-safe: on PK conflict, compares immutable identity fields.
        ...
        try:
            ...
            # Compare immutable identity fields
    ...
    def insert_order(
        ...
        try:
            ...
            # Idempotent replay — update mutable fields
            self.conn.execute(
                """UPDATE orders
                   SET status = ?, filled_qty = ?, avg_fill_px = ?,
                       ts_last = ?, order_json = ?
