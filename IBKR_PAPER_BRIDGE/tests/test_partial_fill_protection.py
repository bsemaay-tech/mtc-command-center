...
class RecordingBroker:
    ...
    def lot_unit(self, symbol: str) -> LotUnit:
        return LotUnit(3)
...
def add_owned_stop(
    ...
    *,
    ...
    """Add one live SL proved owned by the active trade lineage."""
    recovery = store.latest_partial_recovery_for_symbol("BTC")
    assert recovery is not None
    store.insert_order(
        cloid=cloid,
        oid=100 + len(broker.orders),
        group_id=REQUEST_ID,
        order_ref=f"{recovery['entry_decision_uid']}:SL:PRIOR",
...
def test_two_exact_owned_live_stops_fail_closed_shared_protection_decision():
    snapshot = SymbolSnapshot(
        ...
        open_orders=(
            OrderView(
                cloid="owned-sl-a", coin="BTC", side="SELL", size=1.0,
                role="SL", reduce_only=True, status="OPEN",
            ),
            OrderView(
                cloid="owned-sl-b", coin="BTC", side="SELL", size=1.0,
                role="SL", reduce_only=True, status="OPEN",
            ),
        ),
        lot=LotUnit(3),
        evidence=Evidence("TEST", "DUPLICATE_OWNED_STOPS"),
    )
    manager = object.__new__(OrderManager)

    assert manager.qualifying_protection(
        snapshot,
        position_lots=1000,
        exit_side="SELL",
        owned_cloids={"owned-sl-a", "owned-sl-b"},
    ) is None


@pytest.mark.parametrize("stale_size", [0.5, 1.5])
def test_stale_prior_generation_stop_is_cancelled_and_proved_before_acceptance(
    tmp_path, stale_size
):
    store, broker, manager, _clock, _mono = scenario(tmp_path)
    add_owned_stop(
        store, broker, cloid=f"prior-generation-sl-{stale_size}", size=stale_size
    )

    state = asyncio.run(manager.run_partial_recovery("BTC", max_cycles=2))

    recovery = store.latest_partial_recovery_for_symbol("BTC")
    assert state == PartialProtectionState.PROTECTION_PENDING.value
    actions = store.partial_actions_for_recovery(
        str(recovery["recovery_id"]), "CANCEL_PROTECTION"
    )
    assert len(actions) == 1
    assert actions[0]["target_cloid"] == f"prior-generation-sl-{stale_size}"
    events = store.partial_action_events(str(actions[0]["action_id"]))
    assert [event["status"] for event in events[:2]] == ["RESERVED", "SENT"]
    assert any(
        event["reason_code"] == "STALE_PROTECTION_QUERY_RESOLVED"
        and event["status"] == "APPLIED"
        for event in events
    )
    assert (
        "query_order", f"prior-generation-sl-{stale_size}"
    ) in broker.partial_calls

    state = asyncio.run(manager.run_partial_recovery("BTC"))

    assert state == PartialProtectionState.PROTECTED_PARTIAL.value
    assert broker._find_order(f"prior-generation-sl-{stale_size}")["status"] not in {
        "OPEN", "SUBMITTED", "PENDING", "RESTING", "PARTIALLY_FILLED",
        "PENDING_CANCEL",
    }


def test_unknown_stale_stop_cancel_is_query_only_until_same_identity_not_applied(
    tmp_path,
):
    store, broker, manager, _clock, _mono = scenario(tmp_path)
    add_owned_stop(store, broker, cloid="prior-generation-sl", size=0.5)
    broker.scripted_cancel.append(
        ScriptedOutcome(
            ActionOutcome.UNKNOWN,
            applied=False,
            reason_code="MOCK_STALE_CANCEL_TIMEOUT",
        )
    )
    ...
    assert state == PartialProtectionState.PROTECTION_PENDING.value
    assert len(actions) == 1
    stable_action_id = str(actions[0]["action_id"])
    assert store.resolve_partial_action(stable_action_id) == ActionOutcome.UNKNOWN.value
    assert [
        call for call in broker.partial_calls
        if call == ("cancel_order_by_cloid", "prior-generation-sl")
    ] == [("cancel_order_by_cloid", "prior-generation-sl")]

    # UNKNOWN replay is evidence-query only: no second cancel and no accepting state.
    state = asyncio.run(manager.run_partial_recovery("BTC"))
    assert state == PartialProtectionState.PROTECTION_PENDING.value
    assert [
        call for call in broker.partial_calls
    ...
    assert store.partial_actions_for_recovery(
        str(recovery["recovery_id"]), "CANCEL_PROTECTION"
    )[0]["action_id"] == stable_action_id

    # Direct live evidence proves NOT_APPLIED, still without a resend.
    broker.partial_query_available = True
    state = asyncio.run(manager.run_partial_recovery("BTC", max_cycles=1))
    assert state == PartialProtectionState.PROTECTION_PENDING.value
    assert store.resolve_partial_action(stable_action_id) == (
    ...
    # Only now may the same immutable identity be sent again.
    state = asyncio.run(manager.run_partial_recovery("BTC"))
    assert state == PartialProtectionState.PROTECTED_PARTIAL.value
    assert len([
        call for call in broker.partial_calls
    ...
    replayed = store.partial_actions_for_recovery(
        str(recovery["recovery_id"]), "CANCEL_PROTECTION"
    )
    assert [row["action_id"] for row in replayed] == [stable_action_id]


@pytest.mark.parametrize("snapshot_exact", [True, False])
def test_foreign_or_unverified_extra_stop_is_never_cancelled(
    tmp_path, snapshot_exact
):
    store, broker, manager, _clock, _mono = scenario(tmp_path)
    assert asyncio.run(manager.run_partial_recovery("BTC", max_cycles=1)) == (
        PartialProtectionState.PROTECTION_PENDING.value
    )
    broker.partial_foreign_orders.append(
        {
            "cloid": "foreign-extra-sl",
            "role": "SL",
            "status": "OPEN",
            "qty": 0.5,
            "symbol": "BTC",
    ...
    broker.partial_snapshot_exact = snapshot_exact
    ...
    assert ("cancel_order_by_cloid", "foreign-extra-sl") not in broker.partial_calls
...
def test_ordinary_reconcile_rejects_duplicate_exact_owned_live_stops(tmp_path):
    store = make_store(tmp_path, version=4)
    trade_id, decision_uid = seed_trade(store, qty=1.0)
    open_orders = []
    for index, cloid in enumerate(("sl-a", "sl-b"), start=11):
        store.insert_order(
            cloid=cloid, oid=index, group_id="g",
            order_ref=f"{decision_uid}:SL:{cloid}",
            order_json={"symbol": "BTC", "role": "SL", "trigger_px": 95.0},
            decision_uid=decision_uid, trade_id=trade_id, role="SL",
            status="OPEN", qty=1.0,
        )
        open_orders.append(
            BrokerOrder(
                cloid=cloid, oid=index, coin="BTC", side="SELL", size=1.0,
                status="OPEN", role="SL", reduce_only=True, trigger_px=95.0,
            )
        )
    broker = RecordingBroker(
...
def test_human_rearm_rejects_duplicate_exact_owned_live_stops(tmp_path):
    store, broker, manager, _clock, _mono = scenario(tmp_path)
    assert asyncio.run(manager.run_partial_recovery("BTC")) == (
        PartialProtectionState.PROTECTED_PARTIAL.value
    )
    recovery = store.latest_partial_recovery_for_symbol("BTC")
    add_owned_stop(store, broker, cloid="duplicate-owned-sl", size=1.0)
    before = list(broker.broker_mutations)

    proved = asyncio.run(
        manager.confirm_partial_rearm(str(recovery["recovery_id"]))
    )

    assert proved is False
    assert broker.broker_mutations == before
    assert ("cancel_order_by_cloid", "duplicate-owned-sl") not in broker.partial_calls
    assert store.partial_recoveries_awaiting_rearm()
...
@pytest.mark.parametrize("target", [3, 7, True, "5", 4.0])
