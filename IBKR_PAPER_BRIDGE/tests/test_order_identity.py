...
def test_repair2_4_bad_intent_id_prefix_rejected(tmp_path):
    """Wrong intent_id prefix must be rejected by CHECK constraint."""
    db_path = tmp_path / "bridge.db"
    store = Store(db_path)
    store.initialize()
    store.create_run("run-1", "dry_run", "testnet", {})
...
def test_repair2_6_tzinfo_with_null_utcoffset_rejected(tmp_path):
    ...
    # Create a custom tzinfo that returns None for utcoffset.
    # Must use type() because datetime.tzinfo cannot be subclassed normally.
