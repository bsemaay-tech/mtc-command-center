...
def test_hyperliquid_recovery_missing_and_failed_apis_are_not_not_found():
    ...
    class NoneInfo:
        def query_order_by_cloid(self, *args):
            return None
...
def test_hyperliquid_complete_recovery_cycle_uses_installed_sdk_cloid_contract():
    class NetworkFreeInfo(Info):
        def __init__(self):
            self.order_status_payloads = []
            self.oid_lookup = MagicMock(
                side_effect=AssertionError("OID lookup must not be used for CLOID recovery")
            )

        def post(self, path, payload):
            assert path == "/info"
            json.dumps(payload)
            self.order_status_payloads.append(payload)
            return {"status": "unknownOid"}

        @property
        def query_order_by_oid(self):
            return self.oid_lookup
    ...
    info = NetworkFreeInfo()
    assert type(info).query_order_by_cloid is Info.query_order_by_cloid
    broker = HyperliquidBroker(info_client=info, exchange_client=MagicMock())
    request = type("Request", (), {
    ...
    evidence = asyncio.run(broker.submission_recovery_evidence(request))
    expected_cloids = set(request.planned_cloids.values())
    assert len(info.order_status_payloads) == len(expected_cloids)
    assert {
        payload["oid"] for payload in info.order_status_payloads
    } == expected_cloids
    assert all(
        payload["type"] == "orderStatus"
        and isinstance(payload["oid"], str)
        for payload in info.order_status_payloads
    )
    info.oid_lookup.assert_not_called()
    assert all(
