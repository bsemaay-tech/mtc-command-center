...
| 6 | Engine + OrderManager on mock (90m) | engine.py state machine §5, orders.py incl. trail modify, duplicate-signal guard, stale-price guard, reduce-only close semantics, reconciler | `test_engine_dryrun`: full replay on fixture → ≥1 complete trade in DB with full decision chain; induced reject → DISARM; naked-position sim → flatten; same-bar signal re-delivery → exactly one order |
...
| 10 | Dashboard (150m) | index.html/app.css/app.js per §9 — all 6 pages incl. Gate Monitor card, dark theme, lightweight-charts CDN, WS-driven, ARM/DISARM/KILL flows w/ modals | Manual: dry-run replay visible live (equity curve draws, decisions stream, gate card colors, journal drawer shows full chain); screenshot into `docs/screenshots/` |
