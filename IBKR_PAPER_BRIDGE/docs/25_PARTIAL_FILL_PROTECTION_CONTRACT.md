...
`reduceOnly`, or non-finite/non-positive sizes produce inexact/unknown
evidence. Successful membership in the SDK's `Info.open_orders` list is the
authoritative live/open status; those SDK rows do not require an embedded
`status`. Recovery parses a copied row normalized to `OPEN`, without mutating
the SDK response. `reduceOnly` is never inferred. Direct single-order query
statuses remain subject to the explicit live and terminal whitelists.
